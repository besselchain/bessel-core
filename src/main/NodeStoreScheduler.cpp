...
void NodeStoreScheduler::scheduleTask (NodeStore::Task& task)
{
    ...
    m_jobQueue->addJob (jtWRITE, "NodeObject::store", std::bind (&NodeStoreScheduler::doTask, this, std::ref(task), std::placeholders::_1));
}
...
void NodeStoreScheduler::onFetch (NodeStore::FetchReport const& report)
{
    ...
    {
        m_jobQueue->addLoadEvents (report.isAsync ? jtNS_ASYNC_READ : jtNS_SYNC_READ, 1, report.elapsed);
    }
}
