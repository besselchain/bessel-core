...
#include <common/misc/io_latency_probe.h>
...
#include <main/BasicApp.h>
#include <main/Tuning.h>
...
#include <common/base/LoggedTimings.h>
#include <common/base/ResolverAsio.h>
#include <common/base/Sustain.h>
...
#include <common/shamap/Family.h>
#include <transaction/paths/FindPaths.h>
#include <transaction/paths/PathRequests.h>
#include <network/peers/UniqueNodeList.h>
#include <network/overlay/make_Overlay.h>
#include <transaction/tx/InboundTransactions.h>
#include <transaction/tx/TransactionMaster.h>
#include <services/net/SNTPClient.h>
#include <services/rpc/Manager.h>
#include <services/server/make_ServerHandler.h>
#include <services/websocket/MakeServer.h>
#include <protocol/STParsedJSON.h>
#include <crypto/RandomNumbers.h>
#include <consensus/validators/make_Manager.h>
#include <data/nodestore/backend/RocksDBQuickFactory.h>
#include <data/nodestore/backend/RocksDBFactory.h>
#include <data/nodestore/backend/MemoryFactory.h>
#include <data/nodestore/backend/NullFactory.h>
#include <data/nodestore/backend/NuDBFactory.h>
...
public:
    ...
    AppFamily (NodeStore::Database& db, CollectorManager& collectorManager)
        : treecache_ ("TreeNodeCache", 65536, 60, get_seconds_clock(), deprecatedLogs().journal("TaggedCache")),
...
//  TODO Move the function definitions into the class declaration
...
private:
    ...
    private:
        ...
        bessel::io_latency_probe<std::chrono::steady_clock> m_probe;
    ...
    public:
        ...
        void operator() (Duration const& elapsed)
        {
            ...
            if (ms.count() >= 500)
                m_journal.warning << "io_service latency = " << ms;
        }
...
public:
    ...
    //  TODO Make OrderBookDB abstract
    ...
    {
    #if BESSEL_SINGLE_IO_SERVICE_THREAD
        return 1;
    #else
    ...
    ApplicationImp (Logs& logs)
      : RootStoppable ("Application")
        ...
        //  NOTE must come before NetworkOPs to prevent a crash due
        ...
        //  NOTE LocalCredentials starts the deprecated UNL service
	...
	bool initMysqlDbs()
	{
		...
		DatabaseCon::Setup setup = setup_DatabaseCon(getConfig());
		mTxnDB = std::make_unique <DatabaseCon>(setup, "transaction",
			TxnDBInit, TxnDBCount);
		mLedgerDB = std::make_unique <DatabaseCon>(setup, "ledger",
			LedgerDBInit, LedgerDBCount);
		mWalletDB = std::make_unique <DatabaseCon>(setup, "wallet",
			WalletDBInit, WalletDBCount);
    ...
    //  TODO Break this function up into many small initialization segments.
    ...
    void setup ()
    {
		...
		*/
        ...
        //  NOTE Unfortunately, in stand-alone mode some code still
        ...
        if (!getConfig ().RUN_STANDALONE)
        {
            ...
            //  NOTE the state timer resets the deadlock detector.
            //
    ...
    void onStop ()
    {
        ...
        //  Enormous hack, we have to force the probe to cancel
    ...
    void run ()
    {
        //  NOTE I put this here in the hopes that when unit tests run (which
        ...
        if (!getConfig ().RUN_STANDALONE)
        {
            //  NOTE This seems unnecessary. If we properly refactor the load
    ...
    void exitWithCode (int code)
    {
        ...
        //  This breaks invariants: automatic objects
    ...
    void onDeadlineTimer (beast::DeadlineTimer& timer)
    {
        ...
        {
            //  TODO Move all this into doSweep
            boost::filesystem::space_info space = boost::filesystem::space (getConfig ().legacy ("database_path"));

            //  TODO Give this magic constant a name and move it into a well documented header
            ...
            m_jobQueue->addJob (jtSWEEP, "sweep", std::bind (&ApplicationImp::doSweep, this, std::placeholders::_1));
        }
    }

    void doSweep (Job& j)
    {
        //  NOTE Does the order of calls matter?
        //  TODO fix the dependency inversion using an observer,
        ...
        #if (defined BEAST_LINUX)
	logTimedCall(m_journal.warning, "SHAMap::clear_memory", __FILE__, __LINE__, std::bind(
	    &TreeNodeCache::clear_memory, &family().treecache ()));
        #endif
        ...
        //  NOTE does the call to sweep() happen on another thread?
...
{
    // New stuff.
    BesselAddress   rootSeedMaster      = BesselAddress::createSeedGeneric ("masterpassphrase");
    BesselAddress   rootGeneratorMaster = BesselAddress::createGeneratorPublic (rootSeedMaster);
    BesselAddress   rootAddress         = BesselAddress::createAccountPublic (rootGeneratorMaster, 0);
    ...
    try
    {
            ...
            else
            {
                 ...
                 else
                 {
                     ...
                     if (ledger.get ().isMember ("accountState"))
                     {
                          ...
                          {
                              closeTimeResolution = ledger.get ()["close_time_resolution"].asUInt ();
                          }
                          ...
                          {
                              totalCoins = boost::lexical_cast<std::uint64_t> (ledger.get ()["total_coins"].asString ());
                          }
        ...
        else if (ledgerID.length () == 64)
        {
            ...
            if (!loadLedger)
            {
                ...
                auto il = std::make_shared <InboundLedger> (hash, 0, InboundLedger::fcGENERIC, get_seconds_clock ());
                if (il->checkLocal ())
                {
        ...
        {
            loadLedger = Ledger::loadByIndex (boost::lexical_cast<std::uint32_t> (ledgerID));
        }

        if (!loadLedger)
        {
            m_journal.fatal << "No Ledger found from ledgerID=" << ledgerID << std::endl;
        ...
        if (replay)
        {
            ...
            if (!loadLedger)
            {
                ...
                // Try to build the ledger from the back end
                auto il = std::make_shared <InboundLedger> (replayLedger->getParentHash(), 0, InboundLedger::fcGENERIC, get_seconds_clock ());
                if (il->checkLocal ())
                {
                    loadLedger = il->getLedger ();
                }
        ...
        if (replay)
        {
            ...
            for (auto it = txns->peekFirstItem(); it != nullptr; it = txns->peekNextItem(it->getTag ()))
            {
...
// TODO clean this up since it is just a file holding a single member function definition
...
static bool schemaHas (DatabaseCon& dbc, std::string const& dbName, int line, std::string const& content)
{
	...
	{
		if (schema[i].find (content) != std::string::npos)
        {
			return true;
        }
	}
...
static void addTxnSeqField ()
{
	...
	boost::optional<std::string> sociTxnMetaBlob;
    soci::indicator tmi;
    //Blob txnMeta;
	std::string txnMeta;
...
#ifdef USEMYSQL
    session << "Alter TABLE AccountTransactions DROP INDEX AcctTxIndex;";
#else
