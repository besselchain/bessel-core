...
#include <common/base/ThreadName.h>
...
boost::filesystem::path getEntropyFile()
{
    return boost::filesystem::path (getConfig().legacy ("database_path")) / "random.seed";
}
...
void startServer ()
{
    ...
    if (getConfig ().RPC_STARTUP.isArray ())
    {
        for (int i = 0; i != getConfig ().RPC_STARTUP.size (); ++i)
        {
            ...
            RPC::Context context {jvCommand, loadType, getApp().getOPs (), Role::ADMIN};
...
void printHelp (const po::options_description& desc)
{
	std::cerr
		...
		"     account_relation <account> <account>|\"\" [<ledger>]\n"
		"     account_relcp <account> <account>|\"\" [<ledger>]\n"
                         ...
                         std::string const& argument)
{
    ...
    //  TODO Remove dependence on constructing Application object
    std::unique_ptr<Application> app (make_Application (deprecatedLogs ()));
    using namespace beast::unit_test;
    reporter r;
    r.arg (argument);

    bool const failed (r.run_each_if (global_suites (), match_auto (pattern)));
    if (failed)
    {
        return EXIT_FAILURE;
    }
...
int run (int argc, char** argv)
{
    ...
    //  TODO Replace boost program options with something from Beast.
    //
...

    {
        ...
        std::int64_t milliseconds = ((std::int64_t)tv.tv_sec) * 1000 + tv.tv_usec / 1000;

        entropy->timestamp = milliseconds;
    ...
    if (vm.count ("version"))
    {
        std::cout << "besseld version " << BuildInfo::getVersionString () << std::endl;
    ...
    if (iResult == 0)
    {
        ...
        if (vm.count ("rpc_ip"))
        {
            try
            {
                getConfig ().rpc_ip = boost::asio::ip::address_v4::from_string (vm["rpc_ip"].as<std::string> ());
            }
            catch(...)
            {
        ...
        if (vm.count ("rpc_port"))
        {
            ...
            {
                std::cerr << "Invalid rpc_port = " << vm["rpc_port"].as<std::string> ();
...
int main (int argc, char** argv)
{
    ...
    auto constexpr gccver = (__GNUC__ * 100 * 100) + (__GNUC_MINOR__ * 100) + __GNUC_PATCHLEVEL__;

    static_assert (gccver >= 40801, "GCC version 4.8.1 or later is required to compile besseld.");
#endif

    static_assert (BOOST_VERSION >= 105700, "Boost version 1.57 or later is required to compile besseld");
    ...
    auto const result (bessel::run (argc, argv));
