...
// See https://bessel.com/wiki/Transaction_Format#Payment_.280.29
...
public:
    ...
    TER preCheck () override
    {
        ...
        bool const defaultPathsAllowed = !(uTxFlags & tfNoBesselDirect);
	  ...
	  if(getApp().getLedgerMaster ().getClosedLedger()->checkBlackList(uDstAccountID))
        ...
        if (bSWTDirect && !defaultPathsAllowed)
        {
            // Consistent but redundant transaction.
            m_journal.trace << "Malformed transaction: " <<
                "No bessel direct specified for SWT to SWT.";
    ...
    {
        // Bessel if source or destination is non-native or if there are paths.
        std::uint32_t const uTxFlags = mTxn.getFlags ();
        bool const partialPaymentAllowed = uTxFlags & tfPartialPayment;
        bool const limitQuality = uTxFlags & tfLimitQuality;
        bool const defaultPathsAllowed = !(uTxFlags & tfNoBesselDirect);
        ...
        bool const bBessel = bPaths || bMax || !saDstAmount.isNative ();
        // XXX Should bMax be sufficient to imply bessel?

        if (bBessel)
        {
            // Bessel payment with at least one intermediate step and uses
            // transitive balances.
