...
#include <transaction/book/Taker.h>
...
BasicTaker::Rate::divide (Amount const& amount) const
{
    ...
    return bessel::divide (amount, rate_, amount.issue ());
}
...
BasicTaker::Rate::multiply (Amount const& amount) const
{
    ...
    return bessel::multiply (amount, rate_, amount.issue ());
}

BasicTaker::BasicTaker (CrossType cross_type
    ...
    , cross_type_ (cross_type)
{
    ...
    // flavor we expect:
    assert (cross_type != CrossType::XrpToIou || (isSWT (issue_in ()) && !isSWT (issue_out ())));

    assert (cross_type != CrossType::IouToXrp || (!isSWT (issue_in ()) && isSWT (issue_out ())));

    // And make sure we're not crossing SWT for SWT
    assert (!isSWT (issue_in ()) || !isSWT (issue_out ()));
...
BasicTaker::remaining_offer () const
{
   ...
   if (original_ == remaining_)
   {
    ...
    if (sell_)
    {
        ...
        return Amounts (remaining_.in, divRound (remaining_.in, quality_.rate (), issue_out_, true));
    }

    assert (remaining_.out > zero);

    // We scale the input based on the remaining output:
    return Amounts (mulRound (remaining_.out, quality_.rate (), issue_in_, true), remaining_.out);
}
...
BasicTaker::Flow
BasicTaker::flow_xrp_to_iou (Amounts const& order
                            , Quality quality
...
BasicTaker::Flow
BasicTaker::flow_iou_to_xrp (Amounts const& order
                            , Quality quality
                            , Amount const& owner_funds
                            , Amount const& taker_funds
                            , Rate const& rate_in)
...
BasicTaker::Flow
BasicTaker::flow_iou_to_iou (Amounts const& order
                        , Quality quality
                        , Amount const& owner_funds
                        , Amount const& taker_funds
                        , Rate const& rate_in
...
BasicTaker::do_cross (Amounts offer, Quality quality, Account const& owner)
{
    ...
    {
        result = flow_xrp_to_iou (offer, quality, owner_funds, taker_funds, out_rate (owner, account ()));
    }
    else if (cross_type_ == CrossType::IouToXrp)
    {
        result = flow_iou_to_xrp (offer, quality, owner_funds, taker_funds, in_rate (owner, account ()));
    }
    else
    {
        result = flow_iou_to_iou (offer, quality, owner_funds, taker_funds, in_rate (owner, account ()), out_rate (owner, account ()));
    }
    ...
    Amounts offer2, Quality quality2, Account const& owner2)
{
    ...
    // The amount available to flow via SWT is the amount that the owner of the
    ...
    // of SWT that can flow between the two legs is, essentially, infinite
    // since all the owner is doing is taking out SWT of his left pocket
    // and putting it in his right pocket. In that case, we set the available
    // SWT to the largest of the two offers.
    ...
    // equalize them, so that the amount of SWT that flows out of the first leg
    // is the same as the amount of SWT that flows into the second leg. We take
...
std::uint32_t
Taker::calculateRate (LedgerView& view, Account const& issuer, Account const& account)
{
    return isSWT (issuer) || (account == issuer) ? QUALITY_ONE : besselTransferRate (view, issuer);
}

Taker::Taker (CrossType cross_type
            , LedgerView& view
            , Account const& account
            , Amounts const& offer
            , std::uint32_t flags)
...
TER Taker::transfer_xrp (Account const& from, Account const& to, Amount const& amount)
{
    if (!isSWT (amount))
    {
...
TER Taker::redeem_iou (Account const& account, Amount const& amount, Issue const& issue)
{
    if (isSWT (amount))
    {
        throw std::logic_error ("Using redeem_iou with SWT");
    }
...
TER Taker::issue_iou (Account const& account, Amount const& amount, Issue const& issue)
{
    if (isSWT (amount))
    {
        throw std::logic_error ("Using issue_iou with SWT");
    }
    ...
    BasicTaker::Flow const& flow2, Offer const& leg2)
{
    ...
    // leg1 to leg2: bridging over SWT
    if (result == tesSUCCESS)
    {
...
{
    // In direct crossings, at least one leg must not be SWT.
    if (isSWT (offer.amount ().in) && isSWT (offer.amount ().out))
    {
        return tefINTERNAL;
    }

    auto const amount = do_cross (offer.amount (), offer.quality (), offer.owner ());
...
{
    // In bridged crossings, SWT must can't be the input to the first leg
    // or the output of the second leg.
    if (isSWT (leg1.amount ().in) || isSWT (leg2.amount ().out))
    {
        return tefINTERNAL;
    }
