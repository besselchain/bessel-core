...
#include <transaction/book/OfferStream.h>

namespace bessel {
namespace core {

OfferStream::OfferStream (LedgerView& view, LedgerView& view_cancel, BookRef book, Clock::time_point when, beast::Journal journal)
    : m_journal (journal)
    , m_view (view)
...
OfferStream::erase (LedgerView& view)
{
    ...
    if (p == nullptr)
    {
        if (m_journal.error) 
        {
            m_journal.error << "Missing directory " << m_tip.dir () << " for offer " << m_tip.index ();
        }
    ...
    if (it == v.end ())
    {
        if (m_journal.error) 
        {
            m_journal.error << "Missing offer " << m_tip.index () << " for directory " << m_tip.dir ();
        }
    ...
    if (m_journal.trace)
    {
        m_journal.trace << "Missing offer " << m_tip.index () << " removed from directory " << m_tip.dir ();
    }
}
...
OfferStream::step ()
{
    ...
    for(;;)
    {
        ...
        // Remove if expired
        if (entry->isFieldPresent (sfExpiration) && entry->getFieldU32 (sfExpiration) <= m_when)
        {
            view_cancel ().offerDelete (entry->getIndex ());

            if (m_journal.trace)
            {   
                m_journal.trace << "Removing expired offer " << entry->getIndex ();
            }
        ...
        if (amount.empty ())
        {
            ...
            {
                m_journal.warning << "Removing bad offer " << entry->getIndex ();
            }
        ...
        Amount const owner_funds (view ().accountFunds (m_offer.owner (), amount.out, fhZERO_IF_FROZEN));
        ...
        if (owner_funds <= zero)
        {
            ...
            auto const original_funds = view_cancel ().accountFunds (m_offer.owner (), amount.out, fhZERO_IF_FROZEN);
            ...
            if (original_funds == owner_funds)
            {
                ...
                if (m_journal.trace)
                {
                    m_journal.trace << "Removing unfunded offer " << entry->getIndex ();
                }
            }
            else
            {
                if (m_journal.trace) 
                {
                    m_journal.trace << "Removing became unfunded offer " << entry->getIndex ();
                }
            }
