...
#ifndef BESSEL_APP_BOOK_AMOUNT_H_INCLUDED
#define BESSEL_APP_BOOK_AMOUNT_H_INCLUDED
...
private:
    ...
    AmountType (std::uint64_t mantissa, int exponent, bool negative, bool integral)
        : m_mantissa (mantissa)
        , m_exponent (exponent)
        , m_negative (negative)
        , m_integral (integral)
...
public:
    ...
    */
    ...
    AmountType (Integer value, typename std::enable_if<std::is_signed <Integer>::value>::type* = 0) noexcept
        : m_mantissa (value)
        , m_exponent (0)
        , m_negative (value < 0)
        , m_integral (true)
    {
        static_assert (std::is_integral<Integer>::value, "Cannot construct from non-integral type.");
    }
