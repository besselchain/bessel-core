...
BesselCalc::Output BesselCalc::besselCalculate (
    ...
    Input const* const pInputs)
{
    ...
    auto result = rc.besselCalculate ();
    Output output;
...
bool BesselCalc::addPathState(STPath const& path, TER& resultCode)
{
    ...
    if (pathState->status() == tesSUCCESS)
        pathState->checkNoBessel (uDstAccountID_, uSrcAccountID_);
...
TER BesselCalc::besselCalculate ()
{
    ...
    {
        WriteLog (lsDEBUG, BesselCalc)
            << "besselCalc: Invalid transaction:"
            << "No paths and direct bessel not allowed.";

        return temBESSEL_EMPTY;
    }
    ...
    WriteLog (lsTRACE, BesselCalc)
        << "besselCalc: Paths in set: " << spsPaths_.size ();
    ...
    while (resultCode == temUNCERTAIN)
    {
        ...
        ++iPass;
        if (ShouldLog (lsDEBUG, BesselCalc))
        {
            WriteLog (lsDEBUG, BesselCalc)
                << "besselCalc: Summary:"
                << " Pass: " << iPass
        ...
        if (iBest >= 0)
        {
                     ...
                     iDry != pathStateList_.size ())
            {
                ...
                if (iPass >= PAYMENT_MAX_LOOPS)
                {
                    ...
                    WriteLog (lsERROR, BesselCalc)
                       << "besselCalc: pass limit";
