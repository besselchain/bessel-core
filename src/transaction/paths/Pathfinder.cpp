...
#include <transaction/paths/Pathfinder.h>
...
The pathfinding request is identified by category, SWT to SWT, SWT to
non-SWT, non-SWT to SWT, same currency non-SWT to non-SWT, cross-currency
non-SWT to non-SWT.  For each category, there is a table of paths that the
...
bool compareAccountCandidate (std::uint32_t seq
...
Pathfinder::Pathfinder (
    ...
    ,   mEffectiveDst (isSWT(saDstAmount.getIssuer ()) ? uDstAccount : saDstAmount.getIssuer ())
    ...
    ,   mLedger (cache->getLedger ()),
        mRLCache (cache)
{
    assert (isSWT(uSrcCurrency) == isSWT(uSrcIssuer));
}
...
Pathfinder::Pathfinder (
    ...
    ,   mSrcAmount (
        {
            uSrcCurrency,
            isSWT (uSrcCurrency) ? xrpAccount () : uSrcAccount
        }, 1u, 0, true)
    ,   mLedger (cache->getLedger ())
    ,   mRLCache (cache)
{
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsSWT = isSWT (mSrcCurrency);

    bool useIssuerAccount = mSrcIssuer && !currencyIsSWT && !isSWT (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsSWT ? Account() : account;
    ...
    bool bSrcXrp = isSWT (mSrcCurrency);
    bool bDstXrp = isSWT (mDstAmount.getCurrency());
    ...
    {
        WriteLog (lsDEBUG, Pathfinder) << "Non-existent gateway";
    ...
    if (!mLedger->getSLEi (getAccountRootIndex (mDstAccount)))
    {
        ...
        if (!bDstXrp)
        {
            WriteLog (lsDEBUG, Pathfinder) << "New account not being funded in SWT ";
        ...
        {
            WriteLog (lsDEBUG, Pathfinder) << "New account not getting enough funding: " << mDstAmount << " < " << reserve;
    ...
    {
        // SWT -> SWT
        WriteLog (lsDEBUG, Pathfinder) << "SWT to SWT payment";

        paymentType = pt_SWT_to_SWT;
    }
    else if (bSrcXrp)
    {
        // SWT -> non-SWT
        WriteLog (lsDEBUG, Pathfinder) << "SWT to non-SWT payment";

        paymentType = pt_SWT_to_nonSWT;
    }
    else if (bDstXrp)
    {
        // non-SWT -> SWT
        WriteLog (lsDEBUG, Pathfinder) << "non-SWT to SWT payment";

        paymentType = pt_nonSWT_to_SWT;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency ())
    {
        // non-SWT -> non-SWT - Same currency
        WriteLog (lsDEBUG, Pathfinder) << "non-SWT to non-SWT - same currency";

        paymentType = pt_nonSWT_to_same;
    }
    else
    {
        // non-SWT to non-SWT - Different currency
        WriteLog (lsDEBUG, Pathfinder) << "non-SWT to non-SWT - cross currency";

        paymentType = pt_nonSWT_to_nonSWT;
    }
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    {
        WriteLog (lsINFO, Pathfinder) << "checkpath: exception (" << e.what() << ") " << path.getJson (0);
...
void Pathfinder::computePathRanks (int maxPaths)
{
    ...
    try
    {
        ...
        if (rc.result () == tesSUCCESS)
        {
            WriteLog (lsDEBUG, Pathfinder) << "Default path contributes: " << rc.actualAmountIn;

            mRemainingAmount -= rc.actualAmountOut;
        }
        else
        {
            WriteLog (lsDEBUG, Pathfinder) << "Default path fails: " << transToken (rc.result ());
        }
    }
    catch (...)
    {
...
static bool isDefaultPath (STPath const& path)
{
    ...
    // https://forum.bessel.com/viewtopic.php?f=2&t=8206&start=10#p57713
    //
    // JoelKatz writes:
...
void Pathfinder::rankPaths (int maxPaths
    ...
    Account const& srcIssuer)
{
    ...
    const bool issuerIsSender = isSWT (mSrcCurrency) || (srcIssuer == mSrcAccount);
        ...
        extraPathsIterator != extraPathRanks.end())
    {
        ...
        auto& path = usePath ? mCompletePaths[pathRank.index] : extraPaths[pathRank.index];
...
int Pathfinder::getPathsOut (Currency const& currency
                            ...
                            , Account const& dstAccount)
{
    ...
    if (!bFrozen)
    {
        ...
        for (auto const& item : mRLCache->getBesselLines (account))
        {
            BesselState* rspEntry = (BesselState*) item.get ();
            ...
            else if (rspEntry->getNoBesselPeer ())
            {
...
STPathSet& Pathfinder::addPathsForType (PathType const& pathType)
{
    ...
    case nt_SWT_BOOK:
        addLinks (parentPaths, pathsOut, afADD_BOOKS | afOB_SWT);
        break;
...
bool Pathfinder::isNoBessel (Account const& fromAccount
                            , Account const& toAccount
                            , Currency const& currency)
{
    auto sleBessel = mLedger->getSLEi (getBesselStateIndex (toAccount, fromAccount, currency));

    auto const flag ((toAccount > fromAccount) ? lsfHighNoBessel : lsfLowNoBessel);

    return sleBessel && (sleBessel->getFieldU32 (sfFlags) & flag);
}

// Does this path end on an account-to-account link whose last account has
// set "no bessel" on the link?
bool Pathfinder::isNoBesselOut (STPath const& currentPath)
{
    ...
    // no bessel on the output. A path with no bessel on its output can't be
    // followed by a link with no bessel on its input.
    auto const& fromAccount = (currentPath.size () == 1)
        ? mSrcAccount
        : (currentPath.end () - 2)->getAccountID ();

    auto const& toAccount = endElement.getAccountID ();
    return isNoBessel (fromAccount, toAccount, endElement.getCurrency ());
}
    ...
    int addFlags)
{
    ...
    bool const bOnSWT = uEndCurrency.isZero ();
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnSWT)
        {
            ...
            { // non-default path to SWT destination
                WriteLog (lsTRACE, Pathfinder)
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                bool const bIsNoBesselOut (
                    isNoBesselOut (currentPath));
                bool const bDestOnly (
                    addFlags & afAC_LAST);

                auto& besselLines (mRLCache->getBesselLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (besselLines.size ());

                for (auto const& item : besselLines)
                {
                    auto* rs = dynamic_cast<BesselState const *> (item.get ());
                    if (!rs)
                    {
                        WriteLog (lsERROR, Pathfinder)
                                << "Couldn't decipher BesselState";
                        continue;
                    }
                        ...
                        !currentPath.hasSeen (acct, uEndCurrency, acct))
                    {
                        ...
                        else if (bIsNoBesselOut && rs->getNoBessel ())
                        {
    ...
    {
        // add order books
        if (addFlags & afOB_SWT)
        {
            // to SWT only
            if (!bOnSWT && getApp ().getOrderBookDB ().isBookToSWT (
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    { // to SWT
                        ...
                        {
                            // destination is SWT, add account and path is
                            // complete
                            WriteLog (lsTRACE, Pathfinder)
...
Pathfinder::PathType makePath (char const *string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x': // swt book
                ret.push_back (Pathfinder::nt_SWT_BOOK);
                break;
...
void Pathfinder::initPathTable ()
{
    ...
    fillPaths(
        pt_nonSWT_to_SWT, {
            {1, "sxd"},       // gateway buys SWT
            {2, "saxd"},      // source -> gateway -> book(SWT) -> dest
            {6, "saaxd"},
            {7, "sbxd"},
            {8, "sabxd"},
            {9, "sabaxd"}
    ...
    // non-SWT to non-SWT (same currency)
    fillPaths(
        pt_nonSWT_to_same,  {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},   // source -> gateway -> book to SWT -> book ->
                            // destination
            {6, "saxfad"},
