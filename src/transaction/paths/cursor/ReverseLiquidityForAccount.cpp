...
//
// Currency cannot be SWT because we are rippling.
//
...
;
    ...
    //WriteLog (lsTRACE, BesselCalc) << pathState_.getJson ();
    ...
    // page section: https://bessel.com/wiki/Transit_Fees#Implementation
        ...
        else
        {
                ...
                // Did not complete redeeming previous IOUs.
            {
                ...
                besselLiquidity (
                    besselCalc_,
                    QUALITY_ONE,
                    besselTransferRate(ledger(), node().account_, saCurIssueAct.getCurrency()),
                    saPrvRedeemReq,
                    node().saRevIssue,
                    previousNode().saRevRedeem,
                    saCurIssueAct,
    ...
    else if (previousNodeIsAccount && !nextNodeIsAccount)
    {
            ...
            && node().saRevDeliver)                 // Need some issued.
        {
            ...
            besselLiquidity (
                besselCalc_,
                QUALITY_ONE,
                besselTransferRate(ledger(), node().account_, saCurDeliverAct.getCurrency()),
                saPrvRedeemReq,
                node().saRevDeliver,
                previousNode().saRevRedeem,
                saCurDeliverAct,
    ...
    else if (!previousNodeIsAccount && nextNodeIsAccount)
    {
        if (isFinalNode)
        {
            ...
            if (saCurWantedReq <= zero)
            {
                ...
                WriteLog (lsFATAL, BesselCalc) << "CurWantReq was not positive";
                return tefEXCEPTION;
            }
        ...
        else
        {
            ...
            // deliver -> redeem
            // TODO(tom): now we have more checking in nodeBessel, these checks
            // might be redundant.
