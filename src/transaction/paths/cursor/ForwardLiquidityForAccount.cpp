...
// #include <bessel/legacy/0.27/Emulate027.h>
...
// - If next node is an offer and output is non-SWT then we are the issuer and
//   do not need to push funds.
// - If next node is an offer and output is SWT then we need to deliver funds to
//   limbo.
TER PathCursor::forwardLiquidityForAccount () const
{
    ...
    if (previousNode().isAccount() && nextNode().isAccount())
    {
        ...
        if (!nodeIndex_)
        {
            ...
            // For the first node, calculate amount to bessel based on what is
            // available.
        ...
        else if (nodeIndex_ == lastNodeIndex)
        {
            ...
            {
                // Actually receive.
                resultCode = ledger().besselCredit (
                    previousAccountID,
                    node().account_,
        ...
        else
        {
                ...
                // Current wants to issue.
            {
                ...
                besselLiquidity (
                    besselCalc_,
                    QUALITY_ONE,
                    besselTransferRate(ledger(), node().account_, node().saFwdIssue.getCurrency()),
                    previousNode().saFwdRedeem,
                    node().saRevIssue,
                    saPrvRedeemAct,
                    node().saFwdIssue,
    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        if (nodeIndex_)
        {
            // Non-SWT, current node is the issuer.
            WriteLog (lsTRACE, BesselCalc)
                << "forwardLiquidityForAccount: account --> "
                << "ACCOUNT --> offer";
                ...
                // Previous wants to redeem.
            {
                ...
                besselLiquidity (
                    besselCalc_,
                    QUALITY_ONE,
                    besselTransferRate(ledger(), node().account_, node().saFwdDeliver.getCurrency()),
                    previousNode().saFwdRedeem,
                    node().saRevDeliver,
                    saPrvRedeemAct,
                    node().saFwdDeliver,
        ...
        else
        {
            ...
            if (pathState_.inReq() >= zero)
            {
                ...
                // Limit SWT by available. No limit for non-SWT as issuer.
                if (isSWT (node().issue_))
                    node().saFwdDeliver = std::min (
                        node().saFwdDeliver,
                        ledger().accountHolds (
                            node().account_,
                            xrpCurrency(),
                            xrpAccount(),
                            fhIGNORE_FREEZE)); // SWT can't be frozen
            ...
            else if (!isSWT (node().issue_))
            {
                // Non-SWT, current node is the issuer.
                // We could be delivering to multiple accounts, so we don't know
                // which bessel balance will be adjusted.  Assume just issuing.
    ...
    else
    {
        ...
        // if (bessel::legacy::emulate027 (besselCalc_.mActiveLedger.getLedger ()))
