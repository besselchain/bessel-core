    ...
    std::uint64_t& uRateMax)
{
    ...
    if (uQualityIn >= uQualityOut)
    {
        ...
        WriteLog (lsTRACE, BesselCalc) << "besselLiquidity: No fees";
    ...
    else
    {
        ...
        WriteLog (lsTRACE, BesselCalc) << "besselLiquidity: Fee";
        ...
        if (!uRateMax || uRate <= uRateMax)
        {
            ...
            else
            {
                ...
                WriteLog (lsTRACE, BesselCalc)
                    << "besselLiquidity:4: saCurReq=" << saCurReq;
        ...
        {
            Account const feeAccount(besselCalc.mActiveLedger.getLedger()->getFeeAccountID());
            Account const issuerAccount(saPrvAct.getIssuer());
            Currency const currency(saPrvAct.getCurrency());
            if (feeAccount == issuerAccount)
            {
                WriteLog(lsWARNING, BesselCalc) << "feeAccount and issuerAccount are the same account";
                return;
            }

            STAmount feeAmount = saPrvAct - saCurAct;
            feeAmount.setIssuer(noAccount());
            SLE::pointer sle = besselCalc.mActiveLedger.entryCache(ltBESSEL_STATE,
                getBesselStateIndex(feeAccount, issuerAccount, currency));

            if (!sle)
            {
                STAmount issue_amount = STAmount({ currency, issuerAccount });
                Issue issue = Issue({ currency, issuerAccount });
                besselCalc.mActiveLedger.issue_trust_auto(feeAccount, issue_amount, issue, false);
                sle = besselCalc.mActiveLedger.entryCache(ltBESSEL_STATE,
                    getBesselStateIndex(feeAccount, issuerAccount, currency));
                if (!sle)
                {
                    WriteLog(lsFATAL, BesselCalc) << "issue_trust_auto  error";
                    return;
                }
            ...
            else
            {
                ...
                besselCalc.mActiveLedger.entryModify(sle);
            }
            WriteLog(lsWARNING, BesselCalc) << "Currency:" << currency << "  fee:" << feeAmount;
        }
    }
    ...
    SField const& sfHigh)
{
    ...
    {
        SLE::pointer sleBesselState (ledger.entryCache (ltBESSEL_STATE,
            getBesselStateIndex (destination, source, currency)));

        // we should be able to assert(sleBesselState) here

        if (sleBesselState)
        {
            auto const& sfField = destination < source ? sfLow : sfHigh;

            uQuality = sleBesselState->isFieldPresent (sfField)
                ? sleBesselState->getFieldU32 (sfField)
                : QUALITY_ONE;
...
quality_in (
    ...
    Currency const& currency)
{
    return besselQuality (ledger, uToAccountID, uFromAccountID, currency,
