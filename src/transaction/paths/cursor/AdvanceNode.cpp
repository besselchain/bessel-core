...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse) const
{
    ...
    PathCursor withMultiQuality {besselCalc_, pathState_, multi, nodeIndex_};
...
TER PathCursor::advanceNode (bool const bReverse) const
{
    ...
    do
    {
        //  NOTE Why not use a for() loop?
        //  TODO The limit on loop iterations puts an
        ...
        {
            WriteLog (lsWARNING, BesselCalc) << "Loop count exceeded";
            return tefEXCEPTION;
        }
        ...
        if (!node().bEntryAdvance)
        {
            ...
            {
                WriteLog (lsTRACE, BesselCalc) << "advanceNode: as is";
            }
        }
        ...
        else
        {
            ...
            else
            {
                     ...
                     getParentCloseTimeNC ()))
                {
                    ...
                    besselCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= zero || node().saTakerGets <= zero)
                {
                    ...
                    else if (besselCalc_.permanentlyUnfundedOffers_.find (index)
                             != besselCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = besselCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != besselCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        besselCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {
