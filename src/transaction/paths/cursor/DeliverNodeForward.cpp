    ...
    STAmount& saInFees) const       // <-- Fees charged, this invocation.
{
    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            // There's no fee if we're transferring SWT, if the sender is the
            // issuer, or if the receiver is the issuer.
            bool noFee = isSWT (previousNode().issue_)
            ...
            if (!isSWT(nextNode().account_))
            {
                // ? --> OFFER --> account
                // Input fees: vary based upon the consumed offer's owner.
                // Output fees: none as SWT or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send SWT or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isSWT(node().issue_) ?
            ...
            // is redundant.
            if (isSWT (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isSWT(previousNode().issue_.currency) ?
            ...
            {
                CondLog (saOutPassAct >= saOutFunded, lsWARNING, BesselCalc)
