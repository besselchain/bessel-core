...
BesselState::pointer BesselState::makeItem (
    Account const& accountID, STLedgerEntry::ref ledgerEntry)
{
    //  Does this ever happen in practice?
    if (!ledgerEntry || ledgerEntry->getType () != ltBESSEL_STATE)
        return pointer ();

    return pointer (new BesselState (ledgerEntry, accountID));
}

BesselState::BesselState (
...
Json::Value BesselState::getJson (int)
{
...
getBesselStateItems (
    ...
    Ledger::ref ledger)
{
    std::vector <BesselState::pointer> items;

    ledger->visitAccountItems (accountID,
        [&items,&accountID](SLE::ref sleCur)
        {
             auto ret = BesselState::makeItem (accountID, sleCur);
