...
#include <transaction/paths/Credit.h>
#include <transaction/paths/PathState.h>
...
void PathState::reset(STAmount const& in, STAmount const& out)
{
    ...
    CondLog (outAct() >= outReq(), lsWARNING, BesselCalc)
        << "besselCalc: ALREADY DONE:"
        << " saOutAct=" << outAct()
        << " saOutReq=" << outReq();
...
// - A bessel nodes output issuer must be the node's account or the next node's
//   account.
// - Offers can only go directly to another offer if the currency and issuer are
//   an exact match.
// - Real issuers must be specified for non-SWT.
    ...
    Account const& issuer)    // --> Delivering this issuer.
{
    TER resultCode = tesSUCCESS;
     ...
     WriteLog (lsTRACE, BesselCalc) << "pushImpliedNodes>" <<
         " " << account <<
         " " << currency <<
         " " << issuer;
    ...
    if (nodes_.back ().issue_.currency != currency)
    {
        ...
        auto type = isSWT(currency) ? STPathElement::typeCurrency
    ...
    // For bessel, non-SWT, ensure the issuer is on at least one side of the
    // transaction.
    ...
    {
        // Need to bessel through issuer's account.
    ...
    Account const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a bessel account. false, iff node is an offer node.
    ...
    if (iType & ~STPathElement::typeAll)
    {
        ...
        WriteLog (lsDEBUG, BesselCalc) << "pushNode: bad bits.";
        resultCode = temBAD_PATH;
    }
    else if (hasIssuer && isSWT (node.issue_))
    {
        WriteLog (lsDEBUG, BesselCalc) << "pushNode: issuer specified for SWT.";

        resultCode = temBAD_PATH;
    }
    else if (hasIssuer && !issuer)
    {
        WriteLog (lsDEBUG, BesselCalc) << "pushNode: specified bad issuer.";
    ...
    else if (hasAccount)
    {
        ...
        node.issue_.account = hasIssuer ? issuer :
                (isSWT (node.issue_) ? xrpAccount() : account);
        ...
        else
        {
            ...
            resultCode = pushImpliedNodes (
                ...
                isSWT(node.issue_.currency) ? xrpAccount() : account);
        ...
        if (resultCode == tesSUCCESS && !nodes_.empty ())
        {
            ...
            {
                auto sleBesselState = lesEntries.entryCache (
                    ltBESSEL_STATE,
                    getBesselStateIndex (
                        backNode.account_,
                        node.account_,
                ...
                // A "BesselState" means a balance betweeen two accounts for a
                // specific currency.
                if (!sleBesselState)
                {
                    WriteLog (lsTRACE, BesselCalc)
                    ...
                    WriteLog (lsTRACE, BesselCalc) << getJson ();
                ...
                else
                {
                    ...
                    if (!sleBck)
                    {
                        ...
                        resultCode   = terNO_ACCOUNT;
                             ...
                             !(sleBesselState->getFieldU32 (sfFlags) &
                                  (bHigh ? lsfHighAuth : lsfLowAuth)) &&
                             sleBesselState->getFieldAmount(sfBalance) == zero)
                    {
                        WriteLog (lsWARNING, BesselCalc)
                                << "pushNode: delay: can't receive IOUs from "
                                << "issuer without auth.";
    ...
    else
    {
        ...
        else if (isSWT (node.issue_.currency))
            node.issue_.account = xrpAccount();
        else if (isSWT (backNode.issue_.account))
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            WriteLog (lsTRACE, BesselCalc) << "pushNode: imply for offer.";
    ...
    WriteLog (lsTRACE, BesselCalc) << "pushNode< : " << transToken (resultCode);
    return resultCode;
}
    ...
    Account const& uSenderID)
{
    ...
    Account const& uSenderIssuerID
        = isSWT(uMaxCurrencyID) ? xrpAccount() : uSenderID;
    // Sender is always issuer for non-SWT.
    ...
    // SWT with issuer is malformed.
    if ((isSWT (uMaxCurrencyID) && !isSWT (uMaxIssuerID))
        || (isSWT (currencyOutID) && !isSWT (issuerOutID)))
    {
        WriteLog (lsDEBUG, BesselCalc)
            << "expandPath> issuer with SWT";
        terStatus   = temBAD_PATH;
    }

    // Push sending node.
    // For non-SWT, issuer is always sending account.
    // - Trying to expand, not-compact.
    // - Every issuer will be traversed through.
    if (terStatus == tesSUCCESS)
    {
        terStatus   = pushNode (
            !isSWT(uMaxCurrencyID)
    ...
    if (tesSUCCESS == terStatus && uMaxIssuerID != uSenderIssuerID)
    {
        ...
        // - If it was SWT, then issuers would have matched.
        ...
        // account.
        if (!uNxtCurrencyID
            // Next is SWT, offer next. Must go through issuer.
    ...
    for (auto & speElement: spSourcePath)
    {
        if (terStatus == tesSUCCESS)
        {
            WriteLog (lsTRACE, BesselCalc) << "expandPath: element in path";
            terStatus = pushNode (
    ...
    if (terStatus == tesSUCCESS
        && !isSWT(currencyOutID)               // Next is not SWT
    ...
    for (std::size_t i = 0; i < (nodes_.size() - 1); ++i)
    {
        ...
        if (nodes_[i].uFlags & STPathElement::typeAccount)
        {
            ...
            if (inAccount != outAccount)
            {
                ...
                sle = lesEntries.entryCache (ltBESSEL_STATE,
                    getBesselStateIndex (inAccount,
...
/** Check if a sequence of three accounts violates the no bessel constrains
    [first] -> [second] -> [third]
    Disallowed if 'second' set no bessel on [first]->[second] and
    [second]->[third]
*/
TER PathState::checkNoBessel (
...
{
    // fetch the bessel lines into and out of this node
    SLE::pointer sleIn = lesEntries.entryCache (ltBESSEL_STATE,
        getBesselStateIndex (firstAccount, secondAccount, currency));
    SLE::pointer sleOut = lesEntries.entryCache (ltBESSEL_STATE,
        getBesselStateIndex (secondAccount, thirdAccount, currency));
    ...
    if (!sleIn || !sleOut)
    {
        ...
        sleIn->getFieldU32 (sfFlags) &
            ((secondAccount > firstAccount) ? lsfHighNoBessel : lsfLowNoBessel) &&
        sleOut->getFieldU32 (sfFlags) &
            ((secondAccount > thirdAccount) ? lsfHighNoBessel : lsfLowNoBessel))
    {
        WriteLog (lsINFO, BesselCalc)
            << "Path violates noBessel constraint between "
            << firstAccount << ", "
            << secondAccount << " and "
            << thirdAccount;

        terStatus = terNO_BESSEL;
    }
    return terStatus;
}

// Check a fully-expanded path to make sure it doesn't violate no-Bessel
// settings.
TER PathState::checkNoBessel (
    Account const& uDstAccountID,
    Account const& uSrcAccountID)
{
    // There must be at least one node for there to be two consecutive bessel
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    // These are the nodes whose no bessel constraints could be violated
