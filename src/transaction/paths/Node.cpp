...
#include <transaction/paths/Node.h>
...
Json::Value Node::getJson () const
{
    ...
    bool const hasCurrency = !isSWT (issue_.currency);
    bool const hasAccount = !isSWT (account_);
    bool const hasIssuer = !isSWT (issue_.account);
    ...
    {
        jvFlags.append ((uFlags & STPathElement::typeCurrency) && (hasCurrency ? "currency" : "-currency"));
    }

    if (uFlags & STPathElement::typeIssuer || hasIssuer)
    {
        jvFlags.append ((uFlags & STPathElement::typeIssuer) && (hasIssuer ? "issuer" : "-issuer"));
    }
    ...
    if (!isSWT (issue_.currency))
        jvNode[jss::currency] = to_string (issue_.currency);

    if (!isSWT (issue_.account))
