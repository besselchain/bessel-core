...
public:
        ...
        , mReceiveBuffer (256)
    {
        ...
        mSocket.async_receive_from (boost::asio::buffer (mReceiveBuffer, 256),
                                    mReceiveEndpoint, 
                                    std::bind (&SNTPClientImp::receivePacket, 
                                                this,
                                                std::placeholders::_1,
                                                std::placeholders::_2));

        mTimer.expires_from_now (boost::posix_time::seconds (NTP_QUERY_FREQUENCY));
        mTimer.async_wait (std::bind (&SNTPClientImp::timerEntry, this, std::placeholders::_1));
    }
    ...
    bool doQuery ()
    {
        ...
        mResolver.async_resolve (query,
                                 std::bind (&SNTPClientImp::resolveComplete,
                                            this,
                                            std::placeholders::_1, 
                                            std::placeholders::_2));
    #ifdef SNTP_DEBUG
    ...
    void resolveComplete (const boost::system::error_code& error, boost::asio::ip::udp::resolver::iterator it)
    {
        if (!error)
        {
            ...
            if (sel != boost::asio::ip::udp::resolver::iterator ())
            {
                ...
                mSocket.async_send_to (boost::asio::buffer (SNTPQueryData, 48), 
                                       *sel,
                                       std::bind (&SNTPClientImp::sendComplete, 
                                                   this,
                                                   std::placeholders::_1, 
                                                   std::placeholders::_2));
            }
    ...
    void receivePacket (const boost::system::error_code& error, std::size_t bytes_xferd)
    {
        ...
        mSocket.async_receive_from (boost::asio::buffer (mReceiveBuffer, 256),
                                    mReceiveEndpoint,
                                    std::bind (&SNTPClientImp::receivePacket, this, std::placeholders::_1, std::placeholders::_2));
    }
