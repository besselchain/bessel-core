...
#include <services/websocket/AutoSocket.h>
...
{
public:
    HTTPClientSSLContext () : m_context(boost::asio::ssl::context::sslv23)
    {
...
public:
    ...
    void makeGet (std::string const&      strPath, 
            ...
            std::string const& strData)> complete)
    {
        ...
        request (
            bSSL,
            deqSites,
            std::bind (&HTTPClientImp::makeGet, 
                       shared_from_this (), 
                       strPath,
                       std::placeholders::_1, 
                       std::placeholders::_2),
    ...
    void httpsNext ()
    {
        ...
        std::shared_ptr<boost::asio::ip::tcp::resolver::query> query (new boost::asio::ip::tcp::resolver::query (mDeqSites[0],
    ...
    void handleHeader (const boost::system::error_code& ecResult, std::size_t bytes_transferred)
    {
        ...
        mStatus = boost::lexical_cast<int> (std::string (smMatch[1]));
        ...
        {
            int size = boost::lexical_cast <int> (std::string(smMatch[1]));
...
void HTTPClient::get (
    ...
    std::function<bool (const boost::system::error_code& ecResult, 
                        ...
                        std::string const& strData)> complete)
{
    std::shared_ptr <HTTPClientImp> client (new HTTPClientImp (io_service, port, responseMax));
