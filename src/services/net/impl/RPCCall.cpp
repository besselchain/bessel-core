...
#include <services/net/RPCCall.h>
...
private:
    ...
    static bool jvParseLedger (Json::Value& jvRequest, std::string const& strLedger)
    {
        ...
        {
            jvRequest[jss::ledger_index] = boost::lexical_cast <std::uint32_t> (strLedger);
        }
    ...
    static Json::Value jvParseCurrencyIssuer (std::string const& strCurrencyIssuer)
    {
        ...
        if (boost::regex_match (strCurrencyIssuer, smMatch, reCurIss))
        {
            ...
            if (strIssuer.length ())
            {
                // Could confirm issuer is a valid Bessel address.
...
private:
    ...
    //show_blob tx_blob
    Json::Value parseSignerInfo(Json::Value const& jvParams)
    {
    ...
    //show_blob tx_blob
    Json::Value parseShowBlob (Json::Value const& jvParams)
    {
        Json::Value jvRequest (Json::objectValue);
        unsigned int iParams = jvParams.size ();
        if (iParams == 1)  
    ...
    Json::Value parseTreadInfo (Json::Value const& jvParams)
    {
        Json::Value jvRequest (Json::objectValue);
        unsigned int iParams = jvParams.size ();
        if (iParams == 1 )  
        {
           auto parstr = jvParams[0u].asString();
           if (parstr == "reset" ) 
           {
           ...
           {
              jvRequest[jss::job_name] = parstr;
           }
        }
        else if (iParams == 2) 
        {
           auto parJob = jvParams[0u].asString();
           auto parReset = jvParams[1u].asString();
           if (parReset != jss::reset ) 
           {
               return rpcError (rpcINVALID_PARAMS);
           }

           jvRequest[jss::reset] = true;
           jvRequest[jss::job_name] = parJob;
        }
        else if (iParams > 2 ) 
        {
          return rpcError (rpcINVALID_PARAMS);
    ...
    // rpc_info [print [<command>]][<reset>]
    Json::Value parseRpcInfo (Json::Value const& jvParams)
    {
        Json::Value jvRequest (Json::objectValue);
        unsigned int iParams = jvParams.size ();
        if (1 == iParams )
        {
           ...
           if ( parstr == "reset")
           {
              jvRequest[jss::reset] = parstr; 
           }
           else if (parstr == "print") 
           {
              jvRequest[jss::print] = parstr;
           }
           else  
           { 
              return rpcError (rpcINVALID_PARAMS);
        ...
        {
           auto parFirst = jvParams[0u].asString();
           auto parSecond = jvParams[1u].asString();
           if (parFirst == "print" && parSecond == "reset" ) 
           {
              jvRequest[jss::print] = parFirst;
              jvRequest[jss::reset] = parSecond;
           }
           else if ( parFirst == "print" && parSecond.length() != 0  ) 
           {
               jvRequest[jss::print]       = parFirst;
               jvRequest[jss::rpc_command] = parSecond;
           }
           else 
           { 
              return rpcError (rpcINVALID_PARAMS);
        ...
        else if (3 == iParams)
        {
           auto parFirst = jvParams[0u].asString();
           auto parSecond = jvParams[1u].asString();
           auto parThird = jvParams[2u].asString();
           if (parFirst == "print" && 
               parSecond.length() !=0 && 
               parThird == "reset")
           {
    ...
    {
        return parseAccount(jvParams);
    }

    //extract function implemetation
    Json::Value parseAccount(const Json::Value& jvParams)
    {
        Json::Value    jvRequest(Json::objectValue);
        BesselAddress raAccount;
        ...
        {
            std::string jvString = jvParams[iParams - 1].asString();
            if(jvString == jss::binary)
            {
                jvRequest[jss::binary] = true;
                --iParams;
            }
            else if(jvString == jss::count)
            {
                jvRequest[jss::count] = true;
                --iParams;
            }
            else if(jvString == jss::descending) //From parseAccountTransactions
            {
                jvRequest[jss::descending] = true;
                --iParams;
            }
            else if(jvString == jss::forward) //From parseTxAccount
            {
    ...
    Json::Value parseFeature (Json::Value const& jvParams)
    {
        ...
        if (jvParams.size () > 1)
            jvRequest[jss::vote] = boost::lexical_cast <bool> (jvParams[1u].asString ());
   ...
   // show_trust <account> [<currency> [<ledger>]]
    Json::Value parseShowTrust (Json::Value const& jvParams)
    {
       Json::Value jvRequest (Json::objectValue);
       unsigned int iCursor = jvParams.size ();
       if (iCursor <1 || iCursor >3 ) 
       {
          rpcError (rpcINVALID_PARAMS);
       }

       std::string strIdent = jvParams[0u].asString ();
       BesselAddress raAddress;
       ...
       if (iCursor >= 2 ) 
       {
         jvRequest[jss::currency] = jvParams[1u].asString ();         
    ...
    // account_trust <account> <account>|"" [<ledger>]
    Json::Value parseAccountTrust (Json::Value const& jvParams)
    {
       // return parseAccountRaw (jvParams, true);
		Json::Value  jvRequest(Json::objectValue);
		unsigned int iCursor = jvParams.size();
		...
		if (iCursor >= 2)
        {
			jvRequest[jss::relation_type] = jvParams[1u].asString();
    ...
    Json::Value parseAccountRaw (Json::Value const& jvParams, bool bPeer)
    {
        ...
        if (bPeer && iCursor >= 2)
        {
            ...
            strPeer = jvParams[1u].asString ();  //Get second parameter values
        }
    ...
    // bessel_path_find <json> [<ledger>]
    Json::Value parseBesselPathFind (Json::Value const& jvParams)
    {
        Json::Reader reader;
        Json::Value  jvRequest;
...
public:
    ...
    static std::string EncodeBase64 (std::string const& s)
    {
        ...
        //  TODO What the heck is BIO and BUF_MEM!?
    ...
    Json::Value parseCommand (std::string strMethod, Json::Value jvParams, bool allowAnyCommand)
    {
        ...
        Command const commands[] =
        {
            ...
            {   "account_relation",     &RPCParser::parseAccountTrust,          1,  5   },
            {   "account_relcp",        &RPCParser::parseAccountTrust,          1,  5   },
            {   "book_offers",          &RPCParser::parseBookOffers,            2,  7   },
            {   "can_delete",           &RPCParser::parseCanDelete,             0,  1   },
            {   "connect",              &RPCParser::parseConnect,               1,  2   },
            {   "consensus_info",       &RPCParser::parseAsIs,                  0,  0   },
            {   "data_info",            &RPCParser::parseAsIs,                  0,  0   },
    ...
    //      {   "profile",              &RPCParser::parseProfile,               1,  9   },
	        ...
	        {   "rpc_info",           	&RPCParser::parseRpcInfo,               0,  3   },
            {   "bessel_path_find",     &RPCParser::parseBesselPathFind,        1,  2   },
            {   "show_trust",           &RPCParser::parseShowTrust,             1,  3   },
            {   "sign",                 &RPCParser::parseSignSubmit,            2,  3   },
            {   "submit",               &RPCParser::parseSignSubmit,            1,  3   },
            {   "server_info",          &RPCParser::parseAsIs,                  0,  0   },
            {   "signer_info",          &RPCParser::parseSignerInfo,            1,  1   },     
            {   "show_blob",            &RPCParser::parseShowBlob,              1,  1   },
            {   "blacklist_info",       &RPCParser::parseAsIs,                  0,  0   },
            {   "server_state",         &RPCParser::parseAsIs,                  0,  0   },
            {   "stop",                 &RPCParser::parseAsIs,                  0,  0   },
    //      {   "transaction_entry",    &RPCParser::parseTransactionEntry,     -1,  -1  },
            {   "thread_info",          &RPCParser::parseTreadInfo,             0,  2   },
        ...
        for (auto const& command : commands)
        {
            if (strMethod == command.name)
            {
                ...
                {
                    WriteLog (lsDEBUG, RPCParser) << "Wrong number of parameters for " << command.name 
...
struct RPCCallImp
{
    //  NOTE Is this a to-do comment or a doc comment?
    ...
    try
    {
        ...
        if (jvOutput.isMember (jss::error))
        {
            ...
            nRet = jvOutput.isMember (jss::error_code)
                   ? boost::lexical_cast<int> (jvOutput[jss::error_code].asString ())
                   : 1;
        }
    ...
    std::function<void (Json::Value const& jvInput)> callbackFuncP)
{
    ...
    if (!getConfig ().QUIET)
    {
        std::cerr << (bSSL ? "Securely connecting to " : "Connecting to ") << strIp << ":" << iPort << std::endl;
    }
