...
#ifndef BESSEL_NET_INFOSUB_H_INCLUDED
#define BESSEL_NET_INFOSUB_H_INCLUDED
...
public:
    ...
    virtual void send (Json::Value const& jvObj, std::string const& sObj, bool broadcast);

    std::uint64_t getSeq ();

    void onSendEmpty ();

    void insertSubAccountInfo (BesselAddress addr, bool rt);

    void deleteSubAccountInfo (BesselAddress addr, bool rt);
...
private:
    Consumer                      m_consumer;
    Source&                       m_source;
    hash_set<BesselAddress>      mSubAccountInfo_t; // real time subscriptions
    hash_set<BesselAddress>      mSubAccountInfo_f; // normal subscriptions
    hash_set<BesselAddress>      mSubAccountTransaction;
