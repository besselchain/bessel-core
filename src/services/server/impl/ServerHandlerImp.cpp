...
#include <services/server/impl/JSONRPCUtil.h>
#include <services/server/impl/ServerHandlerImp.h>
#include <services/server/make_Server.h>
#include <services/rpc/Coroutine.h>
...
ServerHandlerImp::ServerHandlerImp (Stoppable& parent,
    ...
    , m_server (HTTP::make_Server(*this, io_service, deprecatedLogs().journal("Server")))
{
...
ServerHandlerImp::onHandoff (HTTP::Session& session,
                            ...
                            boost::asio::ip::tcp::endpoint remote_address) -> Handoff
{
    if (session.port().protocol.count("wss") > 0 && isWebsocketUpgrade (request))
    {
        // Pass to websockets
        Handoff handoff;
        // handoff.moved = true;
    ...
    if (session.port().protocol.count("peer") > 0)
        return getApp().overlay().onHandoff (std::move(bundle), std::move(request), remote_address);
...
ServerHandlerImp::onHandoff (HTTP::Session& session,
                             ...
                             boost::asio::ip::tcp::endpoint remote_address) -> Handoff
{
    if (session.port().protocol.count("ws") > 0 && isWebsocketUpgrade (request))
    {
        // Pass to websockets
        Handoff handoff;
        // handoff.moved = true;
...
ServerHandlerImp::onRequest (HTTP::Session& session)
{
    ...
    if (! authorized (session.port(), build_map(session.request().headers)))
    {
        HTTPReply (403, "Forbidden", makeOutput (session));
        session.close (true);
    ...
    if (setup_.yieldStrategy.useCoroutines == RPC::YieldStrategy::UseCoroutines::yes)
    {
        RPC::Coroutine::YieldFunction yieldFunction = [this, detach] (Yield const& y) { processSession (detach, y); };
...
void
ServerHandlerImp::processSession (std::shared_ptr<HTTP::Session> const& session, Yield const& yield)
{
    ...
    end.address(session->remoteAddress().address());
    end.port(0);
    processRequest (
        session->port(),
    ...
    Yield yield)
{
    ...
    //
    //  NOTE Except that "id" isn't included in the following errors.
    //
    ...
    if (jsonRPC.isObject() && 
        jsonRPC.isMember("params") &&
        jsonRPC["params"].isArray() && 
        jsonRPC["params"].size() > 0 &&
        jsonRPC["params"][Json::UInt(0)].isObject())
    {
        role = requestRole(required, port, jsonRPC["params"][Json::UInt(0)], remoteIPAddress);
    }
    else
    {
        role = requestRole(required, port, Json::objectValue, remoteIPAddress);
    }

    Resource::Consumer usage;

    if (role == Role::ADMIN)
        usage = m_resourceManager.newAdminEndpoint (convert_endpoint_to_string(remoteIPAddress));
    else
    ...
    //  TODO Shouldn't we handle this earlier?
    //
    if (role == Role::FORBID)
    {
    ...
    m_journal.debug << "Query: " << strMethod << params.toStyledString();

    // Provide the JSON-RPC method as the field "command" in the request.
    params[jss::command] = strMethod;

    //WriteLog (lsWARNING, RPCErr) <<"----RPC---CMD: "<<params;
    WriteLog (lsTRACE, RPCHandler)
        << "doRpcCommand:" << strMethod << ":" << params.toStyledString();

    auto const start (std::chrono::high_resolution_clock::now ());
    RPC::Context context {params, loadType, m_networkOPs, role, nullptr, yield};
    //RPC::RPCInfo::updateCmd(context.params,true);
	...
	//RPC::RPCInfo::updateError(context.params,result,true);
        // Always report "status".  On an error report the request as received.
        if (result.isMember (jss::error))
        {
            //WriteLog (lsWARNING, RPCErr) <<"----RPC---error\n "<<result;
    ...
    rpc_io_.notify (static_cast <beast::insight::Event::value_type> (context.metrics.fetches));
    rpc_size_.notify (static_cast <beast::insight::Event::value_type> (response.size ()));
...
//  TODO Rewrite to use beast::http::headers
bool
    ...
    std::map<std::string, std::string> const& h)
{
    ...
    std::string strUserPass = bessel::base64_decode (strUserPass64);
    ...
    for(auto& p : ports)
    {
        if (p.secure())
        {
            if (p.ssl_key.empty() && p.ssl_cert.empty() && p.ssl_chain.empty())
                ...
                p.context = make_SSLContextAuthed (p.ssl_key, p.ssl_cert, p.ssl_chain);
        }
        else
        {
...
parse_Port (ParsedPort& port, Section const& section, std::ostream& log)
{
...

    {
        ...
        if (result.second)
        {
            for (auto const& s : std::rfc2616::split_commas(
...

    {
        ...
        if (result.second)
        {
            ...
            {
                auto const addr = address_from_string (ip);
                if (! addr.second)
                {
                ...
                port.admin_ip->emplace_back (addr.first);
            }
        }
    }
...
make_ServerHandler (beast::Stoppable& parent,
                    ...
                    CollectorManager& cm)
{
    return std::make_unique <ServerHandlerImp> (parent, io_service, jobQueue, networkOPs, resourceManager, cm);
}
