...
#ifndef BESSEL_SERVER_PEER_H_INCLUDED
#define BESSEL_SERVER_PEER_H_INCLUDED

#include <services/server/impl/Door.h>
#include <services/server/Session.h>
#include <services/server/impl/ServerImpl.h>
    ...
    , journal_ (journal)
{
    ...
    if (journal_.trace) 
        journal_.trace << id_ << "accept:    " << remote_address_.address().to_string();
...

    {
        ...
        std::strftime(buf, sizeof(buf), "%Y-%b-%d %H:%M:%S", std::localtime(&t));
...
Peer<Impl>::~Peer()
{
    ...
    stat.elapsed   = std::chrono::duration_cast <std::chrono::seconds> (clock_type::now() - when_);
...
Peer<Impl>::close()
{
    if (! strand_.running_in_this_thread())
        return strand_.post(std::bind((void(Peer::*)(void))&Peer::close, impl().shared_from_this()));
...
Peer<Impl>::do_read (yield_context yield)
{
    ...
    for(;;)
    {
        ...
        if (! ec)
        {
            if (! eof)
            {
                //  TODO Currently parsing errors are treated the
...
Peer<Impl>::do_write (yield_context yield)
{
    ...
    for(;;)
    {
        ...
        start_timer();
        bytes = boost::asio::async_write (impl().stream_, boost::asio::buffer (data, bytes), boost::asio::transfer_at_least(1), yield[ec]);
        cancel_timer();
                       ...
                       yield_context yield)
{
    ...
    boost::asio::spawn (strand_, std::bind (&Peer<Impl>::do_read, impl().shared_from_this(), std::placeholders::_1));
}
...
Peer<Impl>::write (void const* buffer, std::size_t bytes)
{
    ...
    if (empty)
        boost::asio::spawn (strand_, std::bind (&Peer<Impl>::do_write, impl().shared_from_this(), std::placeholders::_1));
}
