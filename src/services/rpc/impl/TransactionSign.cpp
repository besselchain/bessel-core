...
void LedgerFacade::snapshotAccountState (BesselAddress const& accountID)
{
...
bool LedgerFacade::findPathsForOneIssuer (
    BesselAddress const& dstAccountID,
    ...
    STPath& fullLiquidityPath) const
{
    ...
    auto cache = std::make_shared<BesselLineCache> (ledger_);
    return bessel::findPathsForOneIssuer (
        cache,
...
bool LedgerFacade::checkQuorum(BesselAddress const& account, std::map<Account, std::pair<KeyPair,bool> >& key_pairs) const
{
    if (!ledger_) 
        return false;
...
NetworkOPs& LedgerFacade::getNetworkOPs() const
{
  ...
  */
  ...
  static Json::Value signPayment(
      ...
      Json::Value& tx_json,
      BesselAddress const& raSrcAddressID,
      RPCDetail::LedgerFacade& ledgerFacade,
      Role role,
      int const& index = 0)
  {
      boost::format  error = boost::format("tx_json[%d].%s");
      BesselAddress dstAccountID;
  ...
  Json::Value getOperations(
      Json::Value& params,
      NetworkOPs& nOps,
      std::set<BesselAddress>& accounts,
