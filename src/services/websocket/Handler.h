...
#ifndef BESSEL_APP_WEBSOCKET_WSSERVERHANDLER_H_INCLUDED
#define BESSEL_APP_WEBSOCKET_WSSERVERHANDLER_H_INCLUDED
...
#include <services/websocket/Connection.h>
#include <services/websocket/WebSocket.h>
#include <services/rpc/handlers/RPCInfo.h>
#include <services/websocket/WebSocket04.h>
#include <memory>
...
inline
boost::asio::ip::tcp::endpoint makeBeastEndpoint (boost::asio::ip::tcp::endpoint const& e)
{
    return e;
}
...
public:
    ...
    void send (connection_ptr const& cpClient, 
               std::string const& strMessage,
               bool broadcast)
    {
        try
        {
    ...
    void on_open (connection_ptr cpClient) override
    {
        ...
        try
        {
            ...
            (void) result.second;
            WriteLog (lsDEBUG, HandlerLog) << "Ws:: on_open(" 
                                           << convert_endpoint_to_string(remoteEndpoint) 
                                           << ")";
        }
        catch (...)
        {
    ...
    void on_pong (connection_ptr cpClient, std::string data) override
    {
        ...
        try
        {
            WriteLog (lsDEBUG, HandlerLog) << "Ws:: on_pong(" << convert_endpoint_to_string(cpClient->get_socket ().remote_endpoint()) << ")";
        }
        catch (...)
        {
        }
    ...
    void doClose (connection_ptr const& cpClient, char const* reason)
    {
        ...
        wsc_ptr ptr;
        {
            ...
            if (it == mMap.end ())
            {
                try
                {
                    WriteLog (lsDEBUG, HandlerLog) << "Ws:: " << reason 
                                                   << "(" << convert_endpoint_to_string(cpClient->get_socket ().remote_endpoint()) 
                                                   << ") not found";
                }
                catch (...)
                {
        ...
        {
            WriteLog (lsDEBUG, HandlerLog) << "Ws:: " << reason
                                           << "(" <<  convert_endpoint_to_string(cpClient->get_socket ().remote_endpoint ()) << ") found";
        }
        catch (...)
        {
        }
    ...
    void on_message (connection_ptr cpClient, message_ptr mpMessage) override
    {
        ...
        if (bRejected)
        {
            try
            {
                WriteLog (lsDEBUG, HandlerLog) << "Ws:: Rejected(" 
                                               << convert_endpoint_to_string(cpClient->get_socket().remote_endpoint()) <<
                     ...
                     const wsc_ptr& conn, const message_ptr& mpMessage)
    {
        ...
        else
        {
            ...
            RPC::RPCInfo::updateCmd(jvRequest,false);

            auto const start (std::chrono::high_resolution_clock::now ());
            Json::Value const jvObj (conn->invokeCommand (jvRequest));
            RPC::RPCInfo::updateError(jvRequest,jvObj,false);
