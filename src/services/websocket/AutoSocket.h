...
#ifndef BESSEL_WEBSOCKET_AUTOSOCKET_AUTOSOCKET_H_INCLUDED
#define BESSEL_WEBSOCKET_AUTOSOCKET_AUTOSOCKET_H_INCLUDED
...
public:
    ...
    static bool rfc2818_verify (std::string const& domain, bool preverified, boost::asio::ssl::verify_context& ctx)
    {
        ...
        WriteLog (lsWARNING, AutoSocket) << "Outbound SSL connection to " << domain 
...
*/
    ...
    void async_handshake (handshake_type type, callback cbFunc)
    {
        ...
        else if (mBuffer.empty ())
        {
            // must be plain
            mSecure = false;
            mSocket->get_io_service ().post (std::bind(cbFunc, error_code()));
        }
        else
        {
            // autodetect
    ...
    void async_shutdown (ShutdownHandler handler)
    {
        ...
        else
        {
            ...
            mSocket->get_io_service ().post (std::bind(handler, ec));
        }
    }
...
protected:
    void handle_autodetect (callback cbFunc, const error_code& ec, size_t bytesTransferred)
    {
        ...
        if (ec)
        {
            WriteLog (lsWARNING, AutoSocket) << "Handle autodetect error: " << ec;
