...
#ifndef BESSEL_APP_WEBSOCKET_WSCONNECTION_H_INCLUDED
#define BESSEL_APP_WEBSOCKET_WSCONNECTION_H_INCLUDED
...
/** A Bessel WebSocket connection handler.
*/
template <class WebSocket>
...
private:
    ...
    boost::asio::ip::tcp::endpoint const m_remoteAddress;
...
{
    WriteLog (lsDEBUG, ConnectionImpl) << "WebSocket: rcvMessage";
...
Json::Value ConnectionImpl <WebSocket>::invokeCommand (Json::Value& jvRequest)
{
    ...
    else
    {
       ...
       // RPC::RPCInfo::update(context.params,false,jvResult[jss::result]); //
    }
...
{
    WriteLog (lsDEBUG, ConnectionImpl) << "WebSocket: sending '" << to_string (jvObj);
...
{
    WriteLog (lsWARNING, ConnectionImpl) << "WebSocket: disconnecting";

    connection_ptr ptr = m_connection.lock ();

    if (ptr)
        this->m_io_service.dispatch (WebSocket::getStrand (*ptr).wrap (std::bind (&ConnectionImpl <WebSocket>::handle_disconnect, m_connection)));
}
...
template <class WebSocket>
void ConnectionImpl <WebSocket>::pingTimer (typename WebSocket::ErrorCode const& e)
{
    if (!e)
    {
