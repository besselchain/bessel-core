...
#include <services/websocket/Handler.h>
#include <services/websocket/Server.h>
...
{
    connection.close (websocketpp::close::status::protocol_error, "overload");
}

void WebSocket04::closeTooSlowClient (Connection& connection,
                                      unsigned int timeout,
                                      std::string const& message)
{
    connection.close (websocketpp::close::status::value (timeout), message);
}
...
void ConnectionImpl <WebSocket04>::setPingTimer ()
{
    ...
    if (auto con = m_connection.lock ())
    {
        ...
        con->set_timer (
            ms,
            std::bind (&ConnectionImpl<WebSocket04>::pingTimer, shared_from_this(),
                                std::placeholders::_1));
    }
}
...
void Server <WebSocket04>::listen()
{
    ...
    WriteLog (lsWARNING, WebSocket) << "Server run with: '" << c;
