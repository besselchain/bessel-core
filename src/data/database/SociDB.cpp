...
{
#ifdef USEMYSQL
	return std::make_pair(config.section("mysql_config").lines()[0], std::ref(soci::mysql));
#else
...
#ifdef USEMYSQL
static
MYSQL* getConnection(soci::session&s)
{
	MYSQL* result = nullptr;
	auto be = s.get_backend();
	if (auto b = dynamic_cast<soci::mysql_session_backend*> (be))
		result = b->conn_;
...
{
	//hard code to be 16M for mysql
	return static_cast <size_t> (16*1024);
}

size_t getKBUsedDB(soci::session& s)
{
	//hard code to be 5M for mysql
	return static_cast <size_t> (5 * 1024);
}
...
private:
    ...
    static
    int sqliteWALHook (void* cp, sqlite_api::sqlite3*, const char* dbName, int walSize)
    {
    ...
    void scheduleCheckpoint ()
    {
        ...
        jobQueue_.addJob (jtWAL, "WAL", std::bind (&WALCheckpointer::checkpoint, this));
    }

    void checkpoint ()
    {
        int log = 0, ckpt = 0;
        int ret = sqlite3_wal_checkpoint_v2 (&conn_, nullptr, SQLITE_CHECKPOINT_PASSIVE, &log, &ckpt);

        auto fname = sqlite3_db_filename (&conn_, "main");
        if (ret != SQLITE_OK)
        {
            WriteLog ((ret == SQLITE_LOCKED) ? lsTRACE : lsWARNING, WALCheckpointer) << "WAL(" << fname << "): error " << ret;
        }
        else
        {
            WriteLog (lsTRACE, WALCheckpointer) << "WAL(" << fname << "): frames=" << log << ", written=" << ckpt;
        }
...
std::unique_ptr <Checkpointer> makeCheckpointer (soci::session& session, JobQueue& queue)
{
#ifdef USEMYSQL
	return {};
#else
