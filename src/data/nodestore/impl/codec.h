...
#ifndef BESSEL_NODESTORE_CODEC_H_INCLUDED
#define BESSEL_NODESTORE_CODEC_H_INCLUDED
    ...
    std::size_t in_size, BufferFactory&& bf)
{
    ...
    case 2: // inner node
    {
        ...
        beast::nudb::detail::istream is(p, in_size);
        std::uint16_t mask;
        ...
        void* const out = bf(result.second);
        result.first = out;
        beast::nudb::detail::ostream os(out, result.second);
    ...
    std::size_t in_size, BufferFactory&& bf)
{
    ...
    if (in_size == 525)
    {
        beast::nudb::detail::istream is(in, in_size);
        std::uint32_t index;
        std::uint32_t unused;
        std::uint8_t  kind;
        std::uint32_t prefix;
...
filter_inner (void* in, std::size_t in_size)
{
    ...
    if (in_size == 525)
    {
        ...
        if (prefix == HashPrefix::innerNode)
        {
            beast::nudb::detail::ostream os(in, 9);
            write<std::uint32_t>(os, 0);
            write<std::uint32_t>(os, 0);
            write<std::uint8_t> (os, hotUNKNOWN);
        }
