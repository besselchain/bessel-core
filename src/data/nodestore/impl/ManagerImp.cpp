...
#include <data/nodestore/impl/ManagerImp.h>
#include <data/nodestore/impl/DatabaseImp.h>
#include <data/nodestore/impl/DatabaseRotatingImp.h>
...
ManagerImp::instance()
{
    static bessel::static_initializer<ManagerImp> _;
    return _.get();
}
...
ManagerImp::missing_backend()
{
    throw std::runtime_error (
        "Your besseld.cfg is missing a [node_db] entry, "
        "please see the besseld-example.cfg file!"
        );
}
...
ManagerImp::find (std::string const& name)
{
    ...
    auto const iter = std::find_if(list_.begin(), list_.end(),
        [&name](Factory* other)
        {
            return std::equal(std::begin(name), std::end(name), other->getName().begin());
        } );
