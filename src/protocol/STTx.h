...
#ifndef BESSEL_PROTOCOL_STTX_H_INCLUDED
#define BESSEL_PROTOCOL_STTX_H_INCLUDED
...
public:
    ...
    //-----------delete
    BesselAddress getOperationAccount() const
    {
        return getFieldAccount(sfAccount);
    }

    BesselAddress getSourceAccount () const
    {
        return getFieldAccount (sfAccount);
    }
    ...
    void setSigningPubKey (const BesselAddress & naSignPubKey);
    void setSourceAccount (const BesselAddress & naSource);
    ...
    void getSignAccount(std::set<Account> &vecAccount) const
    {
        if (isFieldPresent(sfSigns) && getTxnType() == ttOPERATION)
        {
            ...
            for (auto i : stArr)
            {
                BesselAddress acc = BesselAddress::createAccountPublic(i.getFieldVL(sfSigningPubKey));
                vecAccount.emplace(acc.getAccountID());
            }
        }
        else
        {
            BesselAddress acc = BesselAddress::createAccountPublic(getFieldVL(sfSigningPubKey));
            vecAccount.emplace(acc.getAccountID());
        }
    }

    std::vector<BesselAddress> getMentionedAccounts () const;

    //Batch operation,
    std::map<int, std::vector<Account> > getMentionedAccounts(BesselAddress const& account);
    ...
    void sign (BesselAddress const& private_key);

    void sign(BesselAddress const&, std::map<Account, std::pair<KeyPair, bool> >const&, Json::Value&);
