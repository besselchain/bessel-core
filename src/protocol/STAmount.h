...
#ifndef BESSEL_PROTOCOL_STAMOUNT_H_INCLUDED
#define BESSEL_PROTOCOL_STAMOUNT_H_INCLUDED

#include <protocol/SField.h>
#include <protocol/Serializer.h>
#include <protocol/STBase.h>
...
private:
    ...
    bool mIsNative;      // A shorthand for isSWT(mIssue).
    bool mIsNegative;
...
public:
    ...
    //  Is this needed when we have the previous signature?
    STAmount (Issue const& issue,
...
public:
    ...
    zeroed() const
    {
        ...
        // See https://bessellabs.atlassian.net/browse/WC-1847?jql=
        return STAmount (mIssue);
    }

    //  TODO This can be a free function or just call the
    //             member on the issue.
    std::string
    ...
    //  TODO Remove this, it is only called from the unit test
    void roundSelf();
    ...
    //  TODO Rename to setValueOnly (it only sets mantissa and exponent)
    //             Make this private
...
//  TODO The parameter type should be Quality not uint64_t
STAmount
...
//  TODO Return a Quality object
std::uint64_t
getRate (STAmount const& offerOut, STAmount const& offerIn);

// When the currency is SWT, the value in raw unsigned units.
std::uint64_t
getNValue(STAmount const& amount);

//------------------------------------------------------------------------------

inline bool isSWT(STAmount const& amount)
{
    return isSWT (amount.issue().currency);
}

bool isUserTum(Currency const& currency);

//  TODO Make static member accessors for these in STAmount
