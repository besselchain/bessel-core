...
#ifndef BESSEL_PROTOCOL_INDEXES_H_INCLUDED
#define BESSEL_PROTOCOL_INDEXES_H_INCLUDED
...
// of 256).
uint256 getLedgerHashIndex (std::uint32_t desiredLedgerIndex);
...
uint256 getAccountRootIndex (const BesselAddress & account);
uint256 getGeneratorIndex   (Account const& uGeneratorID);
uint256 getBookBase   (Book const& book);
uint256 getOfferIndex (Account const& account, std::uint32_t uSequence);
uint256 getOwnerDirIndex (Account const& account);
uint256 getDirNodeIndex  (uint256 const& uDirRoot, const std::uint64_t uNodeIndex);
uint256 getQualityIndex  (uint256 const& uBase, const std::uint64_t uNodeDir = 0);
uint256 getQualityNext   (uint256 const& uBase);

//  This name could be better
std::uint64_t getQuality (uint256 const& uBase);
uint256 getTicketIndex   (Account const& account, std::uint32_t uSequence);
uint256 getBesselStateIndex (Account const& a, Account const& b, Currency const& currency);
uint256 getBesselStateIndex (Account const& a, Issue const& issue);
...
uint256 getNickNameIndex   (const BesselAddress & account);
uint256 getContractIndex   (Account const& a, std::uint32_t uType);
uint256 getContractStateIndex (Account const& a, Blob & name);
