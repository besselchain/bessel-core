...
#ifndef BESSEL_PROTOCOL_STPATHSET_H_INCLUDED
#define BESSEL_PROTOCOL_STPATHSET_H_INCLUDED
...
public:
    STPathElement (
        ...
        , mIssuerID (issuer), is_offer_ (isSWT(mAccountID))
    {
        if (!is_offer_)
            mType |= typeAccount;

        if (forceCurrency || !isSWT(currency))
            mType |= typeCurrency;
