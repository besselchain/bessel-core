...
STValidation::STValidation (SerialIter& sit, bool checkSignature)
    ...
    , mTrusted (false)
{
    mNodeID = BesselAddress::createNodePublic (getFieldVL (sfSigningPubKey)).getNodeID ();
...
STValidation::STValidation (
    ...
    BesselAddress const& raPub, bool isFull)
...
void STValidation::sign (BesselAddress const& raPriv)
{
    uint256 signingHash;
    sign (signingHash, raPriv);
}

void STValidation::sign (uint256& signingHash, BesselAddress const& raPriv)
{
...
bool STValidation::isValid (uint256 const& signingHash) const
{
    try
    {
        ...
        BesselAddress   raPublicKey = BesselAddress::createNodePublic (getFieldVL (sfSigningPubKey));
...
BesselAddress STValidation::getSignerPublic () const
{
    BesselAddress a;
