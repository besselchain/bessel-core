...
SField const sfMasterWeight    = make::one(&sfMasterWeight,    STI_UINT16, 3, "MasterWeight");
SField const sfWeight		   = make::one(&sfWeight,          STI_UINT16, 4, "Weight");
...
SField const sfTimestamp         = make::one(&sfTimestamp,         STI_UINT32, 15, "Timestamp");
...
SField const sfRelationType        = make::one(&sfRelationType,        STI_UINT32, 35, "RelationType");
SField const sfQuorum			   = make::one(&sfQuorum,			   STI_UINT32, 36, "Quorum");
SField const sfQuorumHigh		   = make::one(&sfQuorumHigh,          STI_UINT32, 37, "QuorumHigh");

//configcontract
SField const sfMethod              = make::one(&sfMethod,              STI_UINT32, 38, "Method");
SField const sfContracttype        = make::one(&sfContracttype,        STI_UINT32, 39, "Contracttype");
...
SField const sfMessageHash     = make::one(&sfMessageHash,     STI_HASH256, 10, "MessageHash");
...
//SField              const sfDomain = make::one(&sfDomain,  STI_HASH256, 21, "Domain");
...
SField const sfBesselEscrow   = make::one(&sfBesselEscrow,   STI_AMOUNT, 17, "BesselEscrow");
...
TypedField<STBlob>  const sfSigningKey      = make::one<STBlob>(&sfSigningKey,    STI_VL,  5, "SigningKey", SField::sMD_Default, false);
...
SField              const sfNickname        = make::one(&sfNickname,      STI_VL, 15, "Nickname");
SField              const sfPayload         = make::one(&sfPayload,       STI_VL, 16, "Payload");
SField              const sfinfo            = make::one(&sfinfo,          STI_VL, 17, "info");
SField              const sfContractMethod  = make::one(&sfContractMethod,STI_VL, 18, "ContractMethod");
SField              const sfParameter       = make::one(&sfParameter,     STI_VL, 19, "Parameter");
// account
...
SField const sfFeeAccountID  = make::one(&sfFeeAccountID,  STI_ACCOUNT, 9, "FeeAccountID");
SField const sfBlackListAccountID  = make::one(&sfBlackListAccountID,  STI_ACCOUNT, 10, "BlackListAccountID");
SField const sfIssuerAccountID  = make::one(&sfIssuerAccountID,  STI_ACCOUNT, 11, "IssuerAccountID");
...
SField const sfOperation           = make::one(&sfOperation,           STI_OBJECT, 11, "Operation");
SField const sfSign                = make::one(&sfSign,                STI_OBJECT, 12, "Sign", SField::sMD_Default, false);
SField const sfSignerEntry         = make::one(&sfSignerEntry,         STI_OBJECT, 13, "SignerEntry");
SField const sfCurrencyRate        = make::one(&sfCurrencyRate,        STI_OBJECT, 14, "CurrencyRate");
...
SField const sfOperations        = make::one(&sfOperations,       STI_ARRAY, 10, "Operations");
SField const sfSigns             = make::one(&sfSigns,            STI_ARRAY, 11, "Signs", SField::sMD_Default, false);
SField const sfSignerEntries     = make::one(&sfSignerEntries,    STI_ARRAY, 12, "SignerEntries");
SField const sfCurrencyRates     = make::one(&sfCurrencyRates,    STI_ARRAY, 13, "CurrencyRates");
