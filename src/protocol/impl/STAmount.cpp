    ...
    : STBase(name)
{
    ...
    if (isSWT (issue.currency))
        throw std::runtime_error ("invalid native currency");

    issue.account.copyFrom (sit.get160 ());

    if (isSWT (issue.account))
...
STAmount::setValue (std::string const& amount)
{
    ...
    try
    {
        ...
        // Can't specify SWT using fractional representation
        if (mIsNative && match[3].matched)
            return false;

        if (!match[4].matched) // integer only
        {
            mValue = boost::lexical_cast <std::uint64_t> (std::string (match[2]));
            mOffset = 0;
        }
        else
        {
            // integer and fraction
            mValue = boost::lexical_cast <std::uint64_t> (match[2] + match[4]);
        ...
        if (match[5].matched)
        {
            ...
            if (match[6].matched && (match[6] == "-"))
                mOffset -= boost::lexical_cast <int> (std::string (match[7]));
            else
                mOffset += boost::lexical_cast <int> (std::string (match[7]));
        }
...
bool isUserTum(Currency const& currency)
{
    ...
    if (!to_currency (base_currency,"8000000000000000000000000000000000000000"))
    {
	    std::cout << "to_currency base_currency error  : "<<std::endl;
    }
...
//  What does this perplexing function do?
...
amountFromJson (SField const& name, Json::Value const& v)
{
    ...
    {
        if (v.isObject ())
            throw std::runtime_error ("SWT may not be specified as an object");
    }
    else
    {
        // non-SWT
    ...
    else if (value.isString ())
    {
        if (native)
        {
            std::int64_t val = boost::lexical_cast <std::int64_t> (
                value.asString ());
...
operator<= (STAmount const& lhs, std::uint64_t rhs)
{
    //  TODO The cast looks dangerous, is it needed?
    return getSNValue (lhs) <= static_cast<std::int64_t>(rhs);
}
...
multiply (STAmount const& v1, STAmount const& v2, Issue const& issue)
{
    ...
    if (v1.native() && v2.native() && isSWT (issue))
    {
    ...
    Issue const& issue, bool roundUp)
{
    ...
    canonicalizeRound (
        isSWT (issue), amount, offset, resultNegative != roundUp);
