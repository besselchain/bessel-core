...
uint256
getAccountRootIndex (const BesselAddress & account)
{
...
uint256
getBesselStateIndex (Account const& a, Account const& b, Currency const& currency)
{
    Serializer  s (62);
...
uint256
getBesselStateIndex (Account const& a, Issue const& issue)
{
    return getBesselStateIndex (a, issue.account, issue.currency);
}
...
uint256
getNickNameIndex(const BesselAddress & account)
{
	return  getNickNameIndex(account.getAccountID());
}

uint256
getContractIndex(Account const& a, std::uint32_t uType)
{
    Serializer s(28);

    s.add16 (spaceContract);
    s.add160(a);
    s.add32(uType);
...
uint256 
getContractStateIndex (Account const& a, Blob & name)
{
     Serializer  s (20);
     s.add16  (spaceContract);
     s.add160 (a);
