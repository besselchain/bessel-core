...
namespace bessel {
	...
	std::map<int, std::vector<Account> > STTransaction::getMentionedAccounts(BesselAddress const& account)
	{
	...
	void STTransaction::sign(BesselAddress const& private_key)
	{
	...
	void STTransaction::setSigningPubKey(BesselAddress const& naSignPubKey)
	{
		setFieldVL(sfSigningPubKey, naSignPubKey.getAccountPublic());
	}

	void STTransaction::setSourceAccount(BesselAddress const& naSource)
	{
