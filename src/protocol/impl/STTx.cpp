...
std::map<int, std::vector<Account> > STTx::getMentionedAccounts(BesselAddress const& account)
{
...
void STTx::sign (BesselAddress const& private_key)
{
...
void STTx::sign(
   BesselAddress const& private_key,  //operation account
...
void STTx::setSigningPubKey (BesselAddress const& naSignPubKey)
{
    setFieldVL (sfSigningPubKey, naSignPubKey.getAccountPublic ());
}

void STTx::setSourceAccount (BesselAddress const& naSource)
{
