...
#include <common/misc/Validations.h>
...
#include <transaction/tx/TransactionAcquire.h>
#include <transaction/tx/InboundTransactions.h>
#include <network/overlay/Overlay.h>
#include <network/overlay/predicates.h>
...
public:
    ...
    */
    void mapComplete (uint256 const& hash, 
                      ...
                      bool acquired)
    {
        ...
        catch (SHAMapMissingNode const& mn)
        {
            ...
            WriteLog (lsERROR, LedgerConsensus) << "Missing node processing complete map " << mn;
    ...
    {
        CondLog (acquired, lsDEBUG, LedgerConsensus) << "We have acquired TXS " << hash;
    ...
    void checkLCL ()
    {
        ...
        if (netLgr != mPrevLedgerHash)
        {
            ...
            WriteLog (lsWARNING, LedgerConsensus) << mPrevLedgerHash
                << " to " << netLgr;
            WriteLog (lsWARNING, LedgerConsensus)
                << bessel::getJson (*mPreviousLedger);
    ...
    void handleLCL (uint256 const& lclHash)
    {
        ...
        if (!newLCL)
        {
            if (mAcquiringLedger != lclHash)
            {
                ...
                getApp().getJobQueue().addJob (jtADVANCE,
                                               "getConsensusLedger",
                                               std::bind (&InboundLedgers::acquire,
                                                          &getApp().getInboundLedgers(),
                                                          mPrevLedgerHash, 
                                                          0, 
                                                          InboundLedger::fcCONSENSUS));
        ...
        mCloseResolution = ContinuousLedgerTiming::getNextLedgerTimeResolution (mPreviousLedger->getCloseResolution (),
                                                                                mPreviousLedger->getCloseAgree (),
    ...
    */
    void timerEntry ()
    {
        ...
        catch (SHAMapMissingNode const& mn)
        {
            ...
            WriteLog (lsERROR, LedgerConsensus) << "Missing node during consensus process " << mn;
    ...
    void doTimer ()
    {
        ...
        mCurrentMSeconds = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - mConsensusStartTime).count ();
...
/*        
    ...
    */
    bool peerPosition (LedgerProposal::ref newPosition)
    {
        ...
        {
            // peer bows out
            WriteLog (lsINFO, LedgerConsensus) << "Peer bows out: " << to_string (peerID);
        ...
        std::shared_ptr<SHAMap> set = getTransactionTree (newPosition->getCurrentHash ());
    ...
    bool isOurPubKey (const BesselAddress & k)
    {
        return k == mValPublic;
    }
...
private:
    ...
    */
    void accept (std::shared_ptr<SHAMap> set)
    {
        ...
        int asf = newLCL->peekAccountStateMap ()->flushDirty (hotACCOUNT_NODE, newLCL->getLedgerSeq());
        int tmf = newLCL->peekTransactionMap ()->flushDirty (hotTRANSACTION_NODE, newLCL->getLedgerSeq());

        WriteLog (lsDEBUG, LedgerConsensus) << "Flushed " << asf << " account and " << tmf << "transaction nodes";
        ...
        if (getApp().getLedgerMaster().storeLedger (newLCL))
            WriteLog (lsDEBUG, LedgerConsensus) << "Consensus built ledger we already had";
        else if (getApp().getInboundLedgers().find (newLCL->getHash()))
            WriteLog (lsDEBUG, LedgerConsensus) << "Consensus built ledger we were acquiring";
        else
            WriteLog (lsDEBUG, LedgerConsensus) << "Consensus built new ledger";
        ...
        if (mValidating && !mConsensusFail)
        {
            ...
            WriteLog (lsINFO, LedgerConsensus) << "CNF Val " << newLCLHash;
        }
        else
        {
        ...
        Ledger::pointer newOL = std::make_shared<Ledger>(true, *newLCL);
        ...
        {
            auto lock = std::unique_lock<std::recursive_mutex>(getApp().getMasterMutex(), std::defer_lock);
        ...
        if (mValidating)
        {
            ...
            WriteLog (lsINFO, LedgerConsensus)
                << "We closed at "
                <<boost::lexical_cast<std::string> (mCloseTime);

            std::uint64_t closeTotal = mCloseTime;
            int closeCount = 1;

            for (auto it = mCloseTimes.begin (), end = mCloseTimes.end (); 
                it != end; 
                ++it)
            {
                ...
                closeTotal += static_cast<std::uint64_t> (it->first) * static_cast<std::uint64_t> (it->second);
            }
    ...
    */
    void addDisputedTransaction (uint256 const& txID, Blob const& tx)
    {
        ...
        if (getApp().getHashRouter ().setFlag (txID, SF_RELAYED))
        {
            ...
            getApp ().overlay ().foreach (send_always (std::make_shared<Message> (msg, protocol::mtTRANSACTION)));
        }
    }
    ...
    */
    void propose ()
    {
        WriteLog (lsTRACE, LedgerConsensus) << "We propose: " 
                                            << (mOurPosition->isBowOut () ? std::string ("bowOut") : to_string (mOurPosition->getCurrentHash ()));
    ...
    */
    void sendHaveTxSet (uint256 const& hash, bool direct)
    {
        ...
        getApp ().overlay ().foreach (send_always (std::make_shared <Message> (msg, protocol::mtHAVE_SET)));
    }
    ...
    */
    void statusChange (protocol::NodeEvent event, Ledger& ledger)
    {
        ...
        s.set_ledgerhashprevious(ledger.getParentHash ().begin (), std::decay_t<decltype(ledger.getParentHash ())>::bytes);
        s.set_ledgerhash (ledger.getHash ().begin (), std::decay_t<decltype(ledger.getHash ())>::bytes);
        ...
        s.set_firstseq (uMin);
        s.set_lastseq (uMax);
        getApp ().overlay ().foreach (send_always (std::make_shared <Message> (s, protocol::mtSTATUS_CHANGE)));
    ...
    */
    void takeInitialPosition (Ledger& initialLedger)
    {
                ...
                && ((mPreviousLedger->getLedgerSeq () % 256) == 0))
        {
            ...
            std::shared_ptr<SHAMap> preSet = initialLedger.peekTransactionMap ()->snapShot (true);
        ...
        if (mValidating)
        {
            mOurPosition = std::make_shared<LedgerProposal> (mValPublic, 
                                                             mValPrivate,
        ...
        {
            mOurPosition = std::make_shared<LedgerProposal> (initialLedger.getParentHash (), txSet, mCloseTime);
        }
    ...
    */
    void updateOurPositions ()
    {
        ...
        auto peerCutoff = std::chrono::steady_clock::now ();
        auto ourCutoff  = peerCutoff - std::chrono::seconds (PROPOSE_INTERVAL);
        ...
        else
        {
            ...
            for (auto it = closeTimes.begin (), end = closeTimes.end (); 
                 it != end; 
                 ++it)
            {
            ...
            CondLog (!mHaveCloseTimeConsensus, lsDEBUG, LedgerConsensus) << "No CT consensus: Proposers:" 
                                                                         << mPeerPositions.size ()
                                                                         << " Proposing:" 
                                                                         << (mProposing ? "yes" : "no") 
                                                                         << " Thresh:"
        ...
        if (changes)
        {
            ...
            WriteLog (lsINFO, LedgerConsensus) << "Position change: CTime " 
                                               << closeTime
                                               << ", tx "
                                               << newHash;
    ...
    */
    void checkOurValidation ()
    {
        ...
        STValidation::pointer lastVal = getApp().getOPs ().getLastValidation ();
        ...
        uint256 signingHash;
        STValidation::pointer v = std::make_shared<STValidation> (mPreviousLedger->getHash (),
                                                                  getApp().getOPs ().getValidationTimeNC (), 
                                                                  mValPublic, 
                                                                  false);
        addLoad(v);
    ...
    */
    ...
    {
        std::shared_ptr<SHAMap> consensusSet = mAcquired[mOurPosition->getCurrentHash ()];
        ...
        {
            getApp().getJobQueue().addJob (jtACCEPT, "acceptLedger", std::bind (&LedgerConsensusImp::accept, shared_from_this (), consensusSet));
        }
    }
    ...
    */
    ...
    {
        std::uint32_t fee = std::max(getApp().getFeeTrack().getLocalFee(),
...
private:
    ...
    //  TODO Rename these to look pretty
    enum LCState
    {
    ...
    BesselAddress          mValPublic, mValPrivate;
...
make_LedgerConsensus (LocalTxs& localtx,
                      ...
                      FeeVote& feeVote)
{
    return std::make_shared <LedgerConsensusImp> (localtx, prevLCLHash, previousLedger, closeTime, feeVote);
}
                      ...
                      bool retryAssured)
{
    ...
    if ((getApp().getHashRouter ().getFlags (txn->getTransactionID ()) & SF_SIGGOOD) == SF_SIGGOOD)
    {
    ...
    if(openLedger == false && (txn->getTimestamp() >= engine.getLedger()->getCloseTimeNC () -1))
    {
    ...
    try
    {
        ...
        if (result.second)
        {
            WriteLog (lsDEBUG, LedgerConsensus) << "Transaction applied: " << transHuman (result.first);
        ...
        {
            // failure
            WriteLog (lsDEBUG, LedgerConsensus) << "Transaction failure: " << transHuman (result.first);

            return LedgerConsensusImp::resultFail;
        }

        WriteLog (lsDEBUG, LedgerConsensus) << "Transaction retry: " << transHuman (result.first);
                        ...
                        bool openLgr)
{
    ...
    if (set)
    {
        for (std::shared_ptr<SHAMapItem> item = set->peekFirstItem (); 
            !!item;
            item = set->peekNextItem (item->getTag ()))
        {
            ...
            if (!checkLedger->hasTransaction (item->getTag ()))
            {
                ...
                WriteLog (lsDEBUG, LedgerConsensus) << "Processing candidate transaction: " << item->getTag ();
    ...
    for (int pass = 0; pass < LEDGER_TOTAL_PASSES; ++pass)
    {
        ...
        while (it != retriableTransactions.end ())
        {
            try
            {
                switch (applyTransaction (engine, it->second, openLgr, certainRetry))
                {
