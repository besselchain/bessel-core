...
- Are we robustly connected to the Bessel network?
...
Logic::Logic (Store& store, beast::Journal journal)
    : /*store_ (store)
    , */journal_ (journal)
    , ledgers_(std::chrono::steady_clock::now())
{
}
...
{
    std::lock_guard<std::mutex> lock(mutex_);
    expire(std::chrono::seconds(5 * 60));
}
...
Logic::onValidation (STValidation const& v)
{
    ...
    auto const seq_no = v.getFieldU32 (sfLedgerSequence);
    ...
    if (journal_.trace) 
        journal_.trace << "onValidation: " << ledger;

    auto const result = ledgers_.map_.emplace (std::piecewise_construct, std::make_tuple(ledger), std::make_tuple());
    auto& meta = ledgers_.map_[ledger].meta_;
    ...
    {
        //std::pair<ledgerhash, meta>
        LedgerMeta& mt = ledgers_.map_[ledger].meta_;
        std::pair<LedgerHash, LedgerMeta> pair = std::make_pair(ledger, mt);
        if (policy_.acceptLedgerMeta (pair))
        {
            //ledgers_.clear();
            latest_ = pair;

            if (journal_.info) 
                journal_.info << "Accepted " << latest_.second.seq_no << " (" << ledger << ")";
...
Logic::onLedgerClosed (LedgerIndex index,
                       ...
                       LedgerHash const& parent)
{
    if (journal_.info) 
        journal_.info << "onLedgerClosed: " << index << " " << hash << " (parent " << parent << ")";
}
