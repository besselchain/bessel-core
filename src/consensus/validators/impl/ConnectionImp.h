...
#ifndef BESSEL_VALIDATORS_CONNECTIONIMP_H_INCLUDED
#define BESSEL_VALIDATORS_CONNECTIONIMP_H_INCLUDED
...
#include <consensus/validators/Connection.h>
...
private:
    ...
    using Item = std::pair<LedgerHash, BesselAddress>;
    ...
    boost::container::flat_map<BesselAddress, Source> sources_;
    boost::container::flat_set<BesselAddress>         good_;
...
public:
    ...
    onValidation (STValidation const& v) override
    {
...

        {
            ...
            if (journal_.debug) 
                journal_.debug << "onValidation: " << ledger;
#if 0
    ...
    onLedger (LedgerHash const& ledger)
    {
        ...
        if (journal_.debug) 
            journal_.debug << "onLedger: " << ledger;

        assert(ledger != ledger_);

        ledger_ = ledger;

        auto item   = items_.lower_bound(std::make_pair(ledger, BesselAddress()));
