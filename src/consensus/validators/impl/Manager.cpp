...
#include <consensus/validators/impl/ConnectionImp.h>
#include <consensus/validators/impl/Logic.h>
#include <consensus/validators/impl/StoreSqdb.h>
#include <functional>
...
/** ChosenValidators (formerly known as UNL)
    ...
    independently sign LedgerHash objects with their BesselPublicKey. These
    ...
    In order that besseld administrators may publish their ChosenValidators
    ...
    TODO:
      ...
      Template: https://bessel.com/wiki/Federation_protocol
      - What to do if you're a publisher of ValidatorList
      - What to do if you're a besseld administrator
      - Overview of how ChosenValidators works

    Goals:
      Make default configuration of besseld secure.
    ...
    What we want from the unique node list:
      ...
      - Besseld will ship with a list of trusted root "certificates"
...
public:
    ...
    onTimer (boost::system::error_code ec)
    {
        ...
        timer_.async_wait(strand_.wrap(std::bind(&ManagerImp::onTimer, 
                                                  this,
                                                  std::placeholders::_1)));
    }
};
...
Manager::Manager () : beast::PropertyStream::Source ("validators")
{
}
...
{
	return std::make_unique<ManagerImp>(parent, io_service, journal, config);
}
}
}
