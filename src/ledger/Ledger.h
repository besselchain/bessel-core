...
#ifndef BESSEL_APP_LEDGER_LEDGER_H_INCLUDED
#define BESSEL_APP_LEDGER_LEDGER_H_INCLUDED
...
//  TODO figure out exactly how this thing works.
...
public:
    ...
    Ledger (const BesselAddress & masterID, std::uint64_t startAmount);
    ...
    bool hasAccount (const BesselAddress & acctID) const;
    AccountState::pointer getAccountState (const BesselAddress & acctID) const;
	AccountState::pointer getAccountState1(const BesselAddress & acctID) const;
    LedgerStateParms writeBack (LedgerStateParms parms, SLE::ref);
    SLE::pointer getAccountRoot (Account const& accountID) const;
    SLE::pointer getAccountRoot (const BesselAddress & naAccountID) const;
    void updateSkipList ();
    ...
    //  NOTE These seem to let you walk the list of ledgers
    //
    ...
    //
    // Bessel functions : credit lines
    //

    SLE::pointer
    getBesselState (uint256 const& uNode) const;
    ...
    static uint256 getBlackListIndex (const Account& uAccountID);
    bool checkBlackList (const Account& uAccountID);

    static uint256 getLedgerManageFeeIndex ();
    static uint256 getLedgerManageIssuerIndex ();
	
    Account getFeeAccountID ();
    Account getManagerAccountID ();
    Account getIssuerOpAccountID ();
...
private:
    ...
    // Bessel cost of the reference transaction
    ...
    typedef BesselMutex StaticLockType;
