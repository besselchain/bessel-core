...
Ledger::Ledger (BesselAddress const& masterID, std::uint64_t startAmount)
...
void Ledger::updateHash ()
{
    ...
    //  TODO Fix this hard coded magic number 122
    Serializer s (122);
...
bool Ledger::hasAccount (BesselAddress const& accountID) const
{
...
AccountState::pointer Ledger::getAccountState (BesselAddress const& accountID) const
{
    SLE::pointer sle = getSLEi (getAccountRootIndex (accountID));
	
	SLE::pointer sle1 = getSLEi(getNickNameIndex(accountID));
    if (!sle)
    {
...
//getnicknamenode
AccountState::pointer Ledger::getAccountState1(const BesselAddress & accountID) const
{
	SLE::pointer sle = getSLEi(getNickNameIndex(accountID));
	if (!sle)
	{
		WriteLog(lsDEBUG, Ledger) << "Ledger:getAccountState:" 
                                  << " not found: " 
...
STTx::pointer Ledger::getSTransaction (std::shared_ptr<SHAMapItem> const& item, SHAMapTreeNode::TNType type)
{
...
STTx::pointer Ledger::getSMTransaction (std::shared_ptr<SHAMapItem> const& item, 
                                        SHAMapTreeNode::TNType type,
...
bool Ledger::getTransaction (uint256 const& txID, 
...
bool Ledger::saveValidatedLedger (bool current)
{
    ...
    // Save the ledger header in the hashed object store
    {
        ...
        getApp().getNodeStore ().store (hotLEDGER, std::move (s.modData ()), mHash);
    }
...

    {
        ...
        *db << boost::str (addLedger %
                           ...
                           boost::lexical_cast<std::string>(mTotCoins) % mCloseTime %
...
void finishLoadByIndexOrHash(Ledger::pointer& ledger)
{
    ...
    WriteLog (lsTRACE, Ledger) << "Loaded ledger: " << to_string (ledger->getHash ());
...
Ledger::pointer Ledger::loadByIndex (std::uint32_t ledgerIndex)
{
    Ledger::pointer ledger;
    {
        ...
        std::tie (ledger, std::ignore, std::ignore) = loadHelper (s.str ());
    }
...
Ledger::getHashesByIndex (std::uint32_t minSeq, std::uint32_t maxSeq)
{
    ...
    while (st.fetch ())
    {
        ...
        if (!ph)
        {
            WriteLog (lsWARNING, Ledger) << "Null prev hash for ledger seq: " << ls;
        }
    }
...
Ledger::pointer Ledger::getLastFullLedger ()
{
    try
    {
        ...
        std::uint32_t ledgerSeq;
        uint256 ledgerHash;
        std::tie (ledger, ledgerSeq, ledgerHash) = loadHelper ("order by LedgerSeq desc limit 1");
...
SLE::pointer Ledger::getAccountRoot (BesselAddress const& naAccountID) const
{
...
SLE::pointer
Ledger::getBesselState (uint256 const& uNode) const
{
    return getASNodeI (uNode, ltBESSEL_STATE);
}

SLE::pointer
Ledger::getBesselState (
    Account const& a, Account const& b, Currency const& currency) const
{
    return getBesselState (getBesselStateIndex (a, b, currency));
}
...
uint256 Ledger::getLedgerHash (std::uint32_t ledgerIndex)
{
    ...
    {
        WriteLog (lsWARNING, Ledger) << "Can't get seq " 
                                     << ledgerIndex
                                     << " from " 
                                     << mLedgerSeq 
                                     << " future";
    ...
    if (diff <= 256)
    {
        ...
        {
            WriteLog (lsWARNING, Ledger) << "Ledger "
                                         << mLedgerSeq
                                         << ":" << getHash ()
                                         << " missing normal list";
        }
    ...
    {
        WriteLog (lsDEBUG, Ledger) << "Can't get seq " 
                                   << ledgerIndex
                                   << " from " 
                                   << mLedgerSeq 
                                   << " past";
...
void Ledger::updateSkipList ()
{
    ...
    if ((prevIndex & 0xff) == 0)
    {
        ...
        //  TODO Document this skip list concept
        if (!skipList)
...
bool Ledger::pendSaveValidated (bool isSynchronous, bool isCurrent)
{
    ...
    {
        getApp().getJobQueue ().addJob (jtPUBLEDGER, 
                                        "Ledger::pendSave",
                                        std::bind (&Ledger::saveValidatedLedgerAsync, 
                                                   shared_from_this (),
                                                   std::placeholders::_1, 
                                                   isCurrent));
    }
    ...
    {
        getApp().getJobQueue ().addJob (jtPUBOLDLEDGER, 
                                        "Ledger::pendOldSave",
                                        std::bind (&Ledger::saveValidatedLedgerAsync, 
                                                   shared_from_this (),
                                                   std::placeholders::_1, 
...
void Ledger::updateFees (){
    ...
    std::uint32_t reserveIncrement = getConfig ().FEE_OWNER_RESERVE;
...
uint256 Ledger::getBlackListIndex (const Account& uAccountID)
{
    Serializer  s (22);

    s.add16 (spaceBlackList); //  2
    s.add160 (uAccountID);  // 20

    return s.getSHA512Half ();
}

uint256 Ledger::getLedgerManageFeeIndex ()
{
    // get the index of the node that holds the fee schedul
    Serializer s (2);
    s.add16 (spaceManageFee);
    return s.getSHA512Half ();
}

uint256 Ledger::getLedgerManageIssuerIndex ()
{
    // get the index of the node that holds the fee schedul
    Serializer s (2);
    s.add16 (spaceIssuer);
...
bool Ledger::checkBlackList (const Account& uAccountID)
{
	LedgerStateParms p = lepNONE;
	SLE::pointer sle = getASNode (p, Ledger::getBlackListIndex (uAccountID), ltBLACKLIST);
	if(sle)
		return true;
	else
		return false;
...
Account Ledger::getFeeAccountID ()
{
	Account feeaccountid = getConfig().FEE_ACCOUNTID.getAccountID();
	LedgerStateParms p = lepNONE;
	SLE::pointer sle = getASNode (p, Ledger::getLedgerManageFeeIndex (), ltMANAGE_FEE);

	if (sle)
	{		
		if (sle->getFieldIndex (sfFeeAccountID) != -1)
			feeaccountid = sle->getFieldAccount160 (sfFeeAccountID);
	}

	mFeeAccountID = feeaccountid;
	return mFeeAccountID;
}
Account Ledger::getManagerAccountID ()
{
	Account mngaccountid = getConfig().SMNG_ACCOUNTID.getAccountID();
	/*
	LedgerStateParms p = lepNONE;
	SLE::pointer sle = getASNode (p, Ledger::getLedgerManageFeeIndex (), ltMANAGE_FEE);
...
Account Ledger::getIssuerOpAccountID ()
{
	Account issueraccountid = getConfig().SISSUER_ACCOUNTID.getAccountID();
	LedgerStateParms p = lepNONE;
	SLE::pointer sle = getASNode (p, Ledger::getLedgerManageIssuerIndex (), ltMANAGE_ISSUER);

	if (sle)
	{		
		if (sle->getFieldIndex (sfIssuerAccountID) != -1)
			issueraccountid = sle->getFieldAccount160 (sfIssuerAccountID);
	}
