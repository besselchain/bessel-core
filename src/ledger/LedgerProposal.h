...
#ifndef BESSEL_APP_LEDGER_LEDGERPROPOSAL_H_INCLUDED
#define BESSEL_APP_LEDGER_LEDGERPROPOSAL_H_INCLUDED
...
public:
    ...
    LedgerProposal (uint256 const& prevLgr, std::uint32_t proposeSeq, uint256 const& propose,
                    std::uint32_t closeTime, const BesselAddress & naPeerPublic, uint256 const& suppress);

    // our first proposal
    LedgerProposal (const BesselAddress & pubKey, const BesselAddress & privKey,
    ...
    BesselAddress const& peekPublic () const
    {
        return mPublicKey;
    }
...
private:
    ...
    NodeID          mPeerID;
    BesselAddress   mPublicKey;
    BesselAddress   mPrivateKey;    // If ours
