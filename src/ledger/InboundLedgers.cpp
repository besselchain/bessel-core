...
#include <common/base/DecayingSample.h>
...
public:
    ...
    InboundLedgersImp (clock_type& clock, Stoppable& parent, beast::insight::Collector::ptr const& collector)
        ...
        , m_clock (clock)
        , mRecentFailures ("LedgerAcquireRecentFailures", clock, 0, kReacquireIntervalSeconds)
    ...
    */
            ...
            std::shared_ptr<protocol::TMLedgerData> packet_ptr)
    {
        ...
        if (!ledger)
        {
            ...
            {
                getApp().getJobQueue().addJob(jtLEDGER_DATA, "gotStaleData", std::bind(&InboundLedgers::gotStaleData, this, packet_ptr));
            }
        ...
        if (ledger->gotData(std::weak_ptr<Peer>(peer), packet_ptr))
        {
            getApp().getJobQueue().addJob (jtLEDGER_DATA, 
                                          "processLedgerData",
                                          std::bind (&InboundLedgers::doLedgerData, this, std::placeholders::_1, hash));
        }
    ...
    */
    ...
    void onLedgerFetched (InboundLedger::fcReason why)
    {
    ...
    void sweep ()
    {
        ...
        WriteLog (lsDEBUG, InboundLedger) << "Swept " 
                                          << stuffToSweep.size () 
                                          << " out of "
                                          << total 
                                          << " inbound ledgers.";
