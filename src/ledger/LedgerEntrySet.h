...
#ifndef BESSEL_APP_LEDGER_LEDGERENTRYSET_H_INCLUDED
#define BESSEL_APP_LEDGER_LEDGERENTRYSET_H_INCLUDED
...
#include <common/misc/Zero.h>

namespace bessel {

//  Does this belong here? Is it correctly named?
...
enum TransactionEngineParams
{
	...
	// Transaction was prechecked
	tapPRE_CHECKED   	= 0x1000,
	// Transaction was signed by master key
	tapMASTER_SIGN   	= 0x2000
};
...
public:
    ...
    TER issue_trust_auto (Account const& account,
        STAmount const& amount, Issue const& issue,bool autodel);
    ...
    TER accountFundsCheck (Account const& account, STAmount const& saTakerGets);

    Account AuthorizeAccountGet (Account const& account, Currency const& currency);
private:
    ...
    bool threadTx (
        const BesselAddress & threadTo, Ledger::ref ledger,
        NodeToLedgerEntry& newMods);
...
std::uint32_t
besselTransferRate(LedgerEntrySet& ledger, Account const& issuer, Currency const& currency = noCurrency());

std::uint32_t
besselTransferRate (LedgerEntrySet& ledger, Account const& uSenderID,
Account const& uReceiverID, Account const& issuer, Currency const& currency = noCurrency());
