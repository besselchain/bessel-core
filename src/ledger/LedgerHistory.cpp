...
LedgerHistory::LedgerHistory (
    ...
    , m_ledgers_by_hash ("LedgerCache", CACHED_LEDGER_NUM, CACHED_LEDGER_AGE, get_seconds_clock (), deprecatedLogs().journal("TaggedCache"))
    , m_consensus_validated ("ConsensusValidated", 64, 300, get_seconds_clock (), deprecatedLogs().journal("TaggedCache"))
{
}
...
log_one(Ledger::pointer ledger, uint256 const& tx, char const* msg)
{
    ...
    if (metaData != nullptr)
    {
        WriteLog (lsERROR, LedgerMaster) << "MISMATCH on TX " 
                                         << tx 
                                         << ": " 
                                         << msg 
        ...
        if (!nodes_diff)
        {
            if (result_diff && index_diff)
            {
                ...
                WriteLog (lsERROR, LedgerMaster) << " Built:" 
                                                 << " Result: "
                                                 << builtMetaData->getResult () 
                                                 << " Index: "
                                                 << builtMetaData->getIndex ();

                WriteLog (lsERROR, LedgerMaster) << " Valid:" 
                                                 << " Result: "
                                                 << validMetaData->getResult () 
                                                 << " Index: "
                                                 << validMetaData->getIndex ();
            }
            else if (result_diff)
            {
...
void LedgerHistory::handleMismatch (LedgerHash const& built, LedgerHash const& valid)
{
    ...
    if (builtLedger->getCloseTimeNC() != validLedger->getCloseTimeNC())
    {
        ...
        WriteLog (lsERROR, LedgerMaster) << "MISMATCH on close time:" <<builtLedger->getCloseTimeNC()<<" validLedger:"<< validLedger->getCloseTimeNC();
        return;
    }
