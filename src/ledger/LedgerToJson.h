...
#ifndef BESSEL_APP_LEDGER_LEDGERTOJSON_H_INCLUDED
#define BESSEL_APP_LEDGER_LEDGERTOJSON_H_INCLUDED
...
void fillJson (Object& json, LedgerFill const& fill)
{
    ...
    if (ledger.isClosed() || bFull)
    {
        ...
        if (closeTime != 0)
        {
            ...
            json[jss::close_time_human] = boost::posix_time::to_simple_string (ptFromSeconds (closeTime));
    ...
    if (transactionMap && (bFull || fill.options & LedgerFill::dumpTxrp))
    {
        ...
        CountedYield count (fill.yieldStrategy.transactionYieldCount, fill.yield);
