...
#include <common/shamap/SHAMapNodeID.h>
...
#include <ledger/TransactionStateSF.h>
...
InboundLedger::InboundLedger (uint256 const& hash, std::uint32_t seq, fcReason reason, clock_type& clock)
    : PeerSet (hash, ledgerAcquireTimeoutMillis, false, clock, deprecatedLogs().journal("InboundLedger"))
    , mHaveHeader (false)
    , mHaveState (false)
    ...
    , mReceiveDispatched (false)
{
    ...
    if (m_journal.trace) 
        m_journal.trace << "Acquiring ledger " << mHash;
}
...
void InboundLedger::init (ScopedLockType& collectionLock)
{
    ...
    {
        if (m_journal.debug) 
            m_journal.debug << "Acquiring ledger we already have locally: " << getHash ();
...
bool InboundLedger::tryLocal ()
{
    ...
    if (!mHaveHeader)
    {
        ...
        if (!node)
        {
            ...
            if (m_journal.trace) 
                m_journal.trace << "Ledger header found in fetch pack";

            mLedger = std::make_shared<Ledger> (data, true);
            getApp().getNodeStore ().store (hotLEDGER, std::move (data), mHash);
        }
        else
        {
            mLedger = std::make_shared<Ledger> (strCopy (node->getData ()), true);
        }
        ...
        if (mLedger->getHash () != mHash)
        {
            ...
            if (m_journal.warning) 
                m_journal.warning << mHash << " cannot be a ledger";
    ...
    if (!mHaveTransactions)
    {
        ...
        {
            if (m_journal.trace)
                m_journal.trace << "No TXNs to fetch";
        ...
        else
        {
                ...
                mLedger->getTransHash (), &filter))
            {
                ...
                {
                    if (m_journal.trace)
                        m_journal.trace << "Had full txn map locally";
    ...
    if (!mHaveState)
    {
        ...
        {
            if (m_journal.fatal) 
                m_journal.fatal << "We are acquiring a ledger with a zero account hash";
        ...
        else
        {
                ...
                mLedger->getAccountHash (), &filter))
            {
                ...
                {
                    if (m_journal.trace) 
                        m_journal.trace << "Had full AS map locally";
    ...
    {
        if (m_journal.debug) 
            m_journal.debug << "Had everything locally";
...
void InboundLedger::onTimer (bool wasProgress, ScopedLockType&)
{
    ...
    {
        if (m_journal.info) 
            m_journal.info << "Already done " << mHash;
    ...
    if (getTimeouts () > ledgerTimeoutRetriesMax)
    {
        ...
        {
            if (m_journal.debug) 
                m_journal.debug << getTimeouts() << " timeouts for ledger " << mSeq;
        }
        else
        {
            if (m_journal.debug) 
                m_journal.debug << getTimeouts() << " timeouts for ledger " << mHash;
        }
    ...
    if (!wasProgress)
    {
        ...
        WriteLog (lsDEBUG, InboundLedger) << "No progress(" 
                                          << pc 
                                          << ") for ledger " 
                                          << mHash;
...
void InboundLedger::done ()
{
    ...
    if (m_journal.trace) 
        m_journal.trace << "Done acquiring ledger " << mHash;
...
void InboundLedger::trigger (Peer::ptr const& peer)
{
    ...
    {
        if (m_journal.debug) 
            m_journal.debug << "Trigger on ledger: " 
                            << mHash << (mAborted ? " aborted" : "") 
                            << (mComplete ? " completed" : "") 
                            << (mFailed ? " failed" : "");
        return;
    }
    ...
    {
        if (peer)
            m_journal.trace << "Trigger acquiring ledger " << mHash << " from " << peer;
        else
            m_journal.trace << "Trigger acquiring ledger " << mHash;

        if (mComplete || mFailed)
            m_journal.trace << "complete=" << mComplete << " failed=" << mFailed;
        else
            m_journal.trace <<
    ...
    if (!mHaveHeader)
    {
        ...
        {
            if (m_journal.warning) 
                m_journal.warning << " failed local for " << mHash;
    ...
    { // Be more aggressive if we've timed out at least once
            ...
            getTimeouts () > ledgerBecomeAggressiveThreshold))
        {
            ...
            if (!need.empty ())
            {
                ...
                if (m_journal.info)
                    m_journal.info << "Attempting by hash fetch for ledger " << mHash;
            }
            else
            {
                if (m_journal.info) 
                    m_journal.info << "getNeededHashes says acquire is complete";
    ...
    if (!mHaveHeader && !mFailed)
    {
        ...
        if (m_journal.trace) 
            m_journal.trace << "Sending header request to " << (peer ? "selected peer" : "all peers");
    ...
    if (mHaveHeader && !mHaveState && !mFailed)
    {
        ...
        else if (mLedger->peekAccountStateMap ()->getHash ().isZero ())
        {
            ...
            *tmGL.add_nodeids () = SHAMapNodeID ().getRawString ();
            ...
            if (m_journal.trace) 
                m_journal.trace << "Sending AS root request to " << (peer ? "selected peer" : "all peers");
            ...
            if (!mFailed && !mComplete && !mHaveState)
            {
                ...
                else
                {
                    ...
                    if (!nodeIDs.empty ())
                    {
                        ...
                        if (m_journal.trace) 
                            m_journal.trace << "Sending AS node "
                                            << nodeIDs.size () 
                                            << " request to " 
                    ...
                    {
                        if (m_journal.trace) 
                            m_journal.trace << "All AS nodes filtered";
                    }
                }
            }
        }
    ...
    if (mHaveHeader && !mHaveTransactions && !mFailed)
    {
        ...
        else if (mLedger->peekTransactionMap ()->getHash ().isZero ())
        {
            ...
            * (tmGL.add_nodeids ()) = SHAMapNodeID ().getRawString ();
            ...
            if (m_journal.trace) 
                m_journal.trace << "Sending TX root request to " << (peer ? "selected peer" : "all peers");
    ...
    std::vector<uint256>& nodeHashes, int max, bool aggressive)
{
    ...
    if (dupCount == nodeIDs.size ())
    {
        ...
        if (!aggressive)
        {
            ...
            if (m_journal.trace) 
                m_journal.trace << "filterNodes: all are duplicates";
    ...
    else if (dupCount > 0)
    {
        ...
        if (m_journal.trace) 
            m_journal.trace << "filterNodes " << nodeIDs.size () << " to " << insertPoint;
...
bool InboundLedger::takeHeader (std::string const& data)
{
    ...
    if (m_journal.trace) 
        m_journal.trace << "got header acquiring ledger " << mHash;
    ...
    {
        if (m_journal.warning)
            m_journal.warning << "Acquire hash mismatch";

        if (m_journal.warning) 
            m_journal.warning << mLedger->getHash () << "!=" << mHash;
    ...
    const std::vector< Blob >& data, SHAMapAddNode& san)
{
    ...
    {
        if (m_journal.warning)
            m_journal.warning << "TX node without header";
    ...
    while (nodeIDit != nodeIDs.cend ())
    {
        if (nodeIDit->isRoot ())
        {
            san += mLedger->peekTransactionMap ()->addRootNode (mLedger->getTransHash (), *nodeDatait, snfWIRE, &tFilter);
            if (!san.isGood())
                return false;
        }
        else
...
{
    if (m_journal.trace) 
        m_journal.trace << "got ASdata (" << nodeIDs.size () << ") acquiring ledger " << mHash;

    if (nodeIDs.size () == 1 && m_journal.trace) 
        m_journal.trace << "got AS node: " << nodeIDs.front ();
    ...
    {
        if (m_journal.warning)
            m_journal.warning << "Don't have ledger header";
    ...
    while (nodeIDit != nodeIDs.cend ())
    {
        if (nodeIDit->isRoot ())
        {
            san += mLedger->peekAccountStateMap ()->addRootNode (mLedger->getAccountHash (), *nodeDatait, snfWIRE, &tFilter);
            if (!san.isGood ())
            {
                if (m_journal.warning) 
                    m_journal.warning << "Bad ledger header";
        ...
        else
        {
            ...
            {
                if (m_journal.warning) 
                    m_journal.warning << "Unable to add AS node";
...
*/
//  TODO Why isn't the shared_ptr passed by const& ?
...
*/
//  NOTE, it is not necessary to pass the entire Peer,
//              we can get away with just a Resource::Consumer endpoint.
//
//        TODO Change peer to Consumer
//
int InboundLedger::processData (std::shared_ptr<Peer> peer, protocol::TMLedgerData& packet)
{
    ScopedLockType sl (mLock);
    ...
    if (packet.type () == protocol::liBASE)
    {
        ...
        {
            if (m_journal.warning) 
                m_journal.warning << "Got empty header data";
        ...
        if (!mHaveHeader)
        {
            ...
            {
                if (m_journal.warning) 
                    m_journal.warning << "Got invalid header data";
