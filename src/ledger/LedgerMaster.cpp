...
#include <common/base/RangeSet.h>
...
#include <consensus/validators/Manager.h>
...
class LedgerMasterImp : public LedgerMaster
{
public:
    ...
    LedgerMasterImp (Config const& config, Stoppable& parent,
        ...
        , mLedgerCleaner (make_LedgerCleaner (*this, deprecatedLogs().journal("LedgerCleaner")))
        , mMinValidations (0)
        , mLastValidateSeq (0)
    ...
    void setValidLedger(Ledger::ref l)
    {
        ...
        {
            times = getApp().getValidations().getValidationTimes(l->getHash());
        }
        ...
        if (! times.empty () && times.size() >= mMinValidations)
        {
            ...
            signTime = (times[times.size() / 2] + times[(times.size() - 1) / 2]) / 2;
        }
        else
        {
    ...
    */
    void applyHeldTransactions ()
    {
        ...
        //  TODO recreate the CanonicalTxSet object instead of resetting it
    ...
    */
        ...
        for (std::uint32_t lSeq = ledger->getLedgerSeq () - 1; lSeq > 0; --lSeq)
            if (haveLedger (lSeq))
            {
                ...
                if (hash.isNonZero ())
                {
                    ...
                    {
                        // we closed the seam
                        CondLog (invalidate != 0, lsWARNING, LedgerMaster) << "Match at " 
                                                                           << lSeq << ", " 
                                                                           << invalidate 
    ...
    void checkAccept (uint256 const& hash, std::uint32_t seq)
    {
        ...
        if (!ledger)
        {
            if ((seq != 0) && (getValidLedgerIndex() == 0))
            {
                ...
                if (getApp().getValidations().getTrustedValidationCount (hash) >= mMinValidations)
                {
            ...
            // trusted validation
            ledger = getApp().getInboundLedgers().acquire(hash, 0, InboundLedger::fcGENERIC);
        }
    ...
    void checkAccept (Ledger::ref ledger)
    {
        ...
        WriteLog (lsINFO, LedgerMaster) << "Advancing accepted ledger to "
                                        << ledger->getLedgerSeq()
                                        << " with >= " 
                                        << minVal
                                        << " validations";
    ...
    void consensusBuilt (Ledger::ref ledger) override
    {
        ...
        if (ledger->getLedgerSeq() <= mValidLedgerSeq)
        {
            WriteLog (lsINFO,  LedgerConsensus) << "Consensus built old ledger: "
                                                << ledger->getLedgerSeq() 
                                                << " <= " << mValidLedgerSeq;
            return;
        }
        ...
        if (ledger->getLedgerSeq() <= mValidLedgerSeq)
        {
            WriteLog (lsDEBUG, LedgerConsensus) << "Consensus ledger fully validated";
        ...
        {
            WriteLog (lsDEBUG, LedgerConsensus) << "Consensus triggered check of ledger";
    ...
    void advanceThread()
    {
		...
		catch (std::exception const& e)
        {
			...
			WriteLog (lsFATAL, LedgerMaster) << e.what();
        }
    ...
    void doAdvance ()
    {
        do
        {
                                ...
                                if (ledger)
                                {
                                    ...
                                    { // Previous ledger is in DB
                                        ...
                                        getApp().getJobQueue().addJob(jtADVANCE, 
                                                                     "tryFill", std::bind (&LedgerMasterImp::tryFill, 
                                                                                           this,
                                ...
                                else
                                {
                                    try
                                    {
                                        for (int i = 0; i < ledger_fetch_size_; ++i)
                                        {
                                            ...
                                            if (hash.isNonZero())
                                                getApp().getInboundLedgers().acquire(hash, seq, InboundLedger::fcHISTORY);
                                        }
                                    }
                                    catch (...)
                                    {
            ...
            {
                WriteLog (lsTRACE, LedgerMaster) << "tryAdvance found " 
                                                 << pubLedgers.size() 
                                                 << " ledgers to publish";
    ...
    std::vector<Ledger::pointer> findNewLedgersToPublish ()
    {
        ...
        {
            WriteLog (lsWARNING, LedgerMaster) << "Gap in validated ledger stream "
                                               << mPubLedgerSeq 
                                               << " - " 
                                               << mValidLedgerSeq - 1;
    ...
    void updatePaths (Job& job)
    {
        ...
        while (! job.shouldCancel())
        {
            ...
            catch (SHAMapMissingNode&)
            {
                ...
                getApp().getInboundLedgers().acquire(lastLedger->getHash (), lastLedger->getLedgerSeq (), InboundLedger::fcGENERIC);
            }
        }
    }
    ...
    */
    void newPFWork (const char *name)
    {
        if (mPathFindThread < 2)
        {
            ...
            getApp().getJobQueue().addJob (jtUPDATE_PF, name, std::bind (&LedgerMasterImp::updatePaths, 
                                                                        this,
                                                                        std::placeholders::_1));
        }
    }
    ...
    uint256 walkHashBySeq (std::uint32_t index, Ledger::ref referenceLedger)
    {
        ...
        if (ledgerHash.isZero())
        {
            ...
            if (nonzero)
            {
                ...
                Ledger::pointer ledger = getApp().getInboundLedgers().acquire (refHash, refIndex, InboundLedger::fcGENERIC);
                if (ledger)
                {
                    ledgerHash = ledger->getLedgerHash (index);
                    // TODO when empty ledger
...
make_LedgerMaster (Config const& config, 
