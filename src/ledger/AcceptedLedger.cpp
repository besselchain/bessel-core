...
//  TODO Remove this global and make it a member of the App
//             Use a dependency injection to give AcceptedLedger access.
//
TaggedCache<uint256, AcceptedLedger> AcceptedLedger::s_cache ("AcceptedLedger", 
                                                              4, 
                                                              60,
                                                              get_seconds_clock (),
...
AcceptedLedger::AcceptedLedger (Ledger::ref ledger) : mLedger (ledger)
{
    ...
    for (std::shared_ptr<SHAMapItem> item = txSet.peekFirstItem (); 
         item;
