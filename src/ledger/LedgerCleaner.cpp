...
public:
    ...
    LedgerCleanerImp (Stoppable& stoppable, beast::Journal journal)
    ...
    LedgerHash getLedgerHash(Ledger::pointer ledger, LedgerIndex index)
    {
        ...
        {
            m_journal.warning << "Node missing from ledger " << ledger->getLedgerSeq();
    ...
    */
        ...
        bool doTxns)
    {
            ...
            (dbLedger->getParentHash() != nodeLedger->getParentHash()))
        {
            ...
            m_journal.debug << "Ledger " << ledgerIndex << " mismatches SQL DB";
        ...
        if (doNodes && !nodeLedger->walkLedger())
        {
            ...
            getApp().getInboundLedgers().acquire(ledgerHash, ledgerIndex, InboundLedger::fcGENERIC);
        ...
        if (referenceLedger->getLedgerSeq() >= ledgerIndex)
        {
            ...
            if (ledgerHash.isZero())
            {
                ...
                if (nonzero)
                {
                    ...
                    referenceLedger = getApp().getInboundLedgers().acquire(refHash, refIndex, InboundLedger::fcGENERIC);
                    if (referenceLedger)
                        ledgerHash = getLedgerHash(referenceLedger, ledgerIndex);
                }
            }
