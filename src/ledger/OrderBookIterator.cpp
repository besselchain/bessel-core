...
{
    mBase = bessel::getBookBase({{currencyIn, issuerIn},
...
bool BookDirIterator::nextDirectory (LedgerEntrySet& les)
{
    ...
    WriteLog (lsTRACE, Ledger) << "BookDirectoryIterator:: index " << to_string (mIndex);
...
{
    WriteLog (lsTRACE, Ledger) << "BookDirIterator(" << to_string (mBase) << ") firstDirectory";
