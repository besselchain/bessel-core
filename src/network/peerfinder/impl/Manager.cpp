...
#include <network/peerfinder/impl/Checker.h>
#include <network/peerfinder/impl/Logic.h>
#include <network/peerfinder/impl/SourceStrings.h>
#include <network/peerfinder/impl/StoreSqdb.h>
...
public:
    ...
    void addFixedPeer (std::string const& name,
                       std::vector<boost::asio::ip::tcp::endpoint> const& addresses) override
    {
    ...
    Slot::ptr
    new_inbound_slot (boost::asio::ip::tcp::endpoint const& local_endpoint,
                      boost::asio::ip::tcp::endpoint const& remote_endpoint) override
    {
        return m_logic.new_inbound_slot (local_endpoint, remote_endpoint);
    }

    Slot::ptr
    new_outbound_slot (boost::asio::ip::tcp::endpoint const& remote_endpoint) override
    {
    ...
    bool
    onConnected (Slot::ptr const& slot, boost::asio::ip::tcp::endpoint const& local_endpoint) override
    {
        SlotImp::ptr impl (std::dynamic_pointer_cast <SlotImp> (slot));
    ...
    activate (Slot::ptr const& slot,
              BesselPublicKey const& key, 
              bool cluster) override
    {
        SlotImp::ptr impl (std::dynamic_pointer_cast <SlotImp> (slot));
...
make_Manager (beast::Stoppable& parent, 
              ...
              BasicConfig const& config)
{
    return std::make_unique<ManagerImp> (parent, io_service, clock, journal, config);
}
