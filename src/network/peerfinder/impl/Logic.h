...
#ifndef BESSEL_PEERFINDER_LOGIC_H_INCLUDED
#define BESSEL_PEERFINDER_LOGIC_H_INCLUDED

#include <network/peerfinder/Manager.h>
#include <network/peerfinder/impl/Bootcache.h>
#include <network/peerfinder/impl/Counts.h>
#include <network/peerfinder/impl/Fixed.h>
#include <network/peerfinder/impl/iosformat.h>
#include <network/peerfinder/impl/Handouts.h>
#include <network/peerfinder/impl/Livecache.h>
#include <network/peerfinder/impl/Reporting.h>
...
public:
    ...
    //
    typedef std::map <boost::asio::ip::tcp::endpoint, std::shared_ptr<SlotImp>> Slots;

    typedef std::map <boost::asio::ip::tcp::endpoint, Fixed> FixedSlots;

    // A set of unique Bessel public keys
    typedef std::set <BesselPublicKey> Keys;

    // A set of non-unique IPAddresses without ports, used
    // to filter duplicates when making outgoing connections.
    typedef std::multiset <boost::asio::ip::tcp::endpoint> ConnectedAddresses;
    ...
    struct State
    {
        State (
            ...
            : stopping (false)
            , counts ()
            , livecache (clock, beast::Journal (journal, Reporting::livecache))
            , bootcache (*store, clock, beast::Journal (journal, Reporting::bootcache))
        {
        }
    ...
    ConnectHandouts::SquelchesSet m_squelches;
    ...
    */
    ...
    void
    addFixedPeer (std::string const& name, boost::asio::ip::tcp::endpoint const& ep)
    {
    ...
    void
    addFixedPeer (std::string const& name, std::vector<boost::asio::ip::tcp::endpoint> const& addresses)
    {
        typename SharedState::Access state (m_state);
        ...
        {
            if (m_journal.info) 
                m_journal.info << "Could not resolve fixed slot '" << name << "'";
        ...
        for (auto const& remote_address : addresses)
        {
            ...
            {
                throw std::runtime_error ("Port not specified for address:" + remote_address.address().to_string ());
            }
            ...
            {
                if (m_journal.debug)
                    m_journal.debug << bessel::leftw (18) 
                                    << "Logic add fixed '"
                                    << name
                                    << "' at " 
                                    << convert_endpoint_to_string(remote_address);
    ...
    void checkComplete (boost::asio::ip::tcp::endpoint const& remoteAddress,
                        boost::asio::ip::tcp::endpoint const& checkedAddress,
                        boost::system::error_code ec)
    {
        ...
        if (iter == state->slots.end())
        {
            ...
            if (m_journal.debug) 
                m_journal.debug << bessel::leftw (18) 
                                << "Logic tested " 
                                << convert_endpoint_to_string(remoteAddress) 
        ...
        if (ec)
        {
            //  TODO Should we retry depending on the error?
            slot.canAccept = false;
            if (m_journal.error) 
                m_journal.error << bessel::leftw (18) 
                                << "Logic testing " 
                                << convert_endpoint_to_string(iter->first)
                                << " with error, " 
                                << ec.message();
        ...
        if (m_journal.debug) 
            m_journal.debug << bessel::leftw (18) 
                            << "Logic testing " 
                            << convert_endpoint_to_string(checkedAddress)
                            << " succeeded";
    }

    //--------------------------------------------------------------------------

    SlotImp::ptr new_inbound_slot (boost::asio::ip::tcp::endpoint const& local_endpoint,
                                   boost::asio::ip::tcp::endpoint const& remote_endpoint)
    {
        if (m_journal.debug)
            m_journal.debug << bessel::leftw (18)
                            << "Logic accept" 
                            << convert_endpoint_to_string(remote_endpoint)
                            << " on local " 
                            << convert_endpoint_to_string(local_endpoint);
        ...
        /*
        {
            ...
            if (iter != state->slots.end ())
            {
                ...
                //assert (consistent);
                if (m_journal.warning) m_journal.warning << bessel::leftw (18) <<
        ...
        */
        ...
        SlotImp::ptr const slot (std::make_shared <SlotImp> (local_endpoint,
                                                             remote_endpoint, 
                                                             fixed (remote_endpoint, state),
                                                             m_clock));
        // Add slot to table
        auto const result (state->slots.emplace (slot->remote_endpoint (), slot));

        // Remote address must not already exist
        assert (result.second);

        // Add to the connected address list
        boost::asio::ip::tcp::endpoint e_endpoint;
        e_endpoint.address(remote_endpoint.address());
        e_endpoint.port(0);
        state->connected_addresses.emplace(e_endpoint);// (remote_endpoint.port (0));
    ...
    SlotImp::ptr
    new_outbound_slot (boost::asio::ip::tcp::endpoint const& remote_endpoint)
    {
        if (m_journal.debug) 
            m_journal.debug << bessel::leftw (18) 
                            << "Logic connect " 
        ...
        // Check for duplicate connection
        if (state->slots.find (remote_endpoint) != state->slots.end ())
        {
            if (m_journal.debug) 
                m_journal.debug << bessel::leftw (18) 
                                << "Logic dropping " 
        ...
        // Create the slot
        SlotImp::ptr const slot (std::make_shared <SlotImp> (remote_endpoint, fixed (remote_endpoint, state), m_clock));

        // Add slot to table
        std::pair <Slots::iterator, bool> result (state->slots.emplace (slot->remote_endpoint (), slot));
    ...
    bool
    onConnected (SlotImp::ptr const& slot, boost::asio::ip::tcp::endpoint const& local_endpoint)
    {
        if (m_journal.trace) 
            m_journal.trace << bessel::leftw (18) 
                            << "Logic connected"
                            << convert_endpoint_to_string(slot->remote_endpoint ())
                            << " on local " 
                            << convert_endpoint_to_string(local_endpoint);
        ...
        assert (state->slots.find (slot->remote_endpoint ()) != state->slots.end ());
        ...
        // Check for self-connect by address
        {
            ...
            if (iter != state->slots.end ())
            {
                assert (iter->second->local_endpoint () == slot->remote_endpoint ());

                if (m_journal.warning) 
                    m_journal.warning << bessel::leftw (18) 
                                      << "Logic dropping "
                                      << convert_endpoint_to_string(slot->remote_endpoint ())
                                      << " as self connect";
    ...
    Result
    activate (SlotImp::ptr const& slot, BesselPublicKey const& key, bool cluster)
    {
        if (m_journal.debug)
            m_journal.debug << bessel::leftw (18) 
                            << "Logic handshake " 
        ...
        assert (slot->state() == Slot::accept || slot->state() == Slot::connected);
        ...
        std::pair<Keys::iterator, bool> const result (state->keys.insert (key));
    ...
    */
        ...
        if (m_whenBroadcast <= now)
        {
            ...
            */
                ...
                state->counts.inboundSlots() > 0)
            {
                ...
                endpoint.port(state->config.listeningPort);
                ep.address = endpoint;
    ...
    void once_per_second()
    {
        ...
        ConnectHandouts::SquelchesSet::expire(m_squelches, Tuning::recentAttemptDuration);
    ...
    void preprocess (SlotImp::ptr const& slot, 
                    Endpoints& list,
        ...
        for (auto iter (list.begin()); iter != list.end();)
        {
            ...
            if (ep.hops > Tuning::maxHops)
            {
                if (m_journal.debug)
                    m_journal.debug << bessel::leftw (18) 
                                    << "Endpoints drop "
                                    << convert_endpoint_to_string(ep.address)
                                    << " for excess hops " 
                                    << ep.hops;
            ...
            if (ep.hops == 0)
            {
                if (! neighbor)
                {
                    ...
                    neighbor = true;
                    boost::asio::ip::tcp::endpoint endpoint = slot->remote_endpoint();
                    endpoint.port(ep.address.port());
                    ep.address = endpoint;//slot->remote_endpoint().port(ep.address.port ());
                }
                else
                {
                    if (m_journal.debug) 
    ...
    void on_endpoints (SlotImp::ptr const& slot, Endpoints list)
    {
        ...
        for (auto const& ep : list)
        {
            ...
            if (ep.hops == 1)
            {
                ...
                {
                    if (m_journal.debug) m_journal.debug << bessel::leftw (18) 
                                                         << "Logic testing " 
                                                         << convert_endpoint_to_string(ep.address)
                                                         << " already in progress";
                    continue;
                ...
                if (! slot->checked)
                {
                    ...
                    //
                    m_checker.async_connect (ep.address, 
                                            std::bind (&Logic::checkComplete, 
                                                        this, 
    ...
    void remove (SlotImp::ptr const& slot, typename SharedState::Access& state)
    {
        ...
        // Remove from connected address table
        {
            boost::asio::ip::tcp::endpoint endpoint = slot->remote_endpoint();
            endpoint.port(0);
            auto const iter (state->connected_addresses.find(endpoint));// (slot->remote_endpoint().port (0)));
            // Address must exist
    ...
    void on_closed (SlotImp::ptr const& slot)
    {
        ...
        case Slot::connected:
            ...
            //  TODO If the address exists in the ephemeral/live
    ...
    bool fixed (boost::asio::ip::tcp::endpoint const& endpoint, typename SharedState::Access& state) const
    {
    ...
    // Returns `true` if the address matches a fixed slot address
    // Note that this does not use the port information in the boost::asio::ip::tcp::Endpoint
    bool fixed (boost::asio::ip::address const& address, typename SharedState::Access& state) const
    {
        for (auto const& entry : state->fixed)
    ...
    void get_fixed (std::size_t needed, 
                    Container& c,
                    typename ConnectHandouts::SquelchesSet& squelches,
                    typename SharedState::Access& state)
    {
        auto const now (m_clock.now());
        for (auto iter = state->fixed.begin ();
             needed && iter != state->fixed.end ();
             ++iter)
        {
            auto const& address (iter->first.address());

            ConnectHandouts::IPAddress ipaddress;
            ipaddress.ep_ = iter->first;
            if (iter->second.when() <= now && 
                squelches.find(ipaddress) == squelches.end() && 
                std::none_of (
    ...
    void squelch_slots (typename SharedState::Access& state)
    {
        ...
        {
            ConnectHandouts::IPAddress ipaddress;
            ipaddress.ep_ = s.second->remote_endpoint();
            auto const result (m_squelches.insert (ipaddress));

            if (! result)
                m_squelches.touch (ipaddress);
        }
    }
    ...
    //
    bool addBootcacheAddress (boost::asio::ip::tcp::endpoint const& address,
                              typename SharedState::Access& state)
    {
    ...
    void fetch (std::shared_ptr<Source> const& source)
    {
        ...
        if (! results.error)
        {
            ...
            if (m_journal.info) 
                m_journal.info << bessel::leftw (18) 
                               << "Logic added " 
                               << count 
                               << " new "
    ...
    // Returns true if the boost::asio::ip::tcp::endpoint contains no invalid data.
    bool is_valid_address (boost::asio::ip::tcp::endpoint const& address)
    {
        if (address.address().is_unspecified ())
            return false;

        if (! is_public (address.address()))
            return false;
    ...
    void writeSlots (beast::PropertyStream::Set& set, Slots const& slots)
    {
        for (auto const& entry : slots)
        {
            ...
            if (slot.local_endpoint () != boost::none)
                item ["local_address"] = convert_endpoint_to_string(*slot.local_endpoint ());

            item ["remote_address"]   = convert_endpoint_to_string (slot.remote_endpoint ());
    ...
    void onWrite (beast::PropertyStream::Map& map)
    {
        ...
        //  NOTE These ugly casts are needed because
...
template <class FwdIter>
void
Logic<Checker>::onRedirects (FwdIter first, 
                             FwdIter last,
                             boost::asio::ip::tcp::endpoint const& remote_address)
{
    typename SharedState::Access state (m_state);
