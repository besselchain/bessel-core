...
#ifndef BESSEL_PEERFINDER_CHECKER_H_INCLUDED
#define BESSEL_PEERFINDER_CHECKER_H_INCLUDED
...
public:
    ...
    */
    template <class Handler>
    void
    async_connect (boost::asio::ip::tcp::endpoint const& endpoint, Handler&& handler);
...
template <class Handler>
void
Checker<Protocol>::async_connect (boost::asio::ip::tcp::endpoint const& endpoint, Handler&& handler)
{
    auto const op = std::make_shared<async_op<Handler>> (*this, io_service_, std::forward<Handler>(handler));
    {
        std::lock_guard<std::mutex> lock (mutex_);
        list_.push_back (*op);
    }

    op->socket_.async_connect (endpoint, std::bind (&basic_async_op::operator(), op, std::placeholders::_1));
}
