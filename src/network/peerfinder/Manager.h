...
#ifndef BESSEL_PEERFINDER_MANAGER_H_INCLUDED
#define BESSEL_PEERFINDER_MANAGER_H_INCLUDED
...
typedef std::vector<boost::asio::ip::tcp::endpoint> IPAddresses;
...
public:
    ...
    */
    virtual void addFixedPeer (std::string const& name,
                               std::vector<boost::asio::ip::tcp::endpoint> const& addresses) = 0;

    /** Add a set of strings as fallback boost::asio::ip::tcp::endpoint sources.
        @param name A label used for diagnostics.
    */
    virtual void addFallbackStrings (std::string const& name,
                                     std::vector<std::string> const& strings) = 0;

    /** Add a URL as a fallback location to obtain boost::asio::ip::tcp::endpoint sources.
        @param name A label used for diagnostics.
    */
    /*  NOTE Unimplemented
    ...
    */
    virtual Slot::ptr new_inbound_slot (
            ...
            boost::asio::ip::tcp::endpoint const& remote_endpoint) = 0;

    /** Create a new outbound slot with the specified remote endpoint.
        If nullptr is returned, then the slot could not be assigned.
        Usually this is because of a duplicate connection.
    */
    virtual Slot::ptr new_outbound_slot (boost::asio::ip::tcp::endpoint const& remote_endpoint) = 0;
    ...
    */
    ...
    onConnected (Slot::ptr const& slot,
                 boost::asio::ip::tcp::endpoint const& local_endpoint) = 0;

    /** Request an active slot type. */
    virtual
    Result
    activate (Slot::ptr const& slot,
              BesselPublicKey const& key, bool cluster) = 0;
