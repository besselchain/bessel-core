...
//  TODO Replace macros with language constructs
    ...
    std::vector<std::array<boost::optional<std::string>, I>>& columns)
{
    ...
    std::vector<boost::optional<std::string>> blobs;
    blobs.reserve (I);
    for (int i = 0; i < I; ++i)
       blobs.emplace_back ("");
    ...
    std::vector<std::tuple<boost::optional<std::string>, boost::optional<TOther>>>& columns)
{
    ...
    boost::optional<std::string> blob;
    soci::indicator ind;
...
//  TODO move all function definitions inlined into the class.
...
public:
    ...
    void nodeAddPublic (BesselAddress const& naNodePublic, ValidatorSource vsWhy, std::string const& strComment)
    {
        seedNode    snCurrent;

            //WriteLog (lsINFO, UniqueNodeList) << "check getSeedNodes";
        bool        bFound      = getSeedNodes (naNodePublic, snCurrent);
        bool        bChanged    = false;
            //WriteLog (lsINFO, UniqueNodeList) << "check getSeedNodes done.";
    ...
    // Queue a domain for a single attempt fetch a bessel.txt.
    ...
    void nodeRemovePublic (BesselAddress const& naNodePublic)
    {
        {
    ...
    bool nodeInUNL (BesselAddress const& naNodePublic)
    {
        ScopedUNLLockType sl (mUNLLock);
    ...
    bool nodeInCluster (BesselAddress const& naNodePublic)
    {
        ScopedUNLLockType sl (mUNLLock);
    ...
    bool nodeInCluster (BesselAddress const& naNodePublic, std::string& name)
    {
        ScopedUNLLockType sl (mUNLLock);
        std::map<BesselAddress, ClusterNodeStatus>::iterator it = m_clusterNodes.find (naNodePublic);
    ...
    bool nodeUpdate (BesselAddress const& naNodePublic, ClusterNodeStatus const& cnsStatus)
    {
        ScopedUNLLockType sl (mUNLLock);
    ...
    std::map<BesselAddress, ClusterNodeStatus> getClusterStatus ()
    {
        std::map<BesselAddress, ClusterNodeStatus> ret;
        {
            ScopedUNLLockType sl (mUNLLock);
            ret = m_clusterNodes;
        }
    ...
    std::uint32_t getClusterFee ()
    {
        ...
        std::vector<std::uint32_t> fees;
        {
            ScopedUNLLockType sl (mUNLLock);
            {
                for (std::map<BesselAddress, ClusterNodeStatus>::iterator it = m_clusterNodes.begin(),
    ...
    void nodeBootstrap ()
    {
        ...
        if (!bLoaded && getConfig ().VALIDATORS_FILE.empty ())
        {
            ...
            //WriteLog (lsINFO, UniqueNodeList) << "Bootstrapping UNL: loading done.";
        }

        // Always load from besseld.cfg
        if (!getConfig ().validators.empty ())
        {
            BesselAddress   naInvalid;  // Don't want a referrer on added entries.

            WriteLog (lsINFO, UniqueNodeList) << boost::str (boost::format ("Bootstrapping UNL: loading from 2 '%s'. ")
        ...
        if (!bLoaded)
        {
            WriteLog (lsINFO, UniqueNodeList) << boost::str (boost::format ("Bootstrapping UNL: loading from 3 '%s'.")
...
private:
    ...
    bool miscLoad ()
    {
        ...
        *db << "SELECT ScoreUpdated, FetchUpdated FROM Misc WHERE Magic=1;",
		...
		mtpFetchUpdated = boost::posix_time::from_time_t(fuO.value_or(-1));
		mtpScoreUpdated = boost::posix_time::from_time_t(suO.value_or(-1));
    ...
    bool miscSave ()
    {
		...
		std::string strtmp = str (boost::format ("REPLACE INTO Misc (Magic,FetchUpdated,ScoreUpdated) VALUES (1,%d,%d);")
                    % iToSeconds (mtpFetchUpdated)
                    % iToSeconds (mtpScoreUpdated)
					//% to_simple_string(mtpFetchUpdated)
					//% to_simple_string(mtpScoreUpdated)
					);
    ...
    void trustedLoad ()
    {
        ...
        for (auto const& c : getConfig ().CLUSTER_NODES)
        {
            ...
            {
                BesselAddress a = BesselAddress::createNodePublic (match[1]);
    ...
    //
    //  TODO Shrink this function, break it up
    //
    void scoreCompute ()
    {
    ...
    // Given a bessel.txt, process it.
    //
    //  TODO Can't we take a filename or stream instead of a string?
    //
    ...
    // Try to process the next fetch of a bessel.txt.
    void fetchNext ()
    {
        bool    bFull;
        ...
        if (!bFull)
        {
            ...
            {
                //soci::blob b(*db);
				boost::optional<std::string> strtmp;
				//std::string strtmp;
                soci::indicator ind;
                boost::optional<int> nO;

                *db << "SELECT Domain,Next FROM SeedDomains ORDER BY Next LIMIT 1;",
                // TODO index
                // *db << "SELECT Domain,Next FROM SeedDomains Force INDEX(SeedDomainNext) ORDER BY Next LIMIT 1;",
                        //soci::into(b, ind),
						soci::into(strtmp, ind),
						soci::into(nO);
                if (nO)
                {
                    int iNext (*nO);
    ...
    // Get the bessel.txt and process it.
    ...
    void getValidatorsUrl (BesselAddress const& naNodePublic,
    ...
    void getIpsUrl (BesselAddress const& naNodePublic, IniFileSections secSite)
    {
    ...
    bool responseIps (std::string const& strSite, BesselAddress const& naNodePublic, const boost::system::error_code& err, int iStatus, std::string const& strIpsFile)
    {
        bool    bReject = !err && iStatus != 200;
    ...
    // After fetching a bessel.txt from a web site, given a IniFileSections with validators, parse and persist it.
    bool responseValidators (std::string const& strValidatorsUrl, BesselAddress const& naNodePublic, IniFileSections secSite, std::string const& strSite, const boost::system::error_code& err, int iStatus, std::string const& strValidatorsFile)
    {
        bool    bReject = !err && iStatus != 200;
    ...
    void processIps (std::string const& strSite, BesselAddress const& naNodePublic, IniFileSections::mapped_type* pmtVecStrIps)
    {
    ...
    int processValidators (std::string const& strSite, std::string const& strValidatorsSrc, BesselAddress const& naNodePublic, ValidatorSource vsWhy, IniFileSections::mapped_type* pmtVecStrValidators)
    {
    ...
    // Process a bessel.txt.
    void processFile (std::string const& strDomain, BesselAddress const& naNodePublic, IniFileSections secSite)
    {
        //
        // Process Validators
        //
    ...
    bool getSeedDomains (std::string const& strDomain, seedDomain& dstSeedDomain)
    {
        ...
        std::string strSql = boost::str (
            boost::format (
                "SELECT Domain, PublicKey, Source, Next, Scan, FetchTime, Sha256, "
        ...
        // column of type DATETIME into a boost::tuple can throw when the
        // datetime column is invalid (even if the value as int is valid).
		boost::optional<std::string>                       domainBlob;
        soci::indicator                  di;
        boost::optional<std::string>     strPublicKey;
		boost::optional<std::string>                      sourceBlob;
        soci::indicator                  si;
		...
		boost::optional<std::string>                       commentBlob;
        soci::indicator                  ci;
        ...
        while (st.fetch ())
        {
            ...
            if (soci::i_ok == di)
			    ...
			    dstSeedDomain.strDomain = *domainBlob;
			...
			if (soci::i_ok == ci)
				dstSeedDomain.strComment = *commentBlob;
    ...
    void setSeedDomains (const seedDomain& sdSource, bool bNext)
    {
        ...
        std::string strSql  = boost::str (boost::format ("REPLACE INTO SeedDomains (Domain,PublicKey,Source,Next,Scan,FetchTime,Sha256,Comment) VALUES (%s, %s, %s, %d, %d, %d, '%s', %s);")
    ...
    bool getSeedNodes (BesselAddress const& naNodePublic, seedNode& dstSeedNode)
    {
        std::string strSql =
                str (boost::format (
                    "SELECT PublicKey, Source, Next, Scan, FetchTime, Sha256, "
        ...
        *db << strSql,
        ...
        if (soci::i_ok == ci)
			dstSeedNode.strComment = *commentBlob;
    ...
    void setSeedNodes (const seedNode& snSource, bool bNext)
    {
        ...
        std::string strSql  = str (boost::format ("REPLACE INTO SeedNodes (PublicKey,Source,Next,Scan,FetchTime,Sha256,Comment) VALUES ('%s', '%c', %d, %d, %d, '%s', %s);")
    ...
    //
    //  TODO Can't we name this processValidatorList?
    //
...
private:
    ...
    std::map<BesselAddress, ClusterNodeStatus> m_clusterNodes;
