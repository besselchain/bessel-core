...
ConnectAttempt::ConnectAttempt (boost::asio::io_service& io_service
    ...
    , strand_ (io_service)
    , timer_ (io_service)
    , ssl_bundle_ (std::make_unique<sslbundle>(context, io_service))
...
{
    if (journal_.debug) 
        journal_.debug << "Connect " << remote_endpoint;
}
...
ConnectAttempt::stop()
{
    if (! strand_.running_in_this_thread())
        return strand_.post(std::bind(&ConnectAttempt::stop, shared_from_this()));
...
ConnectAttempt::setTimer()
{
    ...
    timer_.async_wait(strand_.wrap(std::bind(&ConnectAttempt::onTimer
                                        , shared_from_this()
                                        , std::placeholders::_1)));
}
...
ConnectAttempt::onHandshake (error_code ec)
{
    ...
    if(journal_.trace)
        journal_.trace << "onHandshake";

    if (! overlay_.peerFinder ().onConnected (slot_, local_endpoint))
        return fail("Duplicate connection");

    bool success;
    uint256 sharedValue;
    std::tie(sharedValue, success) = makeSharedValue(stream_.native_handle(), journal_);
    if (! success)
        return close(); // makeSharedValue logs

    beast::http::message req = makeRequest(! overlay_.peerFinder ().config ().peerPrivate
...
ConnectAttempt::onWrite (error_code ec, std::size_t bytes_transferred)
{
    ...
    if(journal_.trace)
        journal_.trace << "onWrite: " << bytes_transferred << " bytes";
...
ConnectAttempt::onRead (error_code ec, std::size_t bytes_transferred)
{
    ...
    if(ec == boost::asio::error::eof)
    {
        ...
        return stream_.async_shutdown(strand_.wrap(std::bind(&ConnectAttempt::onShutdown
                                                        , shared_from_this()
                                                        , std::placeholders::_1)));
    }
    ...
    if(journal_.trace)
    {
        if(bytes_transferred > 0) 
            journal_.trace << "onRead: " << bytes_transferred << " bytes";
        else 
            journal_.trace << "onRead";
    }
    ...
    if (! ec)
    {
        ...
        std::tie (ec, bytes_consumed) = parser_.write(write_buf_.data());
        if (! ec)
        {
    ...
    Streambuf const& body)
{
    ...
    {
        if (journal_.info) 
            journal_.info << "HTTP Response: " << m.status () << " " << m.reason ();
    ...
    std::tie(publicKey, success) = verifyHello (hello, sharedValue, journal_, getApp ());
    if(! success)
        return close(); // verifyHello logs

    if(journal_.info)
        journal_.info << "Public Key: " << publicKey.humanNodePublic ();

    auto const protocol = BuildInfo::make_protocol (hello.protoversion ());

    if(journal_.info) 
        journal_.info << "Protocol: " << to_string(protocol);

    std::string name;
    bool const clusterNode = getApp ().getUNL ().nodeInCluster (publicKey, name);
    if (clusterNode)
        if (journal_.info) 
            journal_.info << "Cluster name: " << name;

    auto const result = overlay_.peerFinder().activate (slot_, publicKey.toPublicKey(), clusterNode);
    if (result != PeerFinder::Result::success)
        return fail("Outbound slots full");

    auto const peer = std::make_shared<PeerImp>(std::move(ssl_bundle_)
                                            , read_buf_.data()
                                            , std::move(slot_)
                                            , std::move(response_)
                                            , usage_
