...
//  Shouldn't we have to include the OpenSSL
...
buildHello (uint256 const& sharedValue, Application& app)
{
    ...
    Blob vchSig;
    app.getLocalCredentials ().getNodePrivate ().signNodePrivate (sharedValue, vchSig);
    ...
    h.set_nodepublic (app.getLocalCredentials ().getNodePublic ().humanNodePublic ());
...
appendHello (beast::http::message& m, protocol::TMHello const& hello)
{
    ...
    h.append ("Session-Signature", bessel::base64_encode (hello.nodeproof()));
    ...
    if (hello.has_ledgerclosed())
        h.append ("Closed-Ledger", bessel::base64_encode (hello.ledgerclosed()));

    if (hello.has_ledgerprevious())
        h.append ("Previous-Ledger", bessel::base64_encode (hello.ledgerprevious()));
}

std::vector<ProtocolVersion>
parse_ProtocolVersions (std::string const& s)
{
    static bessel::static_initializer<boost::regex> re (
        "^"                  // start of line
    ...
    auto const list = std::rfc2616::split_commas (s);
    ...
    for (auto const& s : list)
    {
        ...
        int minor;
        std::string tmp(m[1]);
        if (! boost::conversion::try_lexical_convert (major, tmp))
            continue;

        tmp = m[2];
        if (! boost::conversion::try_lexical_convert (minor, tmp))
            continue;
...
parseHello (beast::http::message const& m, beast::Journal journal)
{
...

    {
        ...
        hello.set_nodeproof (bessel::base64_decode (iter->second));
    }

    {
        auto const iter = h.find (m.request() ? "User-Agent" : "Server");
        if (iter != h.end())
...

    {
        ...
        if (iter != h.end())
        {
            ...
            decltype(iter->second) iterSecond = iter->second;
            if (! boost::conversion::try_lexical_convert(nettime, iterSecond))
                return result;
...

    {
        ...
        if (iter != h.end())
        {
            ...
            decltype(iter->second) iterSecond = iter->second;
            if (! boost::conversion::try_lexical_convert(ledgerIndex, iterSecond))
                return result;
...

    {
        ...
        if (iter != h.end())
            hello.set_ledgerclosed (bessel::base64_decode (iter->second));
    }

    {
        auto const iter = h.find ("Previous-Ledger");
        if (iter != h.end())
            hello.set_ledgerprevious (bessel::base64_decode (iter->second));
    }
...
verifyHello (protocol::TMHello const& h
            ...
            , Application& app)
{
    std::pair<BesselAddress, bool> result = { {}, false };
        ...
        ((h.nettime () < minTime) || (h.nettime () > maxTime)))
    {
        ...
        {
            journal.info << "Clock for is off by +" << h.nettime() - ourTime;
        }
        else if (h.nettime () < minTime)
        {
            journal.info << "Clock is off by -" << ourTime - h.nettime();
        }
    }
    else if (h.protoversionmin () > to_packed (BuildInfo::getCurrentProtocol()))
    {
        journal.info << "Hello: Disconnect: Protocol mismatch [" 
                     << "Peer expects "  
                     << to_string (protocol) 
                     << " and we run " 
                     << to_string (BuildInfo::getCurrentProtocol()) 
                     << "]";
    }
    else if (! result.first.setNodePublic (h.nodepublic()))
    {
        journal.info << "Hello: Disconnect: Bad node public key.";
    }
    else if (! result.first.verifyNodePublic (sharedValue, h.nodeproof (), ECDSA::not_strict))
    {
        // Unable to verify they have private key for claimed public key.
        journal.info << "Hello: Disconnect: Failed to verify session.";
    }
    else
    {
