...
#include <network/overlay/impl/PeerImp.h>
...
#include <common/misc/IHashRouter.h>
...
#include <transaction/tx/InboundTransactions.h>
...
PeerImp::PeerImp (id_t id
                , endpoint_type remote_endpoint
                , PeerFinder::Slot::ptr const& slot
                , beast::http::message&& request
                , protocol::TMHello const& hello
                , BesselAddress const& publicKey
                , Resource::Consumer consumer
                , std::unique_ptr<sslbundle>&& ssl_bundle
                , OverlayImpl& overlay)
    : Child (overlay)
    , id_(id)
...
PeerImp::~PeerImp ()
{
    if (cluster())
        if (journal_.warning)
            journal_.warning << name_ << " left cluster";
...
PeerImp::run()
{
    if(! strand_.running_in_this_thread())
        return strand_.post(std::bind (&PeerImp::run, shared_from_this()));
    {
        auto s = getVersion();
        if (boost::starts_with(s, "besseld-"))
        {
            s.erase(s.begin(), s.begin() + 8);

            bessel::SemanticVersion v;
            if (v.parse(s))
            {
                bessel::SemanticVersion av;
                av.parse("0.28.1-b7");
                hopsAware_ = v >= av;
            }
        }
    ...
    else
    {
        ...
        if ((hello_.has_ledgerclosed ()) && (hello_.ledgerclosed ().size () == (256 / 8)))
        {
            memcpy (closedLedgerHash_.begin (), hello_.ledgerclosed ().data (), 256 / 8);

            if ((hello_.has_ledgerprevious ()) &&
                (hello_.ledgerprevious ().size () == (256 / 8)))
            {
                memcpy (previousLedgerHash_.begin (), hello_.ledgerprevious ().data (), 256 / 8);
...
{
    if(! strand_.running_in_this_thread())
        return strand_.post(std::bind (&PeerImp::stop, shared_from_this()));
...
{
    if (! strand_.running_in_this_thread())
        return strand_.post(std::bind (&PeerImp::send, shared_from_this(), m));
    ...
    boost::asio::async_write (stream_, 
                            ...
                            strand_.wrap(std::bind(&PeerImp::onWriteMessage, 
                                                    shared_from_this(),
                                                    std::placeholders::_1, 
                                                    std::placeholders::_2
                                                  )
...
PeerImp::crawl() const
{
    ...
    return std::equal(iter->second.begin(), iter->second.end(), "public");
}
...
PeerImp::json()
{
    ...
    ret[jss::address]      = remote_address_.address().to_string();
    ...
    if (last_status_.has_newstatus ())
    {
        ...
        default:
            ...
            p_journal_.warning << "Unknown status: " << last_status_.newstatus ();
        }
    }
...
PeerImp::hasLedger (uint256 const& hash, std::uint32_t seq) const
{
    ...
    return std::find (recentLedgers_.begin(), recentLedgers_.end(), hash) != recentLedgers_.end();
}
...
PeerImp::close()
{
    ...
    if (socket_.is_open())
    {
        ...
        {
            if(journal_.debug) 
                journal_.debug << "Closed";
        }
        else
        {
            if(journal_.info) 
                journal_.info << "Closed";
        }
    }
}
...
PeerImp::fail(std::string const& name, error_code ec)
{
    ...
    if (socket_.is_open())
        if (journal_.debug) 
            journal_.debug << name << ": " << ec.message();
...
PeerImp::gracefulClose()
{
    ...
    stream_.async_shutdown(strand_.wrap(std::bind(&PeerImp::onShutdown,shared_from_this(), std::placeholders::_1)));
}
...
PeerImp::setTimer()
{
    ...
    timer_.expires_from_now(std::chrono::seconds((lastPingSeq_ == 0) ? 3 : 15), ec);

    if (ec)
    {
        if (journal_.error) 
            journal_.error << "setTimer: " << ec.message();

        return;
    }

    timer_.async_wait(strand_.wrap(std::bind(&PeerImp::onTimer,shared_from_this(), std::placeholders::_1)));
}
...
PeerImp::onTimer (error_code const& ec)
{
    ...
    if (ec)
    {
        ...
        if (journal_.error) 
            journal_.error << "onTimer: " << ec.message();
...
PeerImp::onShutdown(error_code ec)
{
    ...
    {
        if (journal_.error) 
            journal_.error << "onShutdown: expected error condition";
...
void PeerImp::doAccept()
{
    ...
    if(journal_.debug) 
        journal_.debug << "doAccept: " << convert_endpoint_to_string(remote_address_);

    bool success;
    uint256 sharedValue;
    std::tie(sharedValue, success) = makeSharedValue(ssl_bundle_->stream.native_handle(), journal_);
    ...
    auto resp = makeResponse(! overlay_.peerFinder().config().peerPrivate
                            , http_message_
                            , sharedValue);
    ...
    if(journal_.info) 
        journal_.info << "Protocol: " << to_string(protocol);

    if(journal_.info) 
        journal_.info << "Public Key: " << publicKey_.humanNodePublic();

    bool const cluster = getApp().getUNL().nodeInCluster(publicKey_, name_);
    if (cluster)
        if (journal_.info)
            journal_.info << "Cluster name: " << name_;
        ...
        hello_.ledgerclosed ().size () == (256 / 8)))
    {
        ...
        if ((hello_.has_ledgerprevious ()) && (hello_.ledgerprevious ().size () == (256 / 8)))
        {
            memcpy (previousLedgerHash_.begin (), hello_.ledgerprevious ().data (), 256 / 8);
...
PeerImp::onWriteResponse (error_code ec, std::size_t bytes_transferred)
{
    ...
    if(journal_.trace)
    {
        if (bytes_transferred > 0) 
            journal_.trace << "onWriteResponse: " << bytes_transferred << " bytes";
        else 
...
PeerImp::onReadMessage (error_code ec, std::size_t bytes_transferred)
{
    ...
    {
        if (bytes_transferred > 0) 
            journal_.trace << "onReadMessage: " << bytes_transferred << " bytes";
        else 
    ...
    while (read_buffer_.size() > 0)
    {
        ...
        std::tie(bytes_consumed, ec) = invokeProtocolMessage(read_buffer_.data(), *this);
...
PeerImp::onWriteMessage (error_code ec, std::size_t bytes_transferred)
{
    ...
    {
        if (bytes_transferred > 0)
            journal_.trace << "onWriteMessage: " << bytes_transferred << " bytes";
        else 
    ...
    {
        // Timeout on writes only
        return boost::asio::async_write (stream_, 
                                        boost::asio::buffer(send_queue_.front()->getBuffer()), 
                                        strand_.wrap(std::bind(&PeerImp::onWriteMessage, shared_from_this(),
                                                                std::placeholders::_1,
                                                                std::placeholders::_2)
                                                    )
                                        );
    ...
    if (gracefulClose_)
    {
        return stream_.async_shutdown(strand_.wrap(std::bind(&PeerImp::onShutdown, shared_from_this(),std::placeholders::_1)));
    }
}
...
PeerImp::error_code
PeerImp::onMessageBegin (std::uint16_t type, std::shared_ptr<::google::protobuf::Message> const& m)
{
    load_event_ = getApp().getJobQueue ().getLoadEventAP (jtPEER, protocolMessageName(type));

    fee_ = bessel::Resource::feeLightPeer;
...
PeerImp::onMessage (std::shared_ptr<protocol::TMPing> const& m)
{
    ...
    if ((m->type () == protocol::TMPing::ptPONG) && m->has_seq ())
    {
        ...
        {
            auto estimate = std::chrono::duration_cast<std::chrono::milliseconds>(clock_type::now() - lastPingTime_);
...
{
    //  NOTE I think we should drop the peer immediately
    if (! cluster())
    {
    ...
    if (loadSources != 0)
    {
        ...
        for (int i = 0; i < m->loadsources().size(); ++i)
        {
            ...
            item.address = endpoint_from_string(node.name()).first;
            item.balance = node.cost();

            if (item.address.port() != 0)
...
{
    //  TODO This message is now obsolete due to PeerFinder
}
...
PeerImp::onMessage (std::shared_ptr <protocol::TMPeers> const& m)
{
    ...
    {
        {
            uint32 bytes = ntohl(m->nodes(i).ipv4());
            std::string s_address(std::to_string(bytes));
            std::string s_endpoint = s_address + ":" + std::to_string(m->nodes(i).ipv4port());
            boost::asio::ip::tcp::endpoint endpoint(endpoint_from_string(s_endpoint).first);
...
PeerImp::onMessage (std::shared_ptr<protocol::TMEndpoints> const& m)
{
    ...
    for (int i = 0; i < m->endpoints ().size (); ++i)
    {
        ...
        if (endpoint.hops > 0)
        {
            ...
            uint32 bytes = ntohl(tm.ipv4().ipv4());
            std::string s_address(std::to_string(bytes));
            std::string s_endpoint = s_address + ":" + std::to_string(tm.ipv4().ipv4port());


            endpoint.address = endpoint_from_string(s_endpoint).first;
        }
        else
        {
            // This Endpoint describes the peer we are connected to.
            // We will take the remote address seen on the socket and
            // store that in the boost::asio::ip::tcp::endpoint. If this is the first time,
            // then we'll verify that their listener can receive incoming
            // by performing a connectivity test.
            //

            std::string address = remote_address_.address().to_v4().to_string();
            std::string s_endpoint = address + ":" + std::to_string(tm.ipv4().ipv4port());
...
PeerImp::onMessage (std::shared_ptr<protocol::TMTransaction> const& m)
{
    ...
    try
    {
        ...
        if (! getApp().getHashRouter ().addSuppressionPeer (txID, id_, flags))
        {
        ...
        else
        {
            getApp().getJobQueue ().addJob (jtTRANSACTION
                                        ...
                                        ,  std::bind(&PeerImp::checkTransaction,  shared_from_this(), std::placeholders::_1, flags, stx));
        }
    }
    catch (...)
    {
        p_journal_.warning << "Transaction invalid: " << strHex(m->rawtransaction ());
    }
}
...
PeerImp::onMessage (std::shared_ptr <protocol::TMGetLedger> const& m)
{
    ...
    getApp().getJobQueue().addJob (jtLEDGER_REQ
                            , "recvGetLedger"
                            , std::bind(&PeerImp::getLedger, shared_from_this(), m));
}
...
PeerImp::onMessage (std::shared_ptr <protocol::TMLedgerData> const& m)
{
    ...
    if (m->ledgerhash ().size () != 32)
    {
        ...
        fee_ = bessel::Resource::feeInvalidRequest;
    ...
    if (m->type () == protocol::liTS_CANDIDATE)
    {
        ...
        getApp().getJobQueue().addJob(jtTXN_DATA
                                , "recvPeerData"
                                , std::bind(&PeerImp::peerTXData, shared_from_this()
                                                        , std::placeholders::_1
                                                        , hash
                                                        , m
                                                        , p_journal_));
...
PeerImp::onMessage (std::shared_ptr<protocol::TMProposeSet> const& m)
{
    ...
    uint256 suppression = LedgerProposal::computeSuppressionID (proposeHash
                                                            , prevLedger
                                                            , set.proposeseq()
                                                            , set.closetime ()
                                                            , Blob(set.nodepubkey ().begin ()
                                                            , set.nodepubkey ().end ())
                                                            , Blob(set.signature ().begin ()
                                                            , set.signature ().end ()));
    ...
    BesselAddress signerPublic = BesselAddress::createNodePublic (strCopy (set.nodepubkey ()));
    ...
    p_journal_.trace << "Proposal: " << (isTrusted ? "trusted" : "UNTRUSTED");

    LedgerProposal::pointer proposal = std::make_shared<LedgerProposal> (prevLedger.isNonZero () ? prevLedger : uint256()
                                                                        , set.proposeseq ()
                                                                        , proposeHash
                                                                        , set.closetime ()
    ...
    getApp().getJobQueue ().addJob (isTrusted ? jtPROPOSAL_t : jtPROPOSAL_ut
                                    ...
                                    , std::bind(&PeerImp::checkPropose, shared_from_this()
                                                            , std::placeholders::_1
                                                            , m
                                                            , proposal));
}   
...
PeerImp::onMessage (std::shared_ptr<protocol::TMStatusChange> const& m)
{
    ...
    {
        memcpy (previousLedgerHash_.begin (), m->ledgerhashprevious ().data (), 256 / 8);
    ...
    if (m->has_ledgerseq() && getApp().getLedgerMaster().getValidatedLedgerAge() < 120)
    {
...
void PeerImp::check ()
{
    ...
    if (sanity_.load() == Sanity::insane)
        reject = (insaneTime - clock_type::now()) > std::chrono::seconds (Tuning::maxInsaneTime);

    if (sanity_.load() == Sanity::unknown)
        reject = (insaneTime - clock_type::now()) > std::chrono::seconds (Tuning::maxUnknownTime);

    if (reject)
    {
        overlay_.peerFinder().on_failure (slot_);

        strand_.post (std::bind ((void (PeerImp::*)(std::string const&)) &PeerImp::fail
                                , shared_from_this()
                                , "Not useful"));
    }
}
...
PeerImp::onMessage (std::shared_ptr<protocol::TMHaveTransactionSet> const& m)
{
    ...
    //  TODO There should be no use of memcpy() throughout the program.
...
PeerImp::onMessage (std::shared_ptr <protocol::TMValidation> const& m)
{
    ...
    try
    {
        ...
        SerialIter sit (s);
        STValidation::pointer val = std::make_shared <STValidation> (std::ref (sit), false);
        ...
        if (! getApp().getHashRouter ().addSuppressionPeer (s.getSHA512Half(), id_))
        {
        ...
        if (!isTrusted && (sanity_.load () == Sanity::insane))
        {
            p_journal_.debug << "Validation: dropping untrusted from insane peer";
        }

        if (isTrusted || !getApp().getFeeTrack ().isLoadedLocal ())
        {
            getApp().getJobQueue ().addJob (isTrusted ? jtVALIDATION_t : jtVALIDATION_ut
                                        , "recvValidation->checkValidation"
                                        , std::bind(&PeerImp::checkValidation , shared_from_this()
                                                 , std::placeholders::_1
                                                 , val
                                                 , isTrusted
                                                 , m));
        ...
        {
            p_journal_.debug << "Validation: Dropping UNTRUSTED (load)";
        }
    }
    catch (std::exception const& e)
    {
        p_journal_.warning << "Validation: Exception, " << e.what();

        fee_ = Resource::feeInvalidRequest;
    }
    catch (...)
    {
        p_journal_.warning << "Validation: Unknown exception";
...
PeerImp::onMessage (std::shared_ptr<protocol::TMGetObjectByHash> const& m)
{
    ...
    if (packet.query ())
    {
        ...
        for (int i = 0; i < packet.objects_size (); ++i)
        {
            ...
            if (obj.has_hash () && (obj.hash ().size () == (256 / 8)))
            {
                ...
                //  TODO Move this someplace more sensible so we dont
                ...
                if (hObj)
                {
                    ...
                    //  NOTE "seq" in the message is obsolete
                }
            }
        }

        p_journal_.trace << "GetObj: " 
                         << reply.objects_size () 
                         << " of " 
                         << packet.objects_size ();
    ...
    else
    {
                ...
                if (obj.has_ledgerseq ())
                {
                    if (obj.ledgerseq () != pLSeq)
                    {
                        if ((pLDo && (pLSeq != 0)) &&
                            ...
                            p_journal_.debug << "GetObj: Full fetch pack for " << pLSeq;

                        pLSeq = obj.ledgerseq ();
                        pLDo = !getApp().getOPs ().haveLedger (pLSeq);

                        if (!pLDo)
                                p_journal_.debug << "GetObj: Late fetch pack for " << pLSeq;
                        else
                            progress = true;
                    }
                }
        ...
        if ((pLDo && (pLSeq != 0)) && p_journal_.active(beast::Journal::Severity::kDebug))
...
PeerImp::sendGetPeers ()
{
    ...
    Message::pointer packet = std::make_shared<Message> (msg, protocol::mtGET_PEERS);
...
{
    bool success;
    std::tie(sharedValue_, success) = makeSharedValue(stream_.native_handle(), journal_);
    if (! success)
        return false;

    auto const hello = buildHello (sharedValue_, getApp());
    auto const m = std::make_shared<Message> (std::move(hello), protocol::mtHELLO);
    send (m);
...
PeerImp::addLedger (uint256 const& hash)
{
    ...
    //  TODO See if a sorted vector would be better.
...
{
    //  TODO Invert this dependency using an observer and shared state object.
    ...
    getApp().getJobQueue ().addJob (jtPACK
                                ,"MakeFetchPack"
...
PeerImp::checkTransaction (Job&, int flags,
    STTx::pointer stx)
{
    //  TODO Rewrite to not use exceptions
    try
    {
        // Expired?
        if (stx->isFieldPresent(sfLastLedgerSequence) &&
            (stx->getFieldU32 (sfLastLedgerSequence) < getApp().getLedgerMaster().getValidLedgerIndex()))
        {
        ...
        LedgerProposal::pointer proposal)
{
    ...
    if (set.has_previousledger ())
    {
        ...
        p_journal_.trace << "proposal with previous ledger";
        ...
        {
            p_journal_.warning << "Proposal with previous ledger fails sig check";
    ...
    else
    {
        ...
        else
        {
            ...
            p_journal_.warning << "Ledger proposal fails signature check";
    ...
    if (isTrusted)
    {
        getApp().getOPs ().processTrustedProposal (proposal, packet, publicKey_, prevLedger, sigGood);
    }
    else if (sigGood && (prevLedger == consensusLCL))
    {
        // relay untrusted proposal
        p_journal_.trace << "relaying UNTRUSTED proposal";

        overlay_.relay(set, proposal->getSuppressionID());
    }
    else
    {
        p_journal_.debug << "Not relaying UNTRUSTED proposal";
    }
}
    ...
    bool isTrusted, std::shared_ptr<protocol::TMValidation> const& packet)
{
    try
    {
        ...
        {
            p_journal_.warning << "Validation is invalid";
    ...
    {
        p_journal_.trace << "Exception processing validation";
...
//  NOTE This function is way too big and cumbersome.
void
PeerImp::getLedger (std::shared_ptr<protocol::TMGetLedger> const& m)
{
    ...
    if (packet.itype () == protocol::liTS_CANDIDATE)
    {
        ...
        if (p_journal_.trace) 
            p_journal_.trace << "GetLedger: Tx candidate set";

        if ((!packet.has_ledgerhash () || packet.ledgerhash ().size () != 32))
        {
            charge (Resource::feeInvalidRequest);

            if (p_journal_.warning) 
        ...
        if (!map)
        {
            ...
            {
                if (p_journal_.debug)
                    p_journal_.debug << "GetLedger: Routing Tx set request";

                auto const v = getPeerWithTree(overlay_, txHash, this);
                if (! v)
                {
                    if (p_journal_.info) 
                        p_journal_.info << "GetLedger: Route TX set failed";
                ...
                v->send (std::make_shared<Message> (packet, protocol::mtGET_LEDGER));

                return;
            }

            if (p_journal_.debug) 
                p_journal_.debug << "GetLedger: Can't provide map ";
    ...
    else
    {
        ...
        {
            if (p_journal_.debug) 
                p_journal_.debug << "GetLedger: Too busy";
        ...
        if (p_journal_.trace) 
            p_journal_.trace << "GetLedger: Received";
        ...
        if (packet.has_ledgerhash ())
        {
            ...
            if (packet.ledgerhash ().size () != 32)
            {
                ...
                if (p_journal_.warning)
                    p_journal_.warning << "GetLedger: Invalid request";
            ...
            if (!ledger)
                if (p_journal_.trace)
                    p_journal_.trace << "GetLedger: Don't have " << ledgerhash;
                ...
                !packet.has_requestcookie ()))
            {
                ...
                {
                    if (p_journal_.trace)
                        p_journal_.trace << "GetLedger: Cannot route";
                ...
                if (p_journal_.debug)
                    p_journal_.debug << "GetLedger: Request routed";
        ...
        else if (packet.has_ledgerseq ())
        {
            ...
            {
                if (p_journal_.debug)
                    p_journal_.debug << "GetLedger: Early ledger request";
            ...
            if (! ledger)
                if (p_journal_.debug) 
                    p_journal_.debug << "GetLedger: Don't have " << packet.ledgerseq ();
        }
        ...
        else
        {
            charge (Resource::feeInvalidRequest);
            if (p_journal_.warning) 
                p_journal_.warning << "GetLedger: Unknown request";
            ...
            packet.ledgerseq () != ledger->getLedgerSeq ())))
        {
            ...
            if (ledger)
                if (p_journal_.warning)
                    p_journal_.warning << "GetLedger: Invalid sequence";
        ...
        if (packet.itype () == protocol::liBASE)
        {
            ...
            if (p_journal_.trace)
                p_journal_.trace << "GetLedger: Base data";
    ...
    {
        if (p_journal_.warning)
            p_journal_.warning << "GetLedger: Can't find map or empty request";
    ...
    if (p_journal_.trace)
        p_journal_.trace << "GetLeder: " << logMe;
    ...
    for (int i = 0; i < packet.nodeids ().size (); ++i)
    {
        ...
        {
            if (p_journal_.warning) 
                p_journal_.warning << "GetLedger: Invalid node " << logMe;
        ...
        try
        {
            if (map->getNodeFat (mn, nodeIDs, rawNodes, fatLeaves, depth))
            {
                ...
                if (p_journal_.trace) 
                    p_journal_.trace << "GetLedger: getNodeFat got " << rawNodes.size () << " nodes";
            ...
            {
                p_journal_.warning << "GetLedger: getNodeFat returns false";
            }
        }
        catch (std::exception&)
        {
            ...
            if (p_journal_.warning) 
                p_journal_.warning << "getNodeFat( " << mn << ") throws exception: " << info;
        }
    }

    if (p_journal_.info) 
        p_journal_.info << "Got request for " 
                        << packet.nodeids().size() 
                        << " nodes at depth " 
                        << depth 
                        << ", return " 
