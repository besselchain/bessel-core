...
#ifndef BESSEL_OVERLAY_PEERIMP_H_INCLUDED
#define BESSEL_OVERLAY_PEERIMP_H_INCLUDED
...
public:
    ...
    */
    ...
    enum class State
    {
        ...
        /** Running the Bessel protocol actively */
        ,active
    };
...
private:
    ...
    boost::asio::ip::tcp::endpoint remote_address_;
...
public:
    ...
    //  legacyPublicKey should be implied by the Slot
    template <class Buffers>
    PeerImp (std::unique_ptr<sslbundle>&& ssl_bundle
            , Buffers const& buffers
            , PeerFinder::Slot::ptr&& slot
            , beast::http::message&& response
            , Resource::Consumer usage
            , protocol::TMHello&& hello
            , BesselAddress const& legacyPublicKey
            , id_t id
            , OverlayImpl& overlay);
    ...
    template <class FwdIt, class = typename std::enable_if<std::is_same<typename std::iterator_traits<FwdIt>::value_type, PeerFinder::Endpoint>::value>::type>
    void
    ...
    */
    ...
    /** Return the version of besseld that the peer is running, if reported. */
    std::string
    getVersion () const;
...
public:
    ...
    error_code
    onMessageBegin (std::uint16_t type, std::shared_ptr<::google::protobuf::Message> const& m);
...
template <class Buffers>
PeerImp::PeerImp (std::unique_ptr<sslbundle>&& ssl_bundle
                , Buffers const& buffers
                , PeerFinder::Slot::ptr&& slot
                , beast::http::message&& response
                , Resource::Consumer usage
...
PeerImp::sendEndpoints (FwdIt first, FwdIt last)
{
    ...
    for (;first != last; ++first)
    {
        ...
        if (ep.address.address().is_v4())
        {
            int32_t b{0};
            std::array<unsigned char, 4> bytes = ep.address.address().to_v4().to_bytes();
            unsigned char _1 = bytes[0];
            unsigned char _2 = bytes[1];
            unsigned char _3 = bytes[2];
            unsigned char _4 = bytes[3];
            b = _1 << 24 | _2 << 16 | _3 << 8 | _4;
            tme.mutable_ipv4()->set_ipv4(htobe32(b));
        }
        else
