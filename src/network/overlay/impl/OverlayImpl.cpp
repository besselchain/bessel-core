...
{   
    ...
    timer_.async_wait(overlay_.strand_.wrap(std::bind(&Timer::on_timer
                                                , shared_from_this()
                                                , std::placeholders::_1)));
}
...
{   
    if (ec || overlay_.isStopping())
    {
        if (ec && ec != boost::asio::error::operation_aborted)
            if (overlay_.journal_.error) 
                overlay_.journal_.error << "on_timer: " << ec.message ();
...
OverlayImpl::OverlayImpl (Setup const& setup
    , Stoppable& parent
    ...
    , m_peerFinder (PeerFinder::make_Manager (*this
                                        , io_service
                                        , get_seconds_clock ()
                                        , deprecatedLogs ().journal ("PeerFinder"), config))
    , m_resolver (resolver)
    , next_id_ (1)
    , timer_count_ (0)
{
...
Handoff
OverlayImpl::onHandoff (std::unique_ptr <sslbundle>&& ssl_bundle
                    ...
                    , endpoint_type remote_endpoint)
{
    ...
    if (journal.trace) 
        journal.trace << "Peer connection upgrade from " << remote_endpoint;
    ...
    {
        if (journal.trace) 
            journal.trace << remote_endpoint << " failed: " << ec.message ();
    ...
    auto const slot = m_peerFinder->new_inbound_slot (local_endpoint
                                                    , remote_endpoint);
    ...
    {
        auto const types = std::rfc2616::split_commas(request.headers["Connect-As"]);
        if (std::find_if(types.begin (), types.end (),
                [](std::string const& s)
                {
                    return std::equal (std::begin(s), std::end(s), "peer");
                }) == types.end ())
        {
            handoff.moved = false;
    ...
    uint256 sharedValue;
    std::tie(sharedValue, success) = makeSharedValue (ssl_bundle->stream.native_handle (), journal);
    if(! success)
        return handoff;

    BesselAddress publicKey;
    std::tie(publicKey, success) = verifyHello (hello, sharedValue, journal, getApp ());
    if(! success)
        return handoff;
    ...
    auto const result = m_peerFinder->activate (slot, publicKey.toPublicKey (), cluster);
    if (result != PeerFinder::Result::success)
    {
        if (journal.trace) 
            journal.trace << "Peer " << remote_endpoint << " redirected, slots full";
...
OverlayImpl::isPeerUpgrade (beast::http::message const& request)
{
    ...
    auto const versions = parse_ProtocolVersions(request.headers["Upgrade"]);
    if (versions.size () == 0)
        return false;
                                ...
                                , address_type remote_address)
{
    Json::Value json (Json::objectValue);
    {
        ...
        for (auto const& _ : m_peerFinder->redirect (slot))
            ips.append(_.address.address().to_string ());
    }
...
OverlayImpl::connect (boost::asio::ip::tcp::endpoint const& remote_endpoint)
{
    ...
    if (usage.disconnect ())
    {
        if (journal_.info) 
        {
            ...
            journal_.info << "Over resource limit: " << convert_endpoint_to_string(remote_endpoint);
        }
    ...
    {
        if (journal_.debug) 
            journal_.debug << "Connect: No slot for " << convert_endpoint_to_string(remote_endpoint);

        return;
    }

    auto const p = std::make_shared<ConnectAttempt>(io_service_
                                                , remote_endpoint
                                                , usage
                                                , setup_.context
                                                , next_id_++
                                                , slot
                                                , deprecatedLogs().journal("Peer")
                                                , *this);
...
OverlayImpl::add_active (std::shared_ptr<PeerImp> const& peer)
{
    ...
    // Now track this peer
    {
        auto const result (m_shortIdMap.emplace (std::piecewise_construct
    ...
    {
        auto const result (m_publicKeyMap.emplace(peer->getNodePublic (), peer));
...
OverlayImpl::onPrepare()
{
    ...
    // If no IPs are specified, use the Bessel Labs round robin
    // pool to get some servers to insert into the boot cache.
    if (bootstrapIps.empty ())
        bootstrapIps.push_back ("r.bessel.com 51235");

    if (!bootstrapIps.empty ())
    {
        m_resolver.resolve (bootstrapIps,
            [this](std::string const& name,
                std::vector<boost::asio::ip::tcp::endpoint> const& addresses)
            {
    ...
    // Add the ips_fixed from the besseld.cfg file
...
OverlayImpl::activate (std::shared_ptr<PeerImp> const& peer)
{
    ...
    journal_.debug << "activated " 
                   << peer->getRemoteAddress().address().to_string() + std::to_string(peer->getRemoteAddress().port()) 
                   << " (" << peer->id() 
                   << ":"
                   << peer->getNodePublic().toPublicKey() << ")";
...
void
OverlayImpl::onPeerDeactivate (Peer::id_t id, BesselAddress const& publicKey)
{
    std::lock_guard <decltype(mutex_)> lock (mutex_);
...
std::size_t
OverlayImpl::selectPeers (PeerSet& set
                    , std::size_t limit
                    , std::function<bool(std::shared_ptr<Peer> const&)> score)
{
    ...
    std::vector<item> v;
    {
        ...
        {
            v.emplace_back(e->getScore (score (e)), std::move (e));
        });
    }
...
/** The number of active peers on the network
    ...
    and are running the Bessel protocol.
*/
std::size_t
OverlayImpl::size()
{
...
OverlayImpl::crawl()
{
    ...
    for (auto const& e : m_publicKeyMap)
    {
        if (auto const sp = e.second.lock ())
        {
            ...
            pv[jss::type] = "peer";
            pv[jss::public_key] = bessel::base64_encode(sp->getNodePublic().getNodePublic().data()
...
OverlayImpl::relay (protocol::TMProposeSet& m, uint256 const& uid)
{
    ...
    if (! getApp ().getHashRouter ().swapSet (uid, skip, SF_RELAYED))
        return;
...
make_Overlay (Overlay::Setup const& setup
            ...
            , BasicConfig const& config)
{
    return std::make_unique <OverlayImpl> (setup
                                        , parent
                                        , serverHandler
                                        , resourceManager
                                        , resolver
