...
//  NOTE The txnData constructor parameter is a code smell.
...
//
PeerSet::PeerSet (uint256 const& hash, 
                int interval, 
                bool txnData,
                clock_type& clock, 
...
void PeerSet::setTimer ()
{
    ...
    mTimer.async_wait (std::bind (&PeerSet::TimerEntry, pmDowncast (), std::placeholders::_1));
}
...
void PeerSet::invokeOnTimer ()
{
    ...
    if (!isProgress())
    {
        ...
        WriteLog (lsDEBUG, InboundLedger) << "Timeout("
                                          << mTimeouts
                                          << ") pc=" 
                                          << mPeers.size ()
                                          << " acquiring " 
...
void PeerSet::TimerEntry (std::weak_ptr<PeerSet> wptr, const boost::system::error_code& result)
{
    ...
    if (ptr)
    {
        //  NOTE So this function is really two different functions depending on
        ...
        if (ptr->mTxnData)
        {
            getApp().getJobQueue ().addJob (jtTXN_DATA, "timerEntryTxn",
                std::bind (&PeerSet::TimerJobEntry, std::placeholders::_1, ptr));
        }
        else
        {
...
void PeerSet::sendRequest (const protocol::TMGetLedger& tmGL)
{
    ...
    Message::pointer packet (std::make_shared<Message> (tmGL, protocol::mtGET_LEDGER));
