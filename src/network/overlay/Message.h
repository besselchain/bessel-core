...
#ifndef BESSEL_OVERLAY_MESSAGE_H_INCLUDED
#define BESSEL_OVERLAY_MESSAGE_H_INCLUDED
...
//  NOTE If we forward declare Message and write out shared_ptr
//             instead of using the in-class typedef, we can remove the entire
//             bessel.pb.h from the main headers.
//
...
public:
    ...
    */
    ...
    typename std::enable_if<std::is_same<typename
        FwdIter::value_type, std::uint8_t>::value, std::size_t>::type
    size (FwdIter first, FwdIter last)
    {
        if (std::distance(first, last) < Message::kHeaderBytes)
            return 0;
    ...
    {
        return size(buffers_begin(buffers), buffers_end(buffers));
    }
    ...
    template <class FwdIter>
    static
    typename std::enable_if<std::is_same<typename FwdIter::value_type, std::uint8_t>::value, int>::type
    type (FwdIter first, FwdIter last)
    {
        if (std::distance(first, last) < Message::kHeaderBytes)
            return 0;

        return (int{*std::next (first, 4)} << 8) | *std::next (first, 5);
    }
    ...
    {
        return type (buffers_begin (buffers), buffers_end (buffers));
    }
    /** @} */
...
private:
    ...
    {
        return boost::asio::buffers_iterator<BufferSequence, Value>::begin (buffers);
    }
    ...
    {
        return boost::asio::buffers_iterator<BufferSequence, Value>::end (buffers);
    }
