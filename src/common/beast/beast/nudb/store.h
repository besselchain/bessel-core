...
private:
    ...
    //  Unfortunately boost::optional doesn't support
    ...
    Args&&... args)
{
    ...
    verify<Hasher> (kh);
    verify<Hasher> (dh, kh);
    auto s = beast::make_unique<state>(
...
next:
    ...
    if (iter != s_->c1.end())
        return fetch(h, key,
            iter->second, handler);
    //  Audit for concurrency
    genlock <gentex> g (g_);
    m.unlock();
    buffer buf (s_->kh.block_size);
    //  Constructs with garbage here
...
store<Hasher, Codec, File>::commit()
{
    ...
    //  Should the bulk_writer buffer size be tunable?
    {
