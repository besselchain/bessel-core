...
#ifndef BESSEL_BASICS_BASICCONFIG_H_INCLUDED
#define BESSEL_BASICS_BASICCONFIG_H_INCLUDED
...
{
private:
    std::map <std::string, Section> map_;
    ...
    std::string const& name, Section const& section)
{
    ...
    try
    {
        //  TODO Use try_lexical_convert (boost 1.56.0)
        target = boost::lexical_cast <T> (result.first);
        return true;
    }
    catch (boost::bad_lexical_cast&)
