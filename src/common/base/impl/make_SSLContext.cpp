...
{
    std::mutex lock;
    std::unordered_map<const SSL*, std::chrono::steady_clock::time_point> set;
    StaticData()
    { }
};
...
make_DH(std::string const& params)
{
    ...
    if (p == nullptr)
        bessel::FatalError ("d2i_DHparams returned nullptr.",
            __FILE__, __LINE__);
    return dh_ptr(dh);
}
...
getDH (int keyLength)
{
    ...
    {
        bessel::FatalError ("unsupported key length", __FILE__, __LINE__);
    }
...
disallowRenegotiation (SSL const* ssl, bool isNew)
{
    ...
    static bessel::static_initializer <StaticData> static_data;

    auto& sd (static_data.get ());
    std::lock_guard <std::mutex> lock (sd.lock);
    auto const expired (std::chrono::steady_clock::now() - std::chrono::minutes(4));

    // Remove expired entries
    for (auto iter (sd.set.begin ());
        (iter != sd.set.end ()) && (iter->second <= expired);
        iter = sd.set.begin ())
    {
        sd.set.erase (iter);
    }
    ...
    if (iter != sd.set.end ())
    {
        ...
        sd.set.emplace(iter->first, std::chrono::steady_clock::now());
    }
    else
    {
        sd.set.emplace (ssl, std::chrono::steady_clock::now());
    }
    ...
    std::string key_file, std::string cert_file, std::string chain_file)
{
    ...
    if (! cert_file.empty ())
    {
        ...
        if (ec)
        {
            bessel::FatalError (error_message (
    ...
    if (! chain_file.empty ())
    {
            ...
            for (;;)
            {
                ...
                if (! cert_set)
                {
                    if (SSL_CTX_use_certificate (ssl, x) != 1)
                        bessel::FatalError ("Problem retrieving SSL certificate from chain file.",
                            __FILE__, __LINE__);
                ...
                {
                    X509_free (x);
                    bessel::FatalError ("Problem adding SSL chain certificate.",
                        __FILE__, __LINE__);
                }
            }
        ...
        {
            fclose (f);
            bessel::FatalError ("Reading the SSL chain file generated an exception.",
                __FILE__, __LINE__);
        }
    }
    ...
    {
        bessel::FatalError ("Invalid key in SSL private key file.",
            __FILE__, __LINE__);
    }
}
...
make_SSLContext()
{
    ...
    //  NOTE, It seems the WebSocket context never has
