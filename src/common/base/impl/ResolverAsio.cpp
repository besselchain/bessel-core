...
public:
    ...
    void stop_async ()
    {
        ...
        {
            m_io_service.dispatch (m_strand.wrap (std::bind (&ResolverAsioImpl::do_stop,
                                                             this, 
    ...
    void resolve (std::vector<std::string> const& names, HandlerType const& handler)
    {
        assert (m_stop_called == false);
        assert (m_stopped == true);
        assert (!names.empty());
        ...
        //           reducing cost.
        m_io_service.dispatch (m_strand.wrap (std::bind (&ResolverAsioImpl::do_resolve, 
                                                         this,
                                                         names,
                                                         handler,
                                                         CompletionCounter (this))));
    ...
    void do_finish (std::string name,
                    ...
                    CompletionCounter)
    {
        ...
        m_io_service.post (m_strand.wrap (std::bind (&ResolverAsioImpl::do_work, 
                                                    this,
                                                    CompletionCounter (this))));
    }
    ...
    void do_work (CompletionCounter)
    {
        ...
        if (hp.first.empty ())
        {
            m_journal.error << "Unable to parse '" << name << "'";
        ...
        boost::asio::ip::tcp::resolver::query query (hp.first, hp.second);

        m_resolver.async_resolve (query, std::bind (&ResolverAsioImpl::do_finish, 
                                                    this,
                                                    name,
                                                    std::placeholders::_1, 
                                                    handler,
                     ...
                     CompletionCounter)
    {
        ...
        if (m_stop_called == false)
        {
            ...
            m_journal.debug << "Queued new job with "
                            << names.size() 
                            << " tasks. " 
                            << m_work.size()
                            << " jobs outstanding.";
...
ResolverAsio *ResolverAsio::New (boost::asio::io_service& io_service,
                                 beast::Journal journal)
{
