...
#include <common/misc/impl/AccountTxPaging.h>
...
#include <network/resource/Gossip.h>
...
public:
    //  TODO Make LedgerMaster a SharedPtr or a reference.
    //
    NetworkOPsImp (
    ...
    */
    ...
    boost::posix_time::ptime getConnectLocalTime() const;
    boost::posix_time::time_duration getServerStateChangeTime() const;
    ...
    //  Workaround for MSVC std::function which doesn't swallow return
    // types.
    //
    ...
    int findTransactionsByDestination (
        ...
        BesselAddress const& destinationAccount,
    ...
    AccountState::pointer getAccountState (
        Ledger::ref lrLedger, BesselAddress const& accountID);
	AccountState::pointer getAccountState1(
		Ledger::ref lrLedger, BesselAddress const& accountID);
    ...
    Json::Value getOwnerInfo (
        Ledger::pointer lpLedger, BesselAddress const& naAccount);
    ...
    void processTrustedProposal (
        ...
        BesselAddress nodePublic, uint256 checkLedger, bool sigGood);
    ...
    //  TODO Try to make all these private since they seem to be...private
    //
    ...
    */
    ...
    void storeProposal (
        LedgerProposal::ref proposal, BesselAddress const& peerPublic);
    ...
    std::string transactionsSQL (
        std::string selection, BesselAddress const& account,
    ...
    AccountTxs getAccountTxs (
        BesselAddress const& account,
        std::int32_t minLedger, std::int32_t maxLedger, bool descending,
        std::uint32_t offset, int limit, bool bAdmin);

    AccountTxs getTxsAccount (
        BesselAddress const& account, std::int32_t minLedger,
    ...
    std::vector<BesselAddress> getLedgerAffectedAccounts (
    ...
    void subAccount (
        ...
        const hash_set<BesselAddress>& vnaAccountIDs, bool rt);
    void unsubAccount (
        InfoSub::ref ispListener,
        const hash_set<BesselAddress>& vnaAccountIDs,
        bool rt);
...
private:
    ...
    Json::Value pubBootstrapAccountInfo (
        Ledger::ref lpAccepted, BesselAddress const& naAccountID);
...
//    boost::posix_time::ptime mConnectTime;
    ...
    boost::posix_time::ptime mConnectLocalTime ;     //
 
    boost::posix_time::ptime mServerStateChangeTime;   //  
...
{
    {
        ...
        //  NOTE This is for diagnosing a crash on exit
...
void NetworkOPsImp::processClusterTimer ()
{
    ...
    for (auto& item: gossip.items)
    {
        ...
        node.set_name (convert_endpoint_to_string(item.address));
...
boost::posix_time::time_duration NetworkOPsImp::getServerStateChangeTime() const
{
  boost::posix_time::ptime nowTime = boost::posix_time::second_clock::universal_time (); 
  return nowTime - mServerStateChangeTime;
}

boost::posix_time::ptime NetworkOPsImp::getConnectLocalTime() const
{
   return mConnectLocalTime;
}
...
boost::posix_time::ptime NetworkOPsImp::getNetworkTimePT (int& offset) const
{
    ...
    //  TODO Replace this with a beast call
    ...
    bool bAdmin, bool bLocal, bool bFailHard, stCallback callback)
{
...

    {
        ...
        if (r == tesSUCCESS)
        {
            ...
            //  NOTE The value of trans can be changed here!
...
int NetworkOPsImp::findTransactionsByDestination (
    ...
    BesselAddress const& destinationAccount, std::uint32_t startLedgerSeq,
...
AccountState::pointer  NetworkOPsImp::getAccountState1(
	Ledger::ref lrLedger, BesselAddress const& accountID)
{
	return lrLedger->getAccountState1(accountID);
}
//
// Directory functions
//
    ...
    if (sleNode)
    {
        ...
        do
        {
            for (auto const& uDirEntry : sleNode->getFieldV256 (sfIndexes))
            {
                ...
                case ltBESSEL_STATE:
                    if (!jvObjects.isMember (jss::bessel_lines))
                    {
                        jvObjects[jss::bessel_lines] =
                                Json::Value (Json::arrayValue);
                    }

                    jvObjects[jss::bessel_lines].append (sleCur->getJson (0));
                    break;
...
void NetworkOPsImp::processTrustedProposal (
    LedgerProposal::pointer proposal,
    std::shared_ptr<protocol::TMProposeSet> set, BesselAddress nodePublic,
...
{
    //  TODO Don't hold the lock across calls to send...make a copy of the
    ...
    if (!mSubServer.empty ())
    {
        ...
        for (auto i = mSubServer.begin (); i != mSubServer.end (); )
        {
            ...
            //  TODO research the possibility of using thread queues and
...
void NetworkOPsImp::setMode (OperatingMode om)
{
    ...
    if ((om >= omCONNECTED) && (mMode == omDISCONNECTED))
        mConnectLocalTime = boost::posix_time::second_clock::local_time();
//        mConnectTime = boost::posix_time::second_clock::universal_time ();
    mMode = om;

    mServerStateChangeTime = boost::posix_time::second_clock::universal_time ();   
    ...
    std::uint32_t offset, int limit, bool bAdmin)
{
...

    {
        ...
        boost::optional<std::string> sociTxnBlob, sociTxnMetaBlob;
        soci::indicator rti, tmi;
        std::string rawTxn, txnMeta;
...
NetworkOPsImp::getLedgerAffectedAccounts (std::uint32_t ledgerSeq)
{
    ...
    {
        auto db = getApp().getTxnDB ().checkoutDb ();
        boost::optional<std::string> accountBlob;
        soci::indicator bi;
...
Json::Value NetworkOPsImp::getServerInfo (bool human, bool admin)
{
    ...
    info ["startup_time"] =  to_simple_string(getConnectLocalTime());
    info [jss::server_state] = strOperatingMode ()+ "   "+to_simple_string(getServerStateChangeTime());
    ...
    if (lpClosed)
    {
        ...
        BesselAddress   accountIDMng;
        BesselAddress   accountIDIssuer;

        (void) accountID.setAccountID (lpClosed->getFeeAccountID ());
        (void) accountIDMng.setAccountID (lpClosed->getManagerAccountID ());
        (void) accountIDIssuer.setAccountID (lpClosed->getIssuerOpAccountID ());
		...
		l[jss::fee_account]    = accountID.humanAccountID ();
            l[jss::close_time] =
                    Json::Value::UInt (lpClosed->getCloseTimeNC ());
        }
        else
        {
            l[jss::base_fee_swt] = static_cast<double> (baseFee) /
                    SYSTEM_CURRENCY_PARTS;
            l[jss::reserve_base_swt]   =
                static_cast<double> (Json::UInt (
                    lpClosed->getReserve (0) * baseFee / baseRef))
                    / SYSTEM_CURRENCY_PARTS;
            l[jss::reserve_inc_swt]    =
                static_cast<double> (Json::UInt (
                    lpClosed->getReserveInc () * baseFee / baseRef))
                    / SYSTEM_CURRENCY_PARTS;
		l[jss::fee_account_swt]    = accountID.humanAccountID ();
		l[jss::manager_account]    = accountIDMng.humanAccountID ();		
		l[jss::issuerop_account]    = accountIDIssuer.humanAccountID ();		
    ...
    Ledger::ref lpCurrent)
{
    ...
    {
		auto const account(stTxn.getOperationAccount().getAccountID());
...
void NetworkOPsImp::unsubAccount (
    InfoSub::ref isrListener,
    hash_set<BesselAddress> const& vnaAccountIDs,
    bool rt)
{
    for (auto const& naAccountID : vnaAccountIDs)
    {
...
{ // CAUTION: This is the old get book page logic
    ...
    auto uTransferRate = besselTransferRate (lesActive, book.out.account);
