...
#ifndef BESSEL_APP_MISC_NETWORKOPS_H_INCLUDED
#define BESSEL_APP_MISC_NETWORKOPS_H_INCLUDED
...
/** Provides server functionality for clients.
    ...
    besseld / NetworkOPs. However, client software connecting to non-local
    instances of besseld will need to be hardened to protect against hostile
...
public:
    ...
    //  TODO Fix OrderBookDB to not need this unrelated type.
    //
...
public:
    ...
    //  TODO Make this a TxCallback structure
    ...
    virtual AccountState::pointer getAccountState (Ledger::ref lrLedger,
        BesselAddress const& accountID) = 0;
	virtual  AccountState::pointer getAccountState1(Ledger::ref lrLedger,
		BesselAddress const& accountID) = 0;
    ...
    virtual Json::Value getOwnerInfo (Ledger::pointer lpLedger,
        BesselAddress const& naAccount) = 0;
    ...
    //  TODO rename to setNeedNetworkLedger
    ...
    virtual void storeProposal (LedgerProposal::ref proposal,
        BesselAddress const& peerPublic) = 0;
    ...
    virtual std::string transactionsSQL (std::string selection,
        BesselAddress const& account, std::int32_t minLedger, std::int32_t maxLedger,
    ...
    virtual MetaTxsList getAccountTxsB (BesselAddress const& account,
        std::int32_t minLedger, std::int32_t maxLedger,  bool descending,
            std::uint32_t offset, int limit, bool bAdmin) = 0;

    virtual MetaTxsList getTxsAccountB (BesselAddress const& account,
        std::int32_t minLedger, std::int32_t maxLedger,  bool forward,
        Json::Value& token, int limit, bool bAdmin) = 0;

    virtual std::vector<BesselAddress> getLedgerAffectedAccounts (
