...
#ifndef BESSEL_CORE_LOADFEETRACKIMP_H_INCLUDED
#define BESSEL_CORE_LOADFEETRACKIMP_H_INCLUDED
...
public:
    ...
    // Scale from fee units to millionths of a bessel
    ...
    bool isLoadedLocal ()
    {
        //  TODO This could be replaced with a SharedData and
    ...
    Json::Value getJson (std::uint64_t baseFee, std::uint32_t referenceFeeUnits)
    {
...

        {
            ...
            // base_fee = The cost to send a "reference" transaction under no load, in millionths of a Bessel
            j[jss::base_fee] = Json::Value::UInt (baseFee);

            // load_fee = The cost to send a "reference" transaction now, in millionths of a Bessel
...
private:
    //  TODO Move this function to some "math utilities" file
    // compute (value)*(mul)/(div) - avoid overflow but keep precision
    std::uint64_t mulDiv (std::uint64_t value, std::uint32_t mul, std::uint64_t div)
    {
        //  TODO replace with beast::literal64bitUnsigned ()
        //
